...
export async function confirmFormationReservationPayment(session) {
  ...
  ({ invoice } = await prisma.$transaction(async (tx) => {
    ...
    if (err.message === "RESERVATION_NO_LONGER_PENDING") {
      ...
      console.warn(
        `[confirmFormationReservationPayment] Reservation cancelled concurrently, refunding:`,
        session.id
      );
      await refundSession(session);
      return { received: true, refunded: true, reason: "reservation cancelled concurrently" };
    }
    if (err.message === "HOLD_EXPIRED_OVERBOOKED" || err.message === "INVALID_SESSION_CAPACITY") {
      console.warn(
        `[confirmFormationReservationPayment] Hold expired and seats resold for reservation ${reservation.id}, refunding:`,
        session.id
      );
