...
import { DASHBOARD_PERMISSIONS, isTillCashOperator } from "@/lib/authorization";
...
export async function countPickupsToVerify(user, grantedPermissions = []) {
  ...
  // Boutique orders are the salon's own screen: admin and Marie only.
  if (!isTillCashOperator(user)) return 0;
