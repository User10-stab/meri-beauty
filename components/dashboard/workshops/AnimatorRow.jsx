"use client";

import { RowActions } from "../Tables/RowActions";
import { Mail, Phone, Globe } from "lucide-react";
import { useTranslations } from "next-intl";

function InstagramIcon({ className = "w-4 h-4" }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className} aria-hidden="true">
      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
    </svg>
  );
}

function FacebookIcon({ className = "w-4 h-4" }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className} aria-hidden="true">
      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
    </svg>
  );
}

export function AnimatorRow({ row, onView, onEdit, onDelete }) {
  const t = useTranslations("dashboardWorkshops.animators");
  return (
    <tr className="group border-b border-gray-100 transition-colors hover:bg-gray-50/70">
      {/* Avatar & Name */}
      <td className="px-4 py-4 pl-5 align-middle">
        <div className="flex items-center gap-3">
          {row.avatar ? (
            <img
              src={row.avatar}
              alt={row.name}
              className="h-10 w-10 flex-shrink-0 rounded-full object-cover border border-gray-100"
            />
          ) : (
            <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-indigo-50 text-sm font-bold text-indigo-600 border border-indigo-200 uppercase select-none">
              {row.name.slice(0, 2)}
            </div>
          )}

          <div>
            <span className="block font-medium text-gray-800">{row.name}</span>
            {row.bio && (
              <span className="text-xs text-gray-400 block max-w-xs truncate">
                {row.bio}
              </span>
            )}
          </div>
        </div>
      </td>

      {/* Email */}
      <td className="px-4 py-4 align-middle text-gray-600 text-sm">
        {row.email ? (
          <div className="flex items-center gap-1.5">
            <Mail size={14} className="text-gray-400" />
            <a href={`mailto:${row.email}`} className="hover:underline">{row.email}</a>
          </div>
        ) : (
          <span className="text-xs text-gray-400 italic">—</span>
        )}
      </td>

      {/* Phone */}
      <td className="px-4 py-4 align-middle text-gray-600 text-sm">
        {row.phone ? (
          <div className="flex items-center gap-1.5">
            <Phone size={14} className="text-gray-400" />
            <a href={`tel:${row.phone}`} className="hover:underline">{row.phone}</a>
          </div>
        ) : (
          <span className="text-xs text-gray-400 italic">—</span>
        )}
      </td>

      {/* Social Links */}
      <td className="px-4 py-4 align-middle">
        <div className="flex items-center gap-2">
          {row.website ? (
            <a
              href={row.website}
              target="_blank"
              rel="noopener noreferrer"
              className="text-gray-400 hover:text-indigo-600 transition-colors"
              title={t("socialTitles.website")}
            >
              <Globe size={16} />
            </a>
          ) : null}

          {row.instagram ? (
            <a
              href={row.instagram}
              target="_blank"
              rel="noopener noreferrer"
              className="text-gray-400 hover:text-pink-600 transition-colors"
              title={t("socialTitles.instagram")}
            >
              <InstagramIcon className="w-4 h-4" />
            </a>
          ) : null}

          {row.facebook ? (
            <a
              href={row.facebook}
              target="_blank"
              rel="noopener noreferrer"
              className="text-gray-400 hover:text-blue-600 transition-colors"
              title={t("socialTitles.facebook")}
            >
              <FacebookIcon className="w-4 h-4" />
            </a>
          ) : null}

          {!row.website && !row.instagram && !row.facebook ? (
            <span className="text-xs text-gray-400 italic">—</span>
          ) : null}
        </div>
      </td>

      {/* Actions */}
      <td className="px-4 py-4 pr-5 align-middle">
        <RowActions
          row={row}
          onView={onView}
          onEdit={onEdit}
          onDelete={onDelete}
        />
      </td>
    </tr>
  );
}
