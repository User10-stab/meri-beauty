...
export async function settleReservation({
    ...
    result = await prisma.$transaction(async (tx) => {
      ...
      const openCashSession =
        method === "CASH"
          ...
          : null;
      if (method === "CASH" && !openCashSession) throw new Error("RESERVATION_CASH_SESSION_CLOSED");
  ...
  // collection.
  if (result.balance > 0 && method === "CASH") revalidateCaisseRoutes();
