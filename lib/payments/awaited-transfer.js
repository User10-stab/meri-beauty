...
/**
 * What an accepted transfer would invoice, or why it would not: the whole
 * amount must be in, no invoice may exist yet, and the buyer needs a VIES-valid
 * VAT identity (a particulier gets the ticket alone). Shared by
 * acceptAwaitedTransferInTx and the Factures preview.
 *
 * @returns {{ input: object } | { reason: "PARTIAL"|"ALREADY_INVOICED"|"NOT_VAT_REGISTERED"|"UNKNOWN_SOURCE" }}
 */
export function planAwaitedTransferInvoice(payment) {
  const details = describeAwaitedTransfer(payment);
  if (!details) return { reason: "UNKNOWN_SOURCE" };
  if (payment.invoice) return { reason: "ALREADY_INVOICED" };
  const balance = roundMoney(Number(payment.totalAmount) - Number(payment.paidAmount));
  const awaited = payment.awaitedTransferAmount == null ? balance : Math.min(Number(payment.awaitedTransferAmount), balance);
  if (roundMoney(balance - awaited) > 0.001) return { reason: "PARTIAL" };
  const customer = details.customer ?? customerForPayment(payment);
  if (!hasInvoiceableVatIdentity(customer)) return { reason: "NOT_VAT_REGISTERED" };
  return { input: awaitedTransferInvoiceInput(payment, details, customer) };
}

function awaitedTransferInvoiceInput(payment, details, customer) {
  ...
  return {
    ...
    totalInclVat: Number(payment.totalAmount),
...
export async function acceptAwaitedTransferInTx(tx, { payment, reference, now = new Date() }) {
  ...
  if (fullyPaid && !payment.invoice && hasInvoiceableVatIdentity(customer)) {
    invoice = await issueInvoice(tx, awaitedTransferInvoiceInput(payment, details, customer));
  }
