...
/**
 * Thrown by resolveCounterCustomer (lib/counter/resolve-counter-customer.js)
 * for the failures that should reach the till as-is — a rejected/invalid VAT
 * number, a customer with a VAT number and no billing address — as opposed
 * to an unexpected error worth logging under a generic message instead.
 */
export class CounterCustomerError extends Error {
  constructor(message) {
    super(message);
    this.name = "CounterCustomerError";
  }
}
