 ...
 *  2. Only reviews attached to a COMPLETED appointment, workshop/event
 *     reservation, or formation reservation, which is already enforced at
 *     write time by lib/review-eligibility.js. That is what backs the "avis
 *     vérifiés" claim displayed next to them; without it the claim would
 *     itself be the misleading practice.
 *
  ...
  try {
    const reviews = await prisma.review.findMany({
        ...
        AND: [
          ...
          { comment: { not: "" } },
          {
            OR: [
              ...
              { workshopReservation: { status: "COMPLETED" } },
              { formationReservation: { status: "COMPLETED" } },
            ],
          },
        ],
      },
      ...
      select: {
        ...
        workshopReservation: {
          select: { session: { select: { workshop: { select: { title: true } } } } },
        },
        formationReservation: {
          select: { session: { select: { formation: { select: { title: true } } } } },
        },
      },
    });
    ...
    return reviews.map((review) => ({
      ...
      service:
        review.appointment?.staffService?.service?.name ??
        review.workshopReservation?.session?.workshop?.title ??
        review.formationReservation?.session?.formation?.title ??
        null,
    }));
  } catch (error) {
    // The homepage must render even if the database is briefly unreachable —
    // an empty list simply hides the section.
    reportPublicDataError("[getPublicReviews]", error);
    return [];
  }
}
