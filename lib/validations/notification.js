 ...
 *
 * It was not kept in sync, and the consequence was not a rejected
 * notification: `createNotification`/`createNotificationsBulk` throw
 * VALIDATION_ERROR on an unknown type, and `markAppointmentNoShow` creates
 * its notifications **inside the same transaction** that flips the status —
 * so the throw rolled the whole thing back and marking an absence failed
 * outright, every time, with "Erreur lors du marquage de l'absence". There
 * were zero NO_SHOW appointments in the database when this was found.
 *
 * Four types were missing. The three CANCELLATION_REQUEST ones happened to
 * be unaffected only because their callers use `prisma.notification.createMany`
 * directly and never reach this schema — they are added here anyway, since a
 * list that claims to mirror the enum should mirror it.
 *
 * `tests/critical/notification-types-parity.test.js` now derives the enum
 * from schema.prisma and fails on any drift, because a comment asking future
 * readers to remember is what failed the first time.
 */
...
const baseCreateFields = z.object({
  userId: z
    .string({ error: "L'utilisateur est obligatoire." })
    .min(1, "L'utilisateur est obligatoire."),
  type: z.enum(NOTIFICATION_TYPES, {
    error: (issue) => (issue.input === undefined ? "Le type de notification est obligatoire." : "Type de notification invalide."),
  }),
  title: z
  ...
  status: z
    .enum(NOTIFICATION_STATUSES, {
      error: "Statut de notification invalide.",
    })
    .default("PENDING"),
  isRead: formBoolean(false),
...
export const createNotificationsBulkSchema = z
  .array(baseCreateFields, {
    error: (issue) => (issue.input === undefined ? "La liste des notifications est obligatoire." : "La liste des notifications est invalide."),
  })
...
export const listUserNotificationsSchema = z.object({
  userId: z
    ...
    .min(1, "L'utilisateur est obligatoire."),
  pageSize: z.coerce
    .number({ error: "Taille de page invalide." })
    .int()
...
export const notificationIdSchema = z.object({
  notificationId: z
    .string({ error: "L'identifiant de notification est obligatoire." })
