...
/**
 * Creates an invoice for a staff contract's fixed rent (generation only).
 *
 * Reuses the existing invoice system (issueInvoice)
 ...
 * - Recipient: staff member's email (stored on the invoice, never emailed automatically)
 ...
 * - The invoice is NEVER sent by email here: the admin sends it manually
 *   from the dashboard ("Envoyer la facture").
 *
...
export async function createAndSendStaffContractInvoice({ contract, user }) {
  ...
  // No automatic email: the invoice is generated only. The admin sends it
  // manually from the dashboard ("Envoyer la facture"). emailed stays false
  // so the UI correctly offers the manual send button (GENERATED status).
  return { invoice, emailed: false };
}
