...
export async function sendReservationTicketsForSession(prisma, sessionId) {
  ...
  for (const transaction of transactions) {
    ...
    try {
      const receipt = collectionTicketFields(transaction, invoice);
      ...
      const amount = receipt.totalInclVat.toFixed(2);
      const result = await sendEmail({
        ...
        attachments: [{ filename: `${receipt.ticketNumber}.pdf`, content: ticketPdf }],
      });
