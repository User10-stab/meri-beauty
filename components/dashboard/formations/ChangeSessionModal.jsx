...
import {
  changeFormationReservationSession,
  getFormationTransferOptions,
...
/** Formation session transfers are free admin corrections — structural
 * sibling of components/dashboard/workshops/ChangeSessionModal.jsx, minus
 * its seat-change tab: formations have no paid seat-change feature. */
export function ChangeSessionModal({ open, onClose, reservation }) {
  const t = useTranslations("dashboardFormations.changeSessionModal");
  ...
  useEffect(() => {
    ...
    getFormationTransferOptions(reservation.id)
      .then((result) => {
        if (cancelled) return;
  ...
  const previewBalance = Math.max(
    0,
    (priceDecision === APPLY_TARGET_PRICE ? Number(selectedTarget?.targetTotal ?? 0) : Number(transferData?.currentTotal ?? 0)) -
      Number(transferData?.paidAmount ?? 0)
  );
  ...
  function handleSubmit() {
    ...
    startSubmit(async () => {
      const result = await changeFormationReservationSession(reservation.id, selectedSessionId, { reason, priceDecision });
      if (result.success) {
      ...
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-white p-6 shadow-xl">
        ...
        <div className="space-y-4">
          ...
          {isLoadingOptions ? (
            ...
            <>
              <div>
                <label htmlFor="formation-transfer-session" className="mb-1 block text-sm font-medium text-gray-700">{t("newSessionLabel")}</label>
                <select
                  id="formation-transfer-session"
              ...
              <div>
                <label htmlFor="formation-transfer-reason" className="mb-1 block text-sm font-medium text-gray-700">{t("reasonLabel")}</label>
                <textarea
                  id="formation-transfer-reason"
                  value={reason}
