...
import { parseLocalDateString, buildAvailabilityForDate, slotFitsInFreeIntervals } from "@/lib/slot-availability";
...
export async function findConflictingAppointment(staffServiceId, appointmentDate, startTime, endTime, excludeAppointmentId = null) {
  ...
  // Interval overlap — same as getAvailableSlots/buildAvailability — so a late-night
  // appointment that spills past midnight still blocks the next calendar day.
  const apptDayStart = parseLocalDateString(appointmentDate);
  const apptDayEnd = parseLocalDateString(appointmentDate);
  ...
  const appointments = await prisma.appointment.findMany({
    where: {
      ...
      startTime: { lte: apptDayEnd },
      endTime: { gte: apptDayStart },
...
export async function validateAppointmentSlot(staffServiceId, appointmentDate, startTime, time, excludeAppointmentId = null) {
  ...
  const salon = await prisma.salon.findUnique({ where: { id: "main-salon" }, include: { closures: true, workingDays: true } });

  // Use interval overlap (startTime/endTime) like getAvailableSlots, not a `date` range.
  // An appointment that starts late previous day and ends early selected day
  // (e.g. date=Aug 20, endTime 01:00 Aug 21) would be missed by a `date`-only
  // filter even though it occupies the staff member early on selected day.
  const startOfDay = parseLocalDateString(appointmentDate);
  const endOfDay = parseLocalDateString(appointmentDate);
  ...
  if (!slotFitsInFreeIntervals(time, staffService.duration, availability.freeIntervals)) {
