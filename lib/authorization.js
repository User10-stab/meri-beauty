...
/**
 * Whether this user may e-mail or reprint a client-facing ticket. Admin/owner
 * always pass; any other STAFF profile passes only if granted
 * STAFF_PERMISSIONS.SEND_TICKET_EMAIL — a normal, grantable/revocable
 * dashboard permission, checked (by default) for a newly created staff
 * account (see DEFAULT_STAFF_PERMISSIONS).
 * @param {{ role?: string, email?: string, id?: string } | null | undefined} user
 * @returns {Promise<boolean>}
 */
export async function canSendTicketEmail(user) {
  return hasDashboardPermission(user, STAFF_PERMISSIONS.SEND_TICKET_EMAIL);
}
...
export const STAFF_PERMISSIONS = Object.freeze({
  ...
  // Livre de recettes, scoped to the staff member's own recorded revenue —
  // a distinct grant from CASH_REGISTER, since (once per-staff scoping
  // ships) this is no longer a shared-till view.
...
export const STAFF_PERMISSION_OPTIONS = Object.freeze([
  ...
  { key: STAFF_PERMISSIONS.CASH_REGISTER, label: "Livre de caisse", description: "Consulter et gérer le livre de caisse (ouverture/clôture automatiques, mouvements)." },
  ...
  { key: STAFF_PERMISSIONS.SEND_TICKET_EMAIL, label: "Envoi du ticket client", description: "Envoyer ou réimprimer le ticket de caisse d'un client par e-mail." },
  { key: STAFF_PERMISSIONS.MY_RECEIPTS, label: "Mes recettes", description: "Consulter uniquement les encaissements qu'il a lui-même enregistrés." },
]);
...
// ─── Per-actor data isolation — "Mes opérations" and everything it scopes ───────

/**
 * The acting User's own id, for any dashboard role — the identity every
 * self-only scoping filter in this feature keys on
 * (Transaction.recordedById, Order.createdByStaffId, InventoryMovement.
 * createdById are all User FKs, not Staff FKs). Unlike getStaffId/
 * getCurrentStaffId, this resolves for ADMIN/OWNER too, since they may have
 * no Staff row at all — see seedAdmin() in tests/e2e-dashboard/fixtures/
 * seed-dashboard.mjs — and this feature scopes them exactly like everyone
 * else.
 * @param {{ user?: { id?: string } } | null | undefined} session
 * @returns {string|null}
 */
export function getActingUserId(session) {
  return session?.user?.id ?? null;
}

/**
 * The User.id of the one exempted account (TILL_CASH_OPERATOR_EMAIL) that
 * ADMIN/OWNER may additionally see on /dashboard/operations, on top of their
 * own transactions — see isTillCashOperator. Resolves lazily by email since
 * the id isn't known statically; returns null (widens nothing) if no such
 * user exists, e.g. in local/dev where the default email has no account.
 * @returns {Promise<string|null>}
 */
export async function getTillCashOperatorUserId() {
  const { prisma } = await import("@/lib/prisma");
