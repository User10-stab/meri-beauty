...
export async function verifyVatWithVies(raw) {
  ...
  //
  // A registered name is part of the simulation, not decoration: real VIES
  // always returns one for a valid number, and it is what the invoice prints
  // as the buyer's legal name (the B2B classification itself follows the VAT
  // number — see issueInvoice). Deliberately marked so it can never be
  // mistaken for a real registry answer; a BillingProfile.companyLegalName
...
/**
 * True only when verifyVatWithVies() failed because the registry could not
 * be reached — network error, timeout, HTTP 5xx, or a member-state node
 * reporting itself down ("Le service VIES reste temporairement indisponible
 * pour ce pays"). All of those return `reason: "VIES_UNAVAILABLE"`.
 *
 * A confirmed "not registered" answer ({ success: true, valid: false }) and
 * a bad-format rejection ({ success: false } with no `reason`) are NOT
 * outages and must still block.
 *
 * Customer-facing sale/booking entry points use this to accept a well-formed
 * number PROVISIONALLY while VIES is down (vatValidatedAt set, no registry
 * name) instead of aborting the sale on a free government service being
 * unreachable. VAT treatment then follows the ordinary country-code rule in
 * lib/tax-policy.js with no special casing — a Belgian number is domestic
 * 21% either way, a foreign-EU number gets the 0% reverse charge, a non-EU
 * number never reaches here (isValidVatFormat rejects it upstream). The
 * provisional validation ages out like any other (VAT_VALIDATION_MAX_AGE_MS)
 * and VIES is re-queried automatically on the customer's next sale.
 *
 * Strict entry points (the salon's own number, the standalone checker) do
 * not call this and keep failing closed during an outage.
 */
export function isViesOutage(viesResult) {
  return Boolean(viesResult) && viesResult.success === false && viesResult.reason === "VIES_UNAVAILABLE";
}
