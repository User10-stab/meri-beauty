...
export const REFUND_DENIAL = Object.freeze({
  ...
  FINANCIAL_CORRECTION_REQUIRES_COMPLETED: "FINANCIAL_CORRECTION_REQUIRES_COMPLETED",
...
const DENIAL_MESSAGES = Object.freeze({
  ...
  [REFUND_DENIAL.FINANCIAL_CORRECTION_REQUIRES_COMPLETED]:
    "Cette réservation n'est pas encore terminée : « Correction financière » est réservée à une prestation déjà réalisée. Pour annuler la réservation et rembourser le montant déjà payé (acompte ou totalité), choisissez « Annulation par le salon ». Une demande écrite est nécessaire uniquement si l'annulation est demandée par le client.",
...
export function authorizeRefund({
  ...
  if (source === "APPOINTMENT") {
    ...
    if (trigger === "FINANCIAL_CORRECTION" && appointment.status !== "COMPLETED") {
      return deny(REFUND_DENIAL.FINANCIAL_CORRECTION_REQUIRES_COMPLETED);
    }

    if (appointment.status === "COMPLETED") {
      return trigger === "FINANCIAL_CORRECTION"
        ? ALLOW
        : deny(REFUND_DENIAL.COMPLETED_NOT_REFUNDABLE);
    }
  ...
  if (source === "WORKSHOP" || source === "FORMATION") {
    ...
    if (trigger === "FINANCIAL_CORRECTION" && reservation.status !== "COMPLETED") {
      return deny(REFUND_DENIAL.FINANCIAL_CORRECTION_REQUIRES_COMPLETED);
    }
 ...
 * moving. The NO_SHOW exception is the whole reason this is a function and
 * not an `if` inside the orchestrator: "conserver le statut historique et
 * enregistrer une correction financière".
 */
export function cancelsUnderlyingItem(trigger) {
  return trigger !== "NO_SHOW_EXCEPTION" && trigger !== "FINANCIAL_CORRECTION";
}
