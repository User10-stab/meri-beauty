...
export async function reconcileMissedCheckouts({
  ...
  // The platform account, then every practitioner's connected account: an
  // atelier/formation seat animated by an independent is charged on HER
  // account, so its Checkout Session is only listable there (same pattern as
  // reconcile-missed-refunds.js).
  const connected = await prismaClient.staff.findMany({
    where: { stripeAccountId: { not: null } },
    select: { stripeAccountId: true },
  });
  const accounts = [null, ...new Set(connected.map((s) => s.stripeAccountId))];

  for (const stripeAccountId of accounts) {
    ...
    for (;;) {
      ...
      try {
        ...
        // A connected account we can no longer read is expected and
        // permanent — skip it without paging anyone (see
        // lib/payments/stripe-account-access.js).
        if (isStripeAccountUnavailable(error)) {
          captureWarning("Compte Stripe connecté illisible — ignoré pour cette passe.", {
            area: "checkout-reconciliation",
            context: "list",
            stripeAccountId,
          });
          break;
        ...
        captureCriticalError(error, { area: "checkout-reconciliation", context: "list", stripeAccountId });
        break;
      }
      ...
      for (const session of page.data) {
        ...
        try {
          ...
          // A manual-refund flag is the durable outcome for an orphaned or
          // otherwise unfulfillable paid session. Do not retrieve and replay
          // the same Stripe session every five minutes for the full lookback
          // window after that decision has already been recorded.
          const paymentIntentId =
            typeof session.payment_intent === "string"
              ? session.payment_intent
              : session.payment_intent?.id;
          if (paymentIntentId) {
            const alreadyFlagged = await prismaClient.notification.findFirst({
              where: {
                type: "GENERAL",
                actionUrl: `https://dashboard.stripe.com/payments/${paymentIntentId}`,
              },
              select: { id: true },
            });
          ...
          const fullSession = await stripeClient.checkout.sessions.retrieve(session.id, {}, requestOptions);

          let result;
          if (kind === "order") {
            result = await fulfillOrderPayment(fullSession);
          } else if (kind === "workshop") {
            result = await confirmWorkshopReservationPayment(fullSession, { stripeAccountId });
          } else {
            result = await confirmFormationReservationPayment(fullSession, { stripeAccountId });
          }

          if (result?.flaggedForReview) {
            flagged += 1;
            console.warn(`[reconcile-missed-checkouts] Flagged for manual review, no refund issued — session ${session.id} (${kind}): ${result.reason}`);
          } else {
            reconciled += 1;
  ...
  return { checked, reconciled, flagged, failures };
}
