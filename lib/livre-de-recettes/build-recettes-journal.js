...
// Only customerName/customerEmail are ever read off this (see
// customerForPayment below) — narrower than build-payment-ticket.js's own
// CUSTOMER_SELECT, which also needs VAT fields for invoicing.
const CUSTOMER_SELECT = { fullName: true, email: true };
...
export async function buildRecettesJournal(client, { from, to, fromDate, toDate, method, category, staffId = "", staffName = null }) {
  const found = await client.transaction.findMany({
    where: {
      ...
      isDeleted: false,
      ...(method !== "ALL" ? { method } : {}),
      // Optional staff scope: only transactions behind that member's
      // appointment payments (boutique/order transactions carry no staff).
      ...(staffId ? { payment: { appointment: { staffService: { staffId } } } } : {}),
    },
