  ...
  return prisma.$transaction(async (tx) => {
    ...
    const refundTransaction = await tx.transaction.create({
      data: {
        ...
        // null for an ONLINE leg (Stripe webhook) or the missed-refund
        // reconciliation script — neither has a human actor. Only a manual
        // CASH/CARD confirmation has one (confirmManualRefundLeg, always
        // admin/owner — see requireAdmin in cancel-and-refund.js), so this
        // never lets a non-privileged staff member's refund attribution
        // leak in through a side door.
        recordedById: manual?.confirmedByUserId ?? null,
      },
    });
