 ...
 * @param {"CUSTOMER_SELF_CANCELLATION"|"CUSTOMER_REQUEST_APPROVED"|"SALON_CANCELLATION"|"NO_SHOW_EXCEPTION"|"FINANCIAL_CORRECTION"|"SHOP_RETURN"} input.trigger
...
export async function queueManualRefund(
  ...
  // Mirrors openRefundOperation.js: a refund with no invoice to credit
  // still needs a justificatif, or the customer (and the audit trail) is
  // left with nothing numbered to point to. Missing here until this fix —
  // every B2C particulier refund queued through a *cancellation* action
  // (cancelWorkshopReservation, cancelFormationReservation) got neither a
  // credit note nor a receipt number, and the worklist row permanently
  // read "document en attente" (see documentLabel() in
  // OutstandingRefunds.jsx). openRefundOperation.js — the dashboard's
  // "Cancel and refund" button — already did this correctly; this path
  // just never had it added.
  if (!creditNoteId && customerIsBusiness) {
    throw new Error("B2B_REFUND_REQUIRES_CREDIT_NOTE");
  }
  const refundReceiptNumber = creditNoteId ? null : await allocateRefundReceiptNumber(tx);
