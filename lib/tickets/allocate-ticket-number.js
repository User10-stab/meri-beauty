 ...
 *
 * A second, independently gapless series ("TS-<year>-<seq>", counter
 * "TICKET-STAFF-<year>") exists for non-privileged staff — see
 * isStaffActor below and lib/authorization.js#isTillCashOperator, which is
 * what every caller uses to decide it. Deliberately NOT imported here: this
 * module stays free of "@/" alias imports so scripts/backfill-ticket-numbers.mjs
 * keeps working unchanged under plain node — callers resolve the actor and
 * pass a plain boolean instead. Omitting it (every pre-existing call site,
 ...
 *   (see isTillCashOperator) — routes this allocation onto the separate
 *   "TS-<year>-<seq>" series instead of the default admin one.
 */
