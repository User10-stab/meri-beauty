 ...
 * number and the salon issues nothing. A second, independently gapless
 * series ("TS-<year>-<seq>", counter "TICKET-STAFF-<year>") is still wired
 * up below for exactly that case and is now unreachable — kept because it
 * never issued a single production ticket, so nothing has to be migrated,
 * and deleting the wiring would make the decision expensive to reverse. Deliberately NOT imported here: this
 ...
 * that script's header. Bare `node scripts/...` fails on production's Node 20. Omitting it (every pre-existing call site,
 * the backfill script included) keeps the original admin/default series —
 * correct for historical rows and for every flow with no staff actor at all
 * (a customer's own online checkout).
 */
...
export function formatTicketNumber(year, seq, seriesPrefix = "T") {
  return `${seriesPrefix}-${year}-${String(seq).padStart(TICKET_DIGITS, "0")}`;
}

async function claimNextTicketNumber(tx, now, isStaffActor = false) {
  const seriesPrefix = isStaffActor ? "TS" : "T";
  const key = isStaffActor ? `TICKET-STAFF-${ticketYear(now)}` : `TICKET-${ticketYear(now)}`;
  ...
  `;
  return formatTicketNumber(ticketYear(now), Number(rows[0].lastNumber), seriesPrefix);
}
 ...
 *   returns null — every caller must tolerate that. Historically it routed
 *   the allocation onto the separate "TS-<year>-<seq>" series instead; that
 *   series is still wired up below and never saw a single production ticket.
...
export async function allocateOrderTicketNumber(tx, orderId, now = new Date(), isStaffActor = false) {
  ...
  // issues no ticket for it. Neutralised here rather than at the thirteen
  // call sites — one line, reversible in one line, and the "TS-" series
  // below (its prefix, its counter key, every call site's argument) stays
  // intact rather than being ripped out. `isStaffActor` is always
  ...
  // with `false` and are ticketed exactly as before.
  ...
  const ticketNumber = await claimNextTicketNumber(tx, now, isStaffActor);
 ...
 *   in particular, a true value allocates nothing and returns null.
 * @returns {Promise<string|null>}
 */
export async function allocatePaymentTicketNumber(
  ...
  // with `false` and are ticketed exactly as before.
  if (isStaffActor) return null;
  const current = await tx.payment.findUnique({ where: { id: paymentId }, select: { ticketNumber: true } });
  if (current?.ticketNumber) return current.ticketNumber;
