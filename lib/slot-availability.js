...
function generateAllTimeSlots(workStartMinutes, workEndMinutes, slotInterval, serviceDuration, freeIntervals) {
  ...
  // A slot is available if the full service duration fits within a free interval
  // (i.e., no overlap with existing appointments, time-offs, or out-of-hours).
...
/**
 * Check whether a time slot (start + service duration) fits entirely within
 * a free interval. This is the canonical availability check used both by the
 * UI (to mark slots as clickable) and by server-side validation (to confirm
 * a submitted slot is still valid).
 *
 * @param {string} startTime - "HH:mm"
 * @param {number} durationMinutes - service duration in minutes
 * @param {Array<{ start: number, end: number }>} freeIntervals - free gaps in minutes from midnight
 * @returns {boolean}
 */
export function slotFitsInFreeIntervals(startTime, durationMinutes, freeIntervals) {
  const [hour, minute] = startTime.split(":").map(Number);
  const startMinutes = hour * 60 + minute;
  const endMinutes = startMinutes + durationMinutes;
  return freeIntervals.some((free) => startMinutes >= free.start && endMinutes <= free.end);
}
...
export function buildAvailabilityForDate({ staffService, selectedDate, salon, existingAppointments }) {
  ...
  // Appointments arrive sorted, but partial-day time-offs are pushed
  // afterwards and are often earlier in the day (e.g. morning time-off +
  // afternoon appointment). mergeIntervals requires sorted input, so sort
  // first — otherwise the earlier block is absorbed without moving the
  // start and incorrectly shows as free.
  ...
  // Step 4: Generate reservation windows (canonical bookable slots — chained, deterministic)
  const reservationWindows = generateReservationWindows(freeIntervals, duration);

  // Step 5: Generate ALL time slots at fixed 30-minute intervals for the visual timeline.
  // A slot is available if the full service duration fits within a free interval
  // (no overlap with appointments, time-offs, or out-of-hours). The user can pick
  // any start time; the backend validates at submission that the full duration fits.
