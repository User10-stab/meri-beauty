 ...
 *     }
 *   }
 * }>, discountAmount?: number }} params `discountAmount` only applies to the
 * single-appointment case — multi-appointment drafts have no payment/pricing
 * step at all today, so there's nowhere for a promo code to attach.
 *
 * @returns {{
...
export function computePaymentDecision({ drafts, discountAmount = 0 }) {
  ...
  const price            = Math.max(0, Number(draft?.price ?? 0) - discountAmount);
