 ...
 * - Manual reservations (created by salon/staff): always CONFIRMED status,
 *   but payment rules still respect staff configuration
 *
 * @param {{
 ...
 * `isManualReservation` indicates the reservation was created manually by
 * salon/staff (not online client booking) and should always be CONFIRMED.
 *
 * @returns {{
...
export function getReservationPaymentDecision({
  ...
  // Manual reservations (created by salon/staff) are always CONFIRMED
  // regardless of confirmationMode, but payment rules still apply
  const effectiveConfirmationMode = isManualReservation ? "AUTOMATIC" : normalizedConfirmationMode;
  ...
  if (effectiveConfirmationMode === "MANUAL") {
    // MANUAL mode: no payment during initial reservation — the request stays
    // PENDING with no Payment row until staff accept it. Payment/confirmation
    // happens after acceptance, driven by the confirmation page. CASH_ONLY
    // staff keep the on-site option so the client knows they'll pay in person.
    return {
      requiresPaymentStep: false,
      isManualMode: true,
      onlineFullPaymentAvailable: false,
      salonPaymentAvailable: normalizedAllowedMethods === "CASH_ONLY",
      depositRequired: false,
      depositPercentage: 0,
      depositAmount: 0,
      totalAmount: total,
  ...
  // For manual reservations, always return CONFIRMED status but respect payment rules
  if (isManualReservation) {
    let paymentIntent = "NO_ONLINE_PAYMENT";
    let requiresOnlinePaymentNow = false;
    let paymentType = "ON_SITE";

    // Manual reservations don't enforce online payment since staff are creating them directly
    // They can still create payment records if needed, but the decision is based on context
    // For now, manual reservations default to ON_SITE payment (pay at salon)
    // This can be enhanced later if staff want to collect deposits for manual bookings

    return {
      requiresPaymentStep: false,
      isManualMode: false,
      onlineFullPaymentAvailable: normalizedAllowedMethods === "ONLINE_ONLY" || normalizedAllowedMethods === "BOTH",
      salonPaymentAvailable: normalizedAllowedMethods === "CASH_ONLY" || normalizedAllowedMethods === "BOTH",
      depositRequired: false, // Manual reservations don't enforce deposits
      depositPercentage: 0,
      depositAmount: 0,
      totalAmount: total,
      requiresOnlinePaymentNow: false,
      paymentIntent,
      paymentType,
      shouldCreatePaymentRecord: false, // Manual reservations don't create payment records by default
      appointmentStatusBeforePayment: "CONFIRMED",
      appointmentStatusAfterPayment: "CONFIRMED",
      requiresPaymentBeforeConfirmation: false,
      requiresOptionalDepositPrompt: false,
    };
 ...
 *
 * @param {{ drafts: Array<{ price?: number, staffService?: { staff?: { reservationConfirmationMode?: string, depositEnabled?: boolean, depositPercentage?: number } } }>, discountAmount?: number, isManualReservation?: boolean }} params
 * @returns {ReturnType<typeof getReservationPaymentDecision>}
 */
export function computePaymentDecision({ drafts, discountAmount = 0, isManualReservation = false }) {
