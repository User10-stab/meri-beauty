...
export async function createBillitOrder(payload) {
  ...
  // Billit's production API needs both credentials: ApiKey authenticates
  // the user and PartyID selects the company in whose name the order is
  // created. Never silently send without that company context.
  const partyId = process.env.BILLIT_PARTY_ID?.trim();
  if (!partyId) {
    return { success: false, message: "Billit n'est pas configuré (PartyID manquant — renseignez BILLIT_PARTY_ID)." };
  }
