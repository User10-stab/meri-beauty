...
export async function confirmFormationReservationPayment(session) {
  ...
  ({ invoice } = await prisma.$transaction(async (tx) => {
    ...
    if (reservation.status === "PENDING_DEPOSIT") {
      const otherReserved = await tx.formationReservation.aggregate({
        where: {
    ...
    if (err.message === "HOLD_EXPIRED_OVERBOOKED" || err.message === "INVALID_SESSION_CAPACITY") {
      ...
      sendEmail({
        ...
        text:
          ...
          `Votre réservation pour la formation "${reservation.session.formation.title}" a expiré avant votre paiement et les places ont depuis été réattribuées. ` +
        ...
        html:
          ...
          `<p>Votre réservation pour la formation "${reservation.session.formation.title}" a expiré avant votre paiement et les places ont depuis été réattribuées. ` +
