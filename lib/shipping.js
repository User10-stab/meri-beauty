/**
 * Belgian Poste shipping cost calculation (Marie's requirements)
 * - Free shipping for orders over €150
 * - Otherwise: calculate by weight tiers
 */

const FREE_SHIPPING_THRESHOLD = 150; // €
const BELGIUM_VAT_RATE = 21;

// Belgian Poste weight tiers (2024 rates, domestic shipping)
// Prices in EUR, TVA included
const POSTE_TIERS = [
  ...
  { maxGrams: 30000, price: 35.00 }, // 10kg - 30kg (max domestic parcel)
];
...
export function calculateShippingCost(totalWeightGrams, orderSubtotal) {
  ...
  const tier = POSTE_TIERS.find(t => totalWeightGrams <= t.maxGrams);
...
export function getShippingDetails(totalWeightGrams, orderSubtotal) {
  ...
  const appliedTier = !isFree ? POSTE_TIERS.find(t => totalWeightGrams <= t.maxGrams) : null;
  const nextTier = !isFree ? POSTE_TIERS.find(t => totalWeightGrams > t.maxGrams) : null;
