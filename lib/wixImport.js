...
export function parseWixProductsCsv(csvText) {
  ...
  for (const { product, images } of byHandle.values()) {
    ...
    products.push({
      ...
      // Real manufacturer data when staff filled it in Wix — kept as-is, not
      // required. `weight` is deliberately NOT carried over: Wix's export
      // never states its unit, so a wrong guess here would silently write
      // bad shipping weights rather than leaving the honest "not set yet"
      // default (ProductVariant.weightGrams defaults to 0).
