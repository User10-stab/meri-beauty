...
function StaffChip({staff}){ const name=staff?.user?.fullName??"—"; return (<div className="flex items-center gap-2"><div className="relative h-7 w-7 flex-shrink-0 overflow-hidden rounded-full ring-1 ring-[#ede5d8]">{staff?.photo?(<Image src={staff.photo} alt={name} fill unoptimized className="object-cover" />):(<div className="flex h-full w-full items-center justify-center bg-[#2F3A2E] text-xs font-bold text-white">{name.charAt(0)}</div>)}</div><span className="text-sm font-semibold text-[#2F3A2E]">{name}</span></div>);}
