...
export const REFUND_DENIAL = Object.freeze({
  NOT_ADMIN: "NOT_ADMIN",
  NOT_PAYMENT_OWNER: "NOT_PAYMENT_OWNER",
...
const DENIAL_MESSAGES = Object.freeze({
  ...
  [REFUND_DENIAL.NOT_PAYMENT_OWNER]:
    "Cette vente appartient à la praticienne indépendante qui l'a encaissée : elle seule peut l'annuler et la rembourser.",
...
/**
 * Who may refund this payment — decided by whose sale it is
 * (Payment.payeeStaffId, see lib/payments/resolve-payee.js), never by role
 * alone.
 *
 *   - The salon's sale (payeeStaffId null): OWNER/ADMIN, as confirmed in
 *     docs/QUESTIONS_FOR_MARIE.md §5.
 *   - An independent practitioner's sale: that practitioner and nobody else.
 *     The admin is refused too — the money sits on her own Stripe account,
 *     under her own VAT number, and the salon has no say over it.
 *
 * @param {{actorRole?: string, actorStaffId?: string|null, payeeStaffId?: string|null}} input
 */
export function authorizeRefundActor({ actorRole, actorStaffId = null, payeeStaffId = null }) {
  if (payeeStaffId) {
    return actorStaffId && actorStaffId === payeeStaffId ? ALLOW : deny(REFUND_DENIAL.NOT_PAYMENT_OWNER);
  }
  return actorRole === "OWNER" || actorRole === "ADMIN" ? ALLOW : deny(REFUND_DENIAL.NOT_ADMIN);
}
 ...
 * @param {string|null} [input.actorStaffId] - the session user's own Staff.id, if any
 * @param {"APPOINTMENT"|"WORKSHOP"|"FORMATION"|"ORDER"|"POS"} input.source
 * @param {"CUSTOMER_REQUEST_APPROVED"|"SALON_CANCELLATION"|"NO_SHOW_EXCEPTION"|"SHOP_RETURN"|"POST_COMPLETION_CORRECTION"} input.trigger
 ...
 * @param {{pendingRefundAmount: unknown, payeeStaffId?: string|null, refundOperations?: Array}|null} [input.payment]
...
export function authorizeRefund({
  ...
  // ── Who ────────────────────────────────────────────────────────────────
  // The salon's sale: OWNER/ADMIN only. An independent's sale: her alone.
  // See authorizeRefundActor.
  const who = authorizeRefundActor({ actorRole, actorStaffId, payeeStaffId: payment?.payeeStaffId ?? null });
  ...
  if (source === "APPOINTMENT") {
    ...
    // The one deliberate exception to "COMPLETED is never refundable": a
    // post-completion correction (wrong price/data), routed exclusively
    // through lib/refunds/correct-completed-transaction.js, which always
    // fully voids the original invoice and always cancels the appointment —
    // never a silent financial-only patch. See prisma/schema.prisma's
    // POST_COMPLETION_CORRECTION comment.
    if (appointment.status === "COMPLETED" && trigger === "POST_COMPLETION_CORRECTION") {
      return ALLOW;
    }
  ...
  if (source === "WORKSHOP" || source === "FORMATION") {
    ...
    // Same deliberate exception as APPOINTMENT above.
    if (reservation.status === "COMPLETED" && trigger === "POST_COMPLETION_CORRECTION") {
      return ALLOW;
    }
  ...
  if (source === "ORDER") {
    ...
    if (trigger === "SHOP_RETURN") {
      ...
      // accepted the return"; only COMPLETED means "we have the items", and
      // only COMPLETED is accepted here — the rule and the check disagreed
      // until now, which would have let a refund go out against goods that
      // were merely cleared to come back.
      //
      // This costs the real flow nothing: completeReturn claims the row to
      // COMPLETED at the top of the very transaction that queues the refund
      // (actions/boutique/returns.js), so anything reading it inside that
      // transaction already sees COMPLETED.
    ...
    // Same deliberate exception as APPOINTMENT above — a price/data
    // correction needs no returned goods, unlike SHOP_RETURN.
    if (SHIPPED_ORDER_STATUSES.has(order.status) && trigger === "POST_COMPLETION_CORRECTION") {
      return ALLOW;
    }
