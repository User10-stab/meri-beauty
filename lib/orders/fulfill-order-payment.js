...
export async function fulfillOrderPayment(session) {
  ...
  const order = await prisma.order.findUnique({
    ...
    include: {
      ...
      // Only meaningful for a POS-QR order (source: "POS") — used below to
      // decide the ticket series and whether this staff member may cause an
      // Invoice to be created at all. A genuine online order has no staff
      // actor, so this stays null and every isStaffActor check short-circuits
      // false via the isPointOfSale guard.
      createdByStaff: { select: { id: true, role: true, email: true } },
    },
  });

  if (!order || order.status === "CANCELLED" || order.status === "EXPIRED") {
    console.warn("[fulfillOrderPayment] order gone/cancelled after payment, flagging for manual refund:", session.id);
    if (session.payment_intent) {
      await flagPaymentForManualRefund(session, "commande introuvable, annulée ou expirée").catch((err) => {
        captureCriticalError(err, { area: "refund-reconciliation", sessionId: session.id, orderId, reason: "order unavailable" });
        throw err;
      });
    }
    return { received: true, refunded: false, flaggedForReview: true, reason: "order unavailable" };
  }
  ...
  if (order.status !== "PENDING_PAYMENT") {
    ...
    console.warn(`[fulfillOrderPayment] order ${orderId} already ${order.status}, flagging stray session for manual refund:`, session.id);
    if (session.payment_intent) {
      await flagPaymentForManualRefund(session, "commande déjà payée par une autre session").catch((err) => {
        captureCriticalError(err, { area: "refund-reconciliation", sessionId: session.id, orderId, reason: "order already paid" });
        throw err;
      });
    }
    return { received: true, refunded: false, flaggedForReview: true, reason: "order already paid" };
  }

  const paidAmount = (session.amount_total ?? 0) / 100;
  const isPointOfSale = order.source === "POS";
  // Someone without the till rang this up (QR checkout) — see
  // canUseSalonTill. A boutique sale is the salon's, so a CAISSE staff
  // member's POS order gets its salon ticket like Marie's. False for every
  // genuine online order, which has no staff actor at all.
  const isStaffActor = isPointOfSale && !(await canUseSalonTill(order.createdByStaff));
  ...
  if (paidAmountCents < expectedAmountCents) {
    console.error("[fulfillOrderPayment] Order payment underpayment detected:", { sessionId: session.id, orderId, paidAmount, expectedAmount });
    if (session.payment_intent) {
      await flagPaymentForManualRefund(session, `paiement insuffisant (${paidAmount}€ reçus, ${expectedAmount}€ attendus)`).catch((err) => {
  ...
  if (paidAmountCents > expectedAmountCents) {
    console.error("[fulfillOrderPayment] Order payment overpayment detected:", { sessionId: session.id, orderId, paidAmount, expectedAmount });
    if (session.payment_intent) {
      await flagPaymentForManualRefund(session, `paiement supérieur au total (${paidAmount}€ reçus, ${expectedAmount}€ attendus)`).catch((err) => {
        captureCriticalError(err, { area: "refund-reconciliation", sessionId: session.id, orderId, reason: "overpayment" });
        throw err;
      });
    }
    return { received: true, refunded: false, flaggedForReview: true, reason: "overpayment" };
  }

  let invoice;
  let ticketNumber;
  try {
  ({ invoice, ticketNumber } = await prisma.$transaction(async (tx) => {
    ...
    // A non-privileged staff member (isStaffActor) can never cause an
    // Invoice to be created — see canUseSalonTill — regardless of VAT
    // eligibility. The sale still completes, it simply never gets an
    // invoice.
    const isVatEligible = hasInvoiceableVatIdentity(invoiceCustomerUser);
    const shouldCreateInvoice = isVatEligible && !isStaffActor;
    ...
    const ticketNumber = await allocateOrderTicketNumber(tx, order.id, new Date(), isStaffActor);
  ...
  // genuinely stuck transaction.
    ...
    if (err.message === "ORDER_NO_LONGER_PENDING") {
      ...
      console.warn("[fulfillOrderPayment] order cancelled during payment, flagging for manual refund:", session.id);
      if (session.payment_intent) {
        await flagPaymentForManualRefund(session, "commande annulée pendant le traitement du paiement").catch((refundErr) => {
      ...
      return { received: true, refunded: false, flaggedForReview: true, reason: "order cancelled during payment" };
    }
    throw err;
  }
  ...
  if (isPointOfSale) {
    ...
    // A null ticketNumber means isStaffActor was true: an independent rang
    // this up, it is her sale under her own VAT number, and the salon issues
    // no ticket for it (lib/tickets/allocate-ticket-number.js). This whole
    // e-mail exists to carry that ticket, so it goes with it.
    ...
    const pendingInvoiceNote = !invoice
      ...
      ? ` La facture officielle n°${invoice.number} sera transmise séparément via Peppol (Peppyrus) depuis Opérations.`
  ...
  const pendingInvoiceNote = !invoice
    ? ""
    : order.customerVatNumber?.toUpperCase().startsWith("BE")
    ? ` La facture officielle n°${invoice.number} vous sera transmise séparément via Peppol (Peppyrus) depuis Opérations.`
  ...
  const emailAttachments = [
    // This branch is a customer's own online checkout — there is no staff
    // actor at all, so `isStaffActor` was false and `ticketNumber` is never
    // null here. (Its exact spelling is pinned by
    // tests/critical/qr-email-delivery-contracts.test.js.)
    ...(ticketPdf ? [{ filename: `${ticketNumber}.pdf`, content: ticketPdf }] : []),
    ...(pickupQr ? [pickupQr] : []),
  ];
