...
// Invoice (customerName/customerEmail are non-null there). The address is
// still required by the outer sale refine because every POS ticket must be
// mailed now.
...
export const pointOfSaleSaleSchema = z
  ...
  .refine((data) => data.customer !== null || Boolean(data.walkInEmail), {
    message: "L'e-mail du client est obligatoire pour envoyer le ticket.",
    path: ["walkInEmail"],
  });
