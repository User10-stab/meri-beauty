...
export async function sendB2CRefundConfirmation({ prisma, operationId }) {
  ...
  // An independent's sale never carries a salon credit note, whoever the
  // customer is — her B2B paperwork is her own, so the plain confirmation is
  // the only thing the platform can send for it.
  const independentSale = Boolean(operation.payment?.payeeStaffId);
  if (operation.creditNote || (!independentSale && isBusinessRefundCustomer(resolveCustomer(operation)))) {
