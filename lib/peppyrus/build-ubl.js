...
// PEPPOL-EN16931-R002: at most one document-level note. BT-22 has no hard
// length limit in the spec, but receiving software commonly truncates.
const MAX_INVOICE_NOTE_LENGTH = 1000;
...
export function buildInvoiceUbl({ invoice, salon, buyerParticipantId }) {
  ...
  const doc = {
    ...
    invoiceTypeCode: 380,
    // BT-22 — the admin's free-text comment on a manual invoice.
    ...(invoice.notes?.trim() ? { note: [{ content: invoice.notes.trim().slice(0, MAX_INVOICE_NOTE_LENGTH) }] } : {}),
