...
import { isStripeAccountUnavailable } from "@/lib/payments/stripe-account-access";
...
// The 72h default is right for the routine run and wrong for exactly one
// situation: the scheduler itself was down for longer than that. Every refund
// older than the window is then invisible to this job forever, with no way to
// reach back for it short of a code change and a deploy.
//
// STRIPE_REFUND_LOOKBACK_HOURS widens the window without touching the code.
// Set it after an outage, let one run catch up, then remove it — a permanently
// wide window means every run re-scans months of charges on every connected
// account for nothing.
const MAX_LOOKBACK_HOURS = 24 * 90;

function configuredLookbackHours(raw = process.env.STRIPE_REFUND_LOOKBACK_HOURS) {
  if (raw === undefined || raw === null || `${raw}`.trim() === "") return LOOKBACK_HOURS;
  const parsed = Number(raw);
  // A malformed value must not silently shrink the window to nothing — that
  // would turn a typo into a job that quietly reconciles no refunds at all.
  if (!Number.isFinite(parsed) || parsed <= 0) {
    console.warn(
      `[reconcile-missed-refunds] Ignoring invalid STRIPE_REFUND_LOOKBACK_HOURS=${raw}, using ${LOOKBACK_HOURS}h`,
    );
    return LOOKBACK_HOURS;
  }
  return Math.min(Math.max(parsed, LOOKBACK_HOURS), MAX_LOOKBACK_HOURS);
}

export const __test__ = { configuredLookbackHours, LOOKBACK_HOURS, MAX_LOOKBACK_HOURS };
  ...
  for (;;) {
    ...
    for (const refund of page.data) {
      ...
      try {
        ...
        // A refund operation already owns its credit note and settlement
        // plan. The generic external-refund path below would otherwise add a
        // second REFUND transaction while leaving its RefundLeg pending.
        const ownLeg = await findLegForStripeRefund(prismaClient, {
          refundId: refund.id,
          paymentIntentId,
        });
        if (ownLeg) {
          const result = await settleRefundLeg({
            ...
            stripe: {
              refundId: refund.id,
              paymentIntentId,
              // Stripe reports a cumulative charge total; the ledger tells
              // us the precise unrecorded increment for this missed event.
              amount: round2(stripeRefundedTotal - alreadyRecorded),
            },
          });
          if (result.settled || result.reason === "ALREADY_SETTLED") {
            reconciled += 1;
          }
          // A matching leg must never fall through to external reconciliation.
          continue;
        }
    ...
    // Exactly the revoked-account case this comment describes: expected and
    // permanent, so it is a skip rather than a fatal alert on every sweep
    // (see lib/payments/stripe-account-access.js).
    if (isStripeAccountUnavailable(error)) {
      captureWarning("Compte Stripe connecté illisible — ignoré pour cette passe.", {
...
export async function reconcileMissedRefunds({
  ...
  const cutoffUnix = Math.floor(Date.now() / 1000) - configuredLookbackHours() * 3600;
