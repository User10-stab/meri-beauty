...
/**
 * Deep-link to one appointment on the dashboard list. The appointments page
 * honors `?appointmentId=` by focusing that row (see
 * AppointmentsPageClient), so a tap lands on the exact reservation instead
 * of an unfiltered list.
 */
function appointmentDeepLink(appointmentId) {
  return `/dashboard/appointments?appointmentId=${encodeURIComponent(appointmentId)}`;
}

/**
 * Same idea for atelier/formation bookings: the reservations pages honor
 * `?reservationId=` by focusing that exact row.
 */
function activityReservationDeepLink(basePath, reservationId) {
  return `${basePath}?reservationId=${encodeURIComponent(reservationId)}`;
}
...
export function buildAppointmentCreatedNotification(data) {
  ...
  return {
    ...
    actionUrl: appointmentDeepLink(data.appointmentId),
    payload: {
      appointmentId: data.appointmentId,
...
const OVERDUE_REASON_TEXT = {
  NOT_PREPARED: {
    pickup: "n'a toujours pas été préparée pour le retrait",
    shipping: "n'a toujours pas été expédiée",
  },
  NOT_COLLECTED: {
    pickup: "est prête depuis plusieurs jours mais n'a pas été retirée",
  },
  NOT_CONFIRMED_DELIVERED: {
    shipping: "a été expédiée mais sa réception n'a jamais été confirmée",
  },
};

/**
 * Salon-side notification: a boutique order has stalled mid-fulfilment —
 * paid but never prepared/shipped, marked ready but never collected, or
 * shipped but never confirmed received. See lib/orders/overdue-rules.js for
 * the rule that decides `reason`, and lib/orders/notify-stale-fulfilment.js
 * for the cron job that calls this.
 */
export function buildOrderFulfilmentOverdueNotification(data) {
  const isShipping = data.fulfilmentMode === "SHIPPING_PREPAID";
  const situation = OVERDUE_REASON_TEXT[data.reason]?.[isShipping ? "shipping" : "pickup"] ?? "est en attente depuis plusieurs jours";
  return {
    userId: data.userId,
    type: "ORDER_FULFILMENT_OVERDUE",
    title: `Commande n°${data.orderNumber} en retard`,
    message: `La commande n°${data.orderNumber} de ${clampText(data.customerName, 80) || "une cliente"} ${situation}.`,
    actionUrl: `/dashboard/boutique/orders/${data.orderId}`,
    payload: {
      orderId: data.orderId,
      orderNumber: data.orderNumber,
...
export function buildWorkshopReservationPaidNotification(data) {
  ...
  return {
    ...
    actionUrl: activityReservationDeepLink("/dashboard/workshops/reservations", data.reservationId),
    payload: {
...
export function buildFormationReservationPaidNotification(data) {
  ...
  return {
    ...
    message: `${clampText(data.customerName, 80) || "Une cliente"} a réservé ${seats} pour « ${clampText(data.activityTitle, 120)} » — ${kind} de €${Number(data.paidAmount).toFixed(2)}.`,
    actionUrl: activityReservationDeepLink("/dashboard/formations/reservations", data.reservationId),
    payload: {
      reservationId: data.reservationId,
      activityTitle: data.activityTitle,
      seatsCount: data.seatsCount,
