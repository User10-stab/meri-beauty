  ...
  for (const reservation of stale) {
    if (reservation.payment?.transactionReference) {
      try {
        ...
        if (stripeSession.payment_status === "paid") {
          await confirmFormationReservationPayment(stripeSession);
          continue;
        }
        ...
        console.error("[expireStaleFormationHolds] Stripe session check failed, deferring expiry:", reservation.id, error);
        continue;
      }
    }
    ...
    sendEmail({
      ...
      text:
        ...
        `Votre réservation pour la formation "${reservation.session.formation.title}" a expiré faute de paiement dans le délai prévu. ` +
      ...
      html:
        ...
        `<p>Votre réservation pour la formation "${reservation.session.formation.title}" a expiré faute de paiement dans le délai prévu. ` +
