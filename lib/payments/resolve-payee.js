...
/**
 * An atelier is ALWAYS the salon's money — whoever animates it.
 *
 * Decided 18/09/2026, and it is a business rule, not an implementation
 * detail: ateliers are the salon's own events. A formation is the only
 * activity whose seat can belong to an independent, because a formation is
 * hers to give. The animator on an atelier is displayed to clients (Marie on
 * MABON, YULE, HALLOWEEN COCOON NIGHT; Laëtitia on Atelier Yoga Facial) and
 * decides nothing about the money.
 *
 * Deliberately still a function, and still the one every atelier money path
 * calls — the booking checkout, the webhook that fulfils it, the staff
 * relance and the seat transfer. Collapsing it to a constant at each of those
 * call sites would scatter the rule across four files and make reversing it a
 * hunt; leaving it here means the answer is stated once, with its reason, and
 * every path that ever asks gets it.
 *
 * `sessionId` is unused for that reason. It is kept so the signature matches
 * resolvePayeeForFormationSession and resolvePayeeForActivitySession can
 * dispatch to either.
 *
 * @returns {Promise<Payee>}
 */
export async function resolvePayeeForWorkshopSession(_tx, _params) {
  return SALON_PAYEE;
}
...
/**
 * The same answer as the two resolvers above, chosen by activity kind — for
 * callers that already work in terms of "WORKSHOP" | "FORMATION" rather than
 * one domain, such as the staff relance.
 *
 * @param {object} tx
 * @param {{ kind: "WORKSHOP"|"FORMATION", sessionId: string }} params
 * @returns {Promise<Payee>}
 */
export async function resolvePayeeForActivitySession(tx, { kind, sessionId }) {
  if (kind === "FORMATION") return resolvePayeeForFormationSession(tx, { sessionId });
  if (kind === "WORKSHOP") return resolvePayeeForWorkshopSession(tx, { sessionId });
  throw new Error(`Unknown activity kind for payee resolution: ${kind}`);
}
