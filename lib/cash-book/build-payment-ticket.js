 ...
 * client under the canSendTicketEmail() check. Extracted so both stay in
...
export async function buildPaymentTicket(paymentId, { transactionId = null } = {}) {
  ...
  // No ticket before the balance is in. This used to be inferred from a
  // missing Payment.ticketNumber, which conflated "still owes a balance"
  // with "settled before ticket numbering existed" — and the latter is every
  // reservation payment taken before 15/09/2026. Payment.status is the
  // direct answer, and it is what every allocation path sets alongside the
  // number (see settle-reservation.js, manage-appointment.js#markAppointmentNoShow).
  if (payment.status !== "PAID") {
    return { error: { status: 409, message: "Ce paiement n'est pas encore clôturé — pas de ticket avant le solde." } };
  }
  ...
  if (transactionId) {
    ...
    const receipt = collectionTicketFields(txn, payment.ticketNumber, payment.invoice, ticketFields.vatRate);
    ...
    const receipt = consolidatedTicketFields(paymentId, payment.ticketNumber, payment.transactions, payment.invoice, ticketFields.vatRate);
    ticket = { ...ticketFields, ...receipt, lines: [{ description, quantity: 1, unitPrice: receipt.totalInclVat }] };
  }
