...
export async function settleReservation({
  ...
  const offTill = !isTillCashOperator(actor) || Boolean(payment.payeeStaffId);
  ...
  try {
    result = await prisma.$transaction(async (tx) => {
      ...
      // be created — see isTillCashOperator — resolveSettlementInvoice is a
      // hard no-op for them.
      const { invoice, creditNote } = await resolveSettlementInvoice(tx, {
        existingInvoice: payment.invoice,
        priceChanged: priceAdjustment.changed,
  ...
  try {
    const claimed = await prisma.$transaction(async (tx) => {
      ...
      if (payment && Number(payment.paidAmount) > 0.01 && !payment.invoice) {
        ...
        // invoice.
        const offTillActor = !isTillCashOperator(actor) || Boolean(payment.payeeStaffId);
