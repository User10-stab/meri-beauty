...
export async function createNotificationsBulk(inputs, options) {
  ...
  // `createManyAndReturn` is only available on some Prisma versions.
  // Use `createMany` for compatibility, then fetch the created rows.
  const created = await client.notification.createMany({
    data: rows,
    skipDuplicates: false,
  });

  // Best-effort fetch of the created rows (ordered newest first) so callers
  // still get items back.
