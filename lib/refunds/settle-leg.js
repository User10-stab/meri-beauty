 ...
 * @param {object} [input.manual] - { confirmedByUserId, confirmedByRole, terminalReference, cashHandedOver }
...
export async function settleRefundLeg({ prisma, legId, stripe = null, manual = null }) {
  return prisma.$transaction(async (tx) => {
    ...
    // Same rule for a bank transfer back (a manual invoice settled by
    // virement): the transfer's own reference travels in terminalReference.
    if (manual && leg.method === "TRANSFER" && !manual.terminalReference?.trim()) {
      return {
        settled: false,
        reason: "TRANSFER_REFERENCE_REQUIRED",
        operationId: leg.refundOperationId,
        operationStatus: null,
      };
    }
    ...
    await tx.auditLog.create({
      data: {
        ...
        // The real role: an independent practitioner confirms her own
        // hand-overs, so this is no longer always an admin.
        actorRole: manual?.confirmedByUserId ? manual.confirmedByRole ?? "ADMIN" : null,
