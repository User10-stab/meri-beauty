 ...
 *   payload.signature
 *
 *   payload   = base64url(JSON{ purpose, staffId, userId, nonce, exp })
 ...
 * the expiry before reading `staffId`/`userId` from it. This protects
 * against forged/tampered callbacks — but the signature alone does not stop
 * a *stolen* state value from being replayed by someone else: `state` rides
 * in a URL that gets redirected through the browser and can end up in
 * history, a referrer header, or a shared screen within its 10-minute
 * window. Binding `userId` closes that gap — the callback re-checks it
 * against the live session (see app/api/stripe/oauth/callback/route.js), so
 * a stolen state only works for whoever is still signed in as that same
 * user, not anyone who happens to obtain the value.
 *
...
/**
 * Create a signed OAuth state token bound to a staff member and the user
 ...
 * for *that* — but `userId` is still checked against the live session at
 * callback time (see verifyStripeOAuthState's callers), so a stolen state
 * value can't be replayed by someone signed in as a different user.
 *
 * @param {string} staffId - The staff member initiating the connection.
 * @param {string} userId - The authenticated user initiating the flow.
 ...
 */
export function createStripeOAuthState(staffId, userId, purpose = OAUTH_STATE_PURPOSE) {
  const nonce = randomBytes(16).toString("hex");
  const exp = Date.now() + STATE_TTL_MS;

  const payload = Buffer.from(
    JSON.stringify({ purpose, staffId, userId, nonce, exp })
  ).toString("base64url");
 ...
 * every other case. The caller is still responsible for checking the
 * returned `userId` against the live session — this function only proves
 * the token wasn't forged/tampered/reused-across-flows, not who currently
 * holds it.
 *
 * @param {string} state - The raw `state` value received in the callback.
 * @param {string} [expectedPurpose] - Purpose the token must carry
 *   (defaults to OAUTH_STATE_PURPOSE).
 * @returns {{ staffId: string, userId: string } | null}
 */
export function verifyStripeOAuthState(state, expectedPurpose = OAUTH_STATE_PURPOSE) {
  ...
  return { staffId: data.staffId, userId: data.userId };
}
 ...
 *
 * @param {{ staffId: string, userId: string }} params
 * @returns {string} - The full authorization URL to redirect the user to.
 */
export function buildStripeOAuthUrl({ staffId, userId }) {
  ...
  url.searchParams.set("state", createStripeOAuthState(staffId, userId));
