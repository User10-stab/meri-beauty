...
export async function expireStaleOrders() {
  ...
  const stale = await prisma.order.findMany({
    where: {
      expiresAt: { lt: now },
      // A manual invoice's order is never an abandoned cart: it backs an
      // issued, numbered invoice (actions/invoices/manual-invoice.js). It has
      // no expiresAt and is always COMPLETED, so this is defense in depth only.
      source: { not: "MANUAL" },
      OR: [
          ...
          data: {
            ...
            cancelReason:
              order.status === "PENDING_PAYMENT"
                ...
                : "Retrait non effectué dans le délai.",
          },
        });
