...
const PAYMENT_CONTEXT = {
  ...
  // Whose sale this is (null = the salon's) and the Stripe account its card
  // money landed on — see lib/payments/resolve-payee.js. The owner decides
  // who may refund, whether any salon document is touched, and whether a
  // cash refund belongs in the salon's cash book.
  payeeStaffId: true,
  ...
  appointment: {
    select: {
      ...
      date: true,
      notes: true,
      // vatValidatedAt is load-bearing on every customer/user select in this
      // context object, not cosmetic: isBusinessRefundCustomer
      // (hasReusableVatValidation) requires it to decide B2B vs B2C at all —
      // without it every customer reads as B2C regardless of VAT status,
      // silently disabling the B2B_INVOICE_REQUIRED check below.
      user: { select: { id: true, fullName: true, email: true, isCompany: true, vatNumber: true, vatValidatedAt: true } },
    },
  },
  workshopReservation: {
    select: {
      ...
      customer: { select: { id: true, fullName: true, email: true, isCompany: true, vatNumber: true, vatValidatedAt: true } },
  ...
  order: {
    select: {
      ...
      // false only when a VAT-eligible POS customer explicitly declined the
      // invoice (actions/boutique/point-of-sale.js) — needed below to tell
      // that deliberate choice apart from a B2B sale that is simply missing
      // its invoice by mistake.
 ...
 */
async function cancelUnderlyingItem(tx, { source, context, reason, releaseCapacity, actorRole = "ADMIN", trigger = null }) {
  const now = new Date();
  const note = `Annulation et remboursement : ${reason}`;

  // COMPLETED is only ever reachable here via POST_COMPLETION_CORRECTION —
  // every other trigger is already denied at a COMPLETED appointment/
  // reservation in authorize.js, before this function is ever called. So
  // adding it to the claimable set below never lets a live booking's normal
  // cancellation reach a COMPLETED row by accident; it only unblocks the one
  // trigger that is explicitly allowed to.
  ...
  const reservationStatuses =
    trigger === "POST_COMPLETION_CORRECTION"
      ? ["PENDING_DEPOSIT", "CONFIRMED", "COMPLETED"]
  ...
  if (source === "APPOINTMENT") {
    ...
    const claim = await tx.appointment.updateMany({
      where: { id: appointment.id, status: { in: appointmentStatuses } },
      data: {
        status: "CANCELLED",
        cancelledAt: now,
        cancellationReason: reason,
        // The practitioner refunding her own sale cancels as STAFF.
        cancellationSource: actorRole === "OWNER" || actorRole === "ADMIN" ? "ADMIN" : "STAFF",
  ...
  if (source === "WORKSHOP" || source === "FORMATION") {
    ...
    const claim = await model.updateMany({
      where: { id: reservation.id, status: { in: reservationStatuses } },
      data: {
        status: "CANCELLED",
        cancelledAt: now,
 ...
 * @param {{id: string, role: string, staffId?: string|null}} input.actor - staffId is the actor's own Staff row, which is what lets an independent refund her own sale
...
export async function openRefundOperation({
  ...
  // Kept as a hard boundary for direct/internal callers as well as the
  // Operations screen. The database enum remains for historic audit rows,
  // but no new financial-correction operation may be opened.
  if (trigger === "FINANCIAL_CORRECTION") {
    throw new RefundNotAllowedError(
      "FINANCIAL_CORRECTION_RETIRED",
      "La correction financière n'est plus disponible. Utilisez l'annulation appropriée.",
    );
  }
  ...
  return prisma.$transaction(async (tx) => {
    // Prisma's 5s default is too tight here, same as every other transaction
    // that issues an accounting document: this one also reloads the context,
    // issues a credit note (its own numbering query), cancels the item,
    // restores stock line by line for an ORDER, and allocates a cash-book
    // piece number — on a pooled Neon connection that easily adds up past 5s,
    // which surfaced as an opaque "Transaction not found" from a stranded
    // auditLog.create() after Prisma had already closed the transaction.
    ...
    // An independent practitioner's sale: the salon issued no document for
    // it, so it issues no credit note either, and her cash never went
    // through the salon's cash book. Her own paperwork is hers.
    ...
    let creditNote = null;
    if (independentSale) {
      // A salon invoice on her payment predates the payee rule and cannot be
      // silently left uncredited: send it to reconciliation instead.
      if (context.invoice) {
        throw new RefundNotAllowedError(
          "INDEPENDENT_SALE_HAS_SALON_INVOICE",
          "Une facture du salon est liée à la vente d'une indépendante. Contactez l'administration pour régulariser avant de rembourser.",
        );
      }
      ...
      // A missing invoice on a VAT-identified customer is ordinarily a
      // mistake to fix before touching money — EXCEPT the one case where it
      // is a deliberate choice: a POS sale where the customer, though
      // VAT-eligible, explicitly declined the invoice
      // (Order.invoiceRequested === false, see actions/boutique/point-of-
      // sale.js). That customer was never going to receive an invoice for
      // this sale, so there is no document to be "missing" and nothing to
      // correct first — it is exactly the plain, document-less B2C path.
      const invoiceDeclinedByChoice = source === "ORDER" && context.order?.invoiceRequested === false;
      if (isBusinessRefundCustomer(refundCustomerFromContext(context)) && !invoiceDeclinedByChoice) {
        throw new RefundNotAllowedError(
      ...
      // B2C (or B2B-eligible-but-opted-out): no invoice was ever issued,
      // therefore no credit note or refund receipt is created. Once all legs
      // settle, an administrator may send the customer a plain
      // refund-confirmation e-mail from Operations.
    }
    ...
    // Only the salon's cash refunds join the salon's cash book.
    const salonCashRefund = (leg) => leg.method === "CASH" && !independentSale;
    const openCashSession = plan.legs.some(salonCashRefund)
    ...
    for (const leg of plan.legs) {
      legData.push({
        ...
        // An independent's cash refund is off-till: no number, no session.
        pieceNumber: salonCashRefund(leg) ? await allocatePieceNumber(tx, pieceSeriesFor(source, context)) : null,
        cashSessionId: salonCashRefund(leg) ? openCashSession?.id ?? null : null,
      });
    }
