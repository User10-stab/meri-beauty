 ...
 *
 * @param {Array<Array<{ available: boolean, time: string, startMinutes: number, endMinutes: number }>>} perDraftSlots
 * @param {number[]} draftOrder — indices into perDraftSlots defining scheduling sequence
 * @param {{ maxSchedules?: number }} [options]
 * @returns {Array<Array<{ draftIndex: number, time: string, startMinutes: number, endMinutes: number }>>}
 */
export function findSequentialSchedulesForOrder(perDraftSlots, draftOrder, { maxSchedules = MAX_SCHEDULES_TO_COLLECT } = {}) {
  ...
  function search(position, notBefore, schedule) {
    ...
    const slots = perDraftSlots[draftIdx].filter(
      (s) => s.available && s.startMinutes >= notBefore
    );

    for (const slot of slots) {
      search(position + 1, slot.endMinutes, [
        ...schedule,
        {
          draftIndex: draftIdx,
          time: slot.time,
