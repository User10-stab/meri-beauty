...
async function billStaffMember(staff, contract, billingDate) {
  ...
  // A failure leaves the rent recorded and waiting: the Factures page offers
  // « Émettre la facture » once the missing data is filled in.
  ...
  console.log(
    `${tag} ✓ rent due recorded (row=${pending.id}, invoice=${invoiceNumber ?? "NOT ISSUED — « Émettre la facture »"}), nextInvoiceDate=${nextDate?.toISOString().split("T")[0] ?? "NULL"}`
  );
