...
export const pointOfSaleSaleSchema = z
  .object({
    ...
    // An unpaid pay-at-pickup order taken over from the orders list
    // (« Encaisser ») — its reservation is released and the order closed in
    // the same transaction as this sale. See lib/orders/point-of-sale-handoff.js.
    sourceOrderId: z.string().trim().min(1).max(100).optional().nullable(),
  })
  ...
  // A QR checkout completes later, in the Stripe webhook — too late to close
  // the original order atomically with the sale, and closing it up front
  // would lose it if the client abandons the QR. Cash or terminal only.
  .refine((data) => !data.sourceOrderId || data.method !== "CARD_QR", {
    message: "Une commande reprise se règle en espèces ou au terminal, pas par QR.",
    path: ["method"],
  });
