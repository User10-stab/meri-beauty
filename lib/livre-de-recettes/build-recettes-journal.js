 ...
 *   actorId?: string }} params actorId, when given, scopes every row to
 *   what THIS user personally recorded (Transaction.recordedById) — every
 *   viewer, including the owner, only sees their own recettes; there is no
 *   longer a single screen showing the salon's total revenue.
 */

// Fields resolveGoodsVatPolicy/resolveServiceVatPolicy need to work out the
// applicable rate for a payment that has no invoice yet.
const CUSTOMER_SELECT = { fullName: true, email: true, isCompany: true, vatNumber: true, vatValidatedAt: true };

export async function buildRecettesJournal(client, { from, to, fromDate, toDate, method, category, actorId }) {
