...
import { categoryForPayment, PAYMENT_CATEGORY_LABELS as CATEGORY_LABELS } from "@/lib/payments/payment-category";
 ...
 * arbitrary date range rather than one till session. Degenerates to exactly
 * buildDayReport's figures when only one session falls in the range (the
 * common "today" view).
 *
...
export async function buildRangeReport(client, { fromDate, toDate }) {
  ...
  const [transactions, perSessionTotals] = await Promise.all([
    client.transaction.findMany({
      where: { paidAt: { gte: fromDate, lte: toDate }, isDeleted: false },
      include: {
        payment: {
          include: {
            invoice: { select: { vatRate: true } },
