 ...
 *
 * The VAT number is the whole test, not the account's "entreprise" flag: a
 * sole trader (indépendant·e) with a validated number is a taxable business
 * — B2B — even without a company name or an account created as a company.
 */
export function hasReusableVatValidation(customer, vatNumber = customer?.vatNumber, now = new Date()) {
  if (!customer?.vatNumber || !vatNumber) return false;
 ...
 * afterward (see actions/invoices/send-invoice-peppyrus.js).
 */
