...
// Shared by every job that only needs to run once a day: staff billing, and
// the Livre de caisse auto-open/auto-close pair below.
const DAILY_BILLING_INTERVAL_MS = 24 * 60 * 60 * 1000;
...
function heartbeat() {
  globalThis.__meriJobsHeartbeat ??= {
    ...
    lastCashAutoOpenRunAt: null,
    lastCashAutoCloseRunAt: null,
  };
...
async function runJobs() {
  ...
  // Staff monthly billing — runs once per day, checks nextInvoiceDate <= today.
  // Uses a 24h cooldown to avoid running on every 5-minute tick.
  const billingHeartbeat = heartbeat();
  const now = Date.now();
  const lastBillingRun = billingHeartbeat.lastBillingRunAt ?? 0;
  const billingDue = now - lastBillingRun >= DAILY_BILLING_INTERVAL_MS;

  if (billingDue) {
    try {
      const billing = await sendDailyStaffInvoices();
      results.sendDailyStaffInvoices = billing;
      billingHeartbeat.lastBillingRunAt = now;
      if (billing.errors > 0) {
  ...
  // Livre de caisse auto-open — runs once per day, and internally re-checks
  // "is it actually past the salon's opening time yet" on every tick, since
  // the 24h cooldown alone only guarantees "hasn't run yet today", not
  // "it's time".
  const cashHeartbeat = heartbeat();
  const lastCashOpenRun = cashHeartbeat.lastCashAutoOpenRunAt ?? 0;
  const cashOpenDue = now - lastCashOpenRun >= DAILY_BILLING_INTERVAL_MS;

  if (cashOpenDue) {
    try {
      const autoOpen = await autoOpenCashSession();
      results.autoOpenCashSession = autoOpen;
      if (autoOpen.opened) cashHeartbeat.lastCashAutoOpenRunAt = now;
    } catch (err) {
      failedJobs.push("autoOpenCashSession");
      captureCriticalError(err, { area: "background-jobs", job: "autoOpenCashSession" });
    }
  }

  // Livre de caisse auto-close — always at midnight Brussels time,
  // regardless of the salon's listed closing time (real sales still happen
  // past 10pm). The cooldown is enough on its own here: once past midnight,
  // "should close" stays true for the rest of the day, and the job itself
  // is a no-op if nothing is open.
  const lastCashCloseRun = cashHeartbeat.lastCashAutoCloseRunAt ?? 0;
  const cashCloseDue = now - lastCashCloseRun >= DAILY_BILLING_INTERVAL_MS;

  if (cashCloseDue) {
    try {
      const autoClose = await autoCloseCashSession();
      results.autoCloseCashSession = autoClose;
      if (autoClose.closed) cashHeartbeat.lastCashAutoCloseRunAt = now;
    } catch (err) {
  ...
  const cashAutoOpen = results.autoOpenCashSession;
