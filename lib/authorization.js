...
// ─── Livre de caisse — who may take cash into the till ──────────────────────────

/**
 * The one staff account (besides any OWNER/ADMIN) whose on-site cash
 * collections are allowed to enter the Livre de caisse and the till
 * reconciliation. Defaults to Marie's production e-mail; overridable per
 * environment so a dev/staging Marie works the same way.
 */
export const TILL_CASH_OPERATOR_EMAIL = (
  process.env.TILL_CASH_OPERATOR_EMAIL || "contact@meribeautystudio.com"
).toLowerCase();

/**
 * Whether this user's cash belongs in the Livre de caisse. Everyone else
 * still settles bookings and rings up sales — their cash collection is just
 * recorded "off-till" (no cash-session link, no piece number), so it shows
 * in Opérations but never in the drawer's book or its X/Z reconciliation.
 * See lib/reservations/settle-reservation.js and actions/appointment/
 * manage-appointment.js for the collecting side.
 *
 * @param {{ role?: string, email?: string } | null | undefined} user
 * @returns {boolean}
 */
export function isTillCashOperator(user) {
  if (!user) return false;
  if (isAdminRole(user.role)) return true;
  return typeof user.email === "string" && user.email.toLowerCase() === TILL_CASH_OPERATOR_EMAIL;
}

/**
 * Whether this user may run the salon's till (the « Caisse » screen): Marie
 * and the admins, plus any staff member granted the CAISSE permission.
 *
 * What the till records is decided by WHOSE sale it is, never by who is
 * standing at it (lib/payments/resolve-payee.js):
 *   - a boutique sale is always the salon's — on-till, salon ticket, salon
 *     revenue, whoever rings it up;
 *   - an independent's own appointment or formation is hers — off-till, no
 *     salon ticket or invoice, never in the salon's revenue.
 * So every "does this go into the Livre de caisse" test is
 * `!independentSale && await canUseSalonTill(actor)`.
 *
 * The screens that are the salon's to review — Livre de caisse, Commandes,
 * invoice sales, awaited transfers — stay on isTillCashOperator.
 *
 * @param {{ id?: string, role?: string, email?: string } | null | undefined} user
 * @returns {Promise<boolean>}
 */
export async function canUseSalonTill(user) {
  if (!user) return false;
  if (isTillCashOperator(user)) return true;
  return hasDashboardPermission(user, STAFF_PERMISSIONS.CAISSE);
}

// ─── Ticket e-mail — who may send a client-facing ticket ───────────────────────

/**
 * Whether this user may e-mail or reprint a client-facing ticket — that is,
 * put a document bearing the salon's name and VAT number into a client's
 * inbox. Only the salon itself may: every practitioner here is legally
 * independent and documents her own sales under her own VAT number.
 *
 * So this is deliberately NOT a grantable permission any more — the
 * SEND_TICKET_EMAIL permission it used to check was deleted on 16/09/2026.
 *
 * Marie Mercier keeps the access the naive reading would take from her: she
 * fails `isAdminRole` — her role is STAFF — and passes
 * `isTillCashOperator`, whose whole purpose is that her VAT number is the
 * salon's. Never replace this with a role test.
 *
 * @param {{ role?: string, email?: string, id?: string } | null | undefined} user
 * @returns {Promise<boolean>}
 */
export async function canSendTicketEmail(user) {
  if (!user) return false;
  return isAdminRole(user.role) || isTillCashOperator(user);
}
...
export const DASHBOARD_PERMISSIONS = {
  ...
  // Marketing (campagnes + prospects) — OWNER/ADMIN uniquement. Le staff
  // (praticiennes indépendantes) ne voit ni les campagnes ni les prospects :
  // données commerciales du salon + e-mails clients hors de leur périmètre.
  // Contrairement à NEWSLETTER, il n'y a volontairement PAS de capacité
  // STAFF associée — voir lib/prospects/require-marketing.js.
  MARKETING: [ROLES.OWNER, ROLES.ADMIN],
...
export const STAFF_PERMISSIONS = Object.freeze({
  ...
  ACTIVITY_SETTLEMENTS: "ACTIVITY_SETTLEMENTS",
  FORMATIONS: "FORMATIONS",
  FORMATION_RESERVATIONS: "FORMATION_RESERVATIONS",
  RETURNS: "RETURNS",
  BOUTIQUE_STOCK: "BOUTIQUE_STOCK",
  // Added 25/09/2026: runs the salon's till (canUseSalonTill). Money still
  // follows the payee, not the cashier: boutique sales are the salon's, her
  // own appointments/formations are hers.
  CAISSE: "CAISSE",
  // Deleted 16/09/2026: POINT_OF_SALE, CASH_REGISTER, ORDERS and
  // SEND_TICKET_EMAIL. The till, the Livre de caisse, boutique orders and the
  // ticket e-mail all produce or record the SALON's sales, and every other
  // practitioner is legally independent — so they are not permissions any
  // more, they belong to the salon's own accounts: isTillCashOperator (admin +
  // Marie Mercier). Stale values still stored on a Staff row are dropped by
  // normalizeStaffPermissions and removed by migration
  // 20260916120000_remove_salon_only_staff_permissions.
});
...
export const DEFAULT_STAFF_PERMISSIONS = Object.freeze([
  ...
  // SEND_TICKET_EMAIL deliberately absent since 16/09/2026: a ticket carries
  // the salon's name and VAT number, and an independent's sale is not the
  // salon's to document. canSendTicketEmail() no longer consults this list
  // at all — see its own note on why removing the default is not enough on
  // its own for accounts that already hold the key.
]);
...
export const STAFF_PERMISSION_OPTIONS = Object.freeze([
  ...
  { key: STAFF_PERMISSIONS.CAISSE, label: "Caisse", description: "Vendre les produits de la boutique (recette du salon) et encaisser ses propres rendez-vous et formations (sa propre recette, sans ticket du salon)." },
  ...
  { key: STAFF_PERMISSIONS.ACTIVITY_ATTENDANCE, label: "Présence ateliers et formations", description: "Marquer absent uniquement les réservations de ses propres séances." },
  { key: STAFF_PERMISSIONS.ACTIVITY_SETTLEMENTS, label: "Clôture ateliers et formations", description: "Encaisser le solde et clôturer uniquement les réservations de ses propres séances." },
