...
/** The ways money actually arrives — the `TransactionMethod` enum. */
export const RECETTES_METHODS = ["CASH", "CARD", "ONLINE", "TRANSFER"];

/** Revenue categories, matching categoryForPayment()'s return values. */
export const RECETTES_CATEGORIES = ["ORDER", "APPOINTMENT", "WORKSHOP", "EVENT", "FORMATION", "STAFF_RENT"];
 ...
 */
export function normalizeRecettesParams({ from, to, method, category } = {}) {
  const now = new Date();
