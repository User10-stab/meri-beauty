...
export function checkStaffReservationReadiness(staff) {
  ...
  if (!contract) {
    ...
    if (today < contractStart) {
      warnings.push("Le contrat n'a pas encore commencé (débute le " + contractStart.toLocaleDateString("fr-FR", { timeZone: "Europe/Brussels" }) + ").");
    }
    ...
    if (contract.endDate) {
      ...
      if (today > contractEnd) {
        warnings.push("Le contrat a expiré (terminé le " + new Date(contract.endDate).toLocaleDateString("fr-FR", { timeZone: "Europe/Brussels" }) + ").");
      }
    }
  }
