...
// SHIPPED is deliberately excluded: a Stripe-Dashboard refund is money
// movement, not proof that shipped stock physically came back. Restocking a
// shipped order here creates phantom inventory; staff must process an actual
// return/cancellation flow once goods are in hand.
const REFUNDABLE_ORDER_STATUSES = ["PAID", "PROCESSING", "READY_FOR_PICKUP"];
...
export async function reconcileStripeProductOrderRefund(
  ...
  // The caller checks payment.invoice (not included in this query) and
  // issues a credit note for newlyRefunded only after this returns — this id
  // is what lets it link that credit note back to the row created here.
