...
const ERROR_MESSAGES = {
  ...
  INVOICE_REPLACEMENT_SALON_ONLY:
    "Cette réservation est déjà facturée : modifier le nombre de places émettrait une note de crédit et une nouvelle facture du salon. Seuls l'administration et Marie peuvent le faire.",
 ...
 * @param {boolean} [params.canReplaceInvoice] True only for the salon's own
 *   accounts (isTillCashOperator: admin + Marie). An invoiced booking's seat
 *   change issues a credit note and a replacement invoice in the salon's
 *   name, which no independent may do — so it is refused for anyone else.
 *   Defaults to false: a caller that forgets it fails closed.
 */
export async function changeReservationSeatsFree({ kind, reservationId, newSeatsCount, reason, actorId, canReplaceInvoice = false }) {
  ...
  try {
    result = await prisma.$transaction(
      async (tx) => {
        ...
        if (payment.invoice && !canReplaceInvoice) throw new Error("INVOICE_REPLACEMENT_SALON_ONLY");
