...
export async function fulfillOrderPayment(session) {
  ...
  const order = await prisma.order.findUnique({
    ...
    include: {
      ...
      createdByStaff: { select: { role: true, email: true } },
    },
  });
  ...
  // A non-privileged staff member rang this up at the till (QR checkout) —
  // see isTillCashOperator. False for every genuine online order, which has
  // no staff actor at all.
  const isStaffActor = isPointOfSale && !isTillCashOperator(order.createdByStaff);
  ...
  ({ invoice, ticketNumber } = await prisma.$transaction(async (tx) => {
    ...
    // Invoice to be created — see isTillCashOperator — regardless of VAT
