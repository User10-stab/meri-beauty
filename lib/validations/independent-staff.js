...
export const updateIndependentStaffSchema = z.object({
  ...
  // Address stays optional here — for two reasons: the StaffTable
  // active-toggle sends a partial payload without address fields at all, and
  // the full edit form always sends real strings (never undefined), so a
  // profile created without an address (optional since creation was relaxed
  // — see createIndependentStaffSchema) must still be editable with these
  // left blank instead of failing on the old "obligatoire" min-length checks.
  addressLine1: addressLine1OptionalSchema,
  addressLine2: addressLine2Schema,
  addressCity: addressCityOptionalSchema,
  addressPostalCode: addressPostalCodeOptionalSchema,
  addressCountry: addressCountryOptionalSchema,
  ...
  yearsOfExperience: yearsOfExperienceSchema,
  hireDate:          hireDateSchema,
  // Optional here too, same reasoning as the address fields above: a profile
  // created without a VAT number must stay editable without one.
  vatNumber:         vatNumberOptionalSchema,
  rythme:            rythmeSchema,
