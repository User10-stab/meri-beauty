 ...
 * — which is exactly why NOTIFIED entries are included below alongside
 * WAITING ones: without an expiry, someone notified about an earlier opening
 * who hasn't booked (or given up) yet is still on the list, and must keep
 * hearing about every later opening too. Only WAITING was queried here
 * originally, so once the very first opening notified everyone, the whole
 * list silently went permanently deaf to every subsequent one — a real seat
 * freed up with nobody left to tell.
 *
