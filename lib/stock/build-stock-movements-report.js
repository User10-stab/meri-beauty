 ...
 * @param {{ from?: string, to?: string, type?: string, actorId?: string }} [params]
 *   actorId, when given, scopes every row to movements THIS user personally
 *   recorded (InventoryMovement.createdById) — every viewer, including the
 *   owner, only sees their own stock movements.
 */
...
export async function buildStockMovementsReport(client, params = {}) {
  // actorId deliberately taken from the raw params, not `normalized` —
  // normalizeStockMovementsParams only knows about from/to/type and would
  // silently drop it.
