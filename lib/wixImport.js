...
/**
 * Resolves one product's brand/subcategory names from the admin-confirmed
 ...
 * Neither is guaranteed to resolve — a product with no subcategory-classified
 * slug at all needs the caller's "Non classé" fallback.
 */
export function resolveProductClassification(product, slugMapping) {
  ...
  const subcategorySlug = ordered.find((s) => kindOf(s) === "subcategory");

  return {
    brandName: brandSlug ? titleCase(brandSlug) : null,
    subcategoryName: subcategorySlug ? titleCase(subcategorySlug) : null,
  };
}
