 ...
 *   6. Advance nextInvoiceDate (status stays GENERATED until manual send)
 */
...
async function billStaffMember(staff, contract, billingDate) {
  ...
  // ── 6. Advance the schedule — generation only, never emailed ──────────
  // The invoice stays GENERATED until an admin sends it manually from the
  // dashboard ("Envoyer la facture" / "Renvoyer" via resendMonthlyInvoiceEmail).
  // nextInvoiceDate advances here so the next cycle is scheduled even though
  // no email was sent. Billing amounts, periods and dedup rules are unchanged.
  ...
  console.log(`${tag} ✓ invoice=${invoice.number} generated (not emailed), nextInvoiceDate=${nextDate?.toISOString().split("T")[0] ?? "NULL"}`);
  return { staffId: staff.id, status: "GENERATED", invoiceNumber: invoice.number, emailed: false };
}
...
export async function sendDailyStaffInvoices() {
  ...
  console.log(
    `[monthly-billing] done — processed=${summary.processed} generated=${summary.generated} ` +
