...
async function runJobs() {
  try {
    const [orders, workshopReminders, formationReminders, appointmentReminders, workshopHolds, formationHolds, refundRetries, missedRefunds] = await Promise.all([
      expireStaleOrders(),
    ...
    if (
      ...
      console.log(
        `[background-jobs] expired ${orders.expiredCount} order(s), ${workshopHolds.expiredCount} workshop hold(s), ${formationHolds.expiredCount} formation hold(s), sent ${workshopReminders.sentCount} workshop + ${formationReminders.sentCount} formation + ${appointmentReminders.sentCount} appointment reminder(s), retried ${refundRetries.retried} refund(s) (${refundRetries.succeeded} recovered, ${refundRetries.exhausted} exhausted), recovered ${missedRefunds.reconciled} missed Stripe refund(s) out of ${missedRefunds.checked} checked`
      );
    }
