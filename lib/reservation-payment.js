...
export function getReservationPaymentDecision({
  ...
  if (normalizedConfirmationMode === "MANUAL") {
    // A CASH_ONLY staff member takes no online payment ever, so pay-first
    // cannot apply — preserve the original manual flow: request now, decide
    // later, pay in person. Checked here rather than left to a caller's
    // short-circuit, because this function is also called directly by
    // createReservation and createCheckoutSession, which bypass
    // computePaymentDecision's own CASH_ONLY branch entirely.
    if (normalizedAllowedMethods === "CASH_ONLY") {
      return {
        requiresPaymentStep: false,
        isManualMode: true,
        onlineFullPaymentAvailable: false,
    ...
    // Every other manual staff member (BOTH or ONLINE_ONLY): the customer
    // pays before staff decide, not after. Whichever they're configured for
    // — a deposit, or the full amount — is taken up front, and the request
    // stays PENDING until accepted. If declined, rejectAppointment refunds
    // what was taken (see isDeclineOfUnacceptedRequest there).
    //
    // No pay-on-site choice here: the whole point is that money has changed
    // hands before anyone commits to the slot.
    return {
      ...
      salonPaymentAvailable: false,
      depositRequired,
      depositPercentage: depositRequired ? depositPct : 0,
      depositAmount,
      totalAmount: total,
      requiresOnlinePaymentNow: true,
      paymentIntent: depositRequired ? "DEPOSIT_ONLINE" : "FULL_ONLINE",
      paymentType: depositRequired ? "DEPOSIT" : "ONLINE",
      shouldCreatePaymentRecord: true,
      appointmentStatusBeforePayment: "PENDING",
      // Paying does not confirm a manual request — only staff acceptance
      // does. resolveAppointmentStatusAfterPayment enforces this on the
      // webhook side by preserving PENDING.
      appointmentStatusAfterPayment: "PENDING",
