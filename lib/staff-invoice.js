 ...
 *
 * Since 2026-09-21 NO invoice is issued here: the rent is paid by transfer
 * and, like every sale on the site, only invoiced once paid. This records the
 * rent due — a StaffMonthlyInvoice row (AWAITING_PAYMENT) with a PENDING
 * Payment (lib/staff-rent-payment.js) — and the admin accepts it from the
 * Factures page once the transfer arrived, which issues the invoice
 * (actions/invoices/staff-rent.js).
 *
...
export async function createAndSendStaffContractInvoice({ contract }) {
  ...
  return { invoice: null, pendingRent, emailed: false };
}
