...
export async function buildRecettesWorkbook(data) {
  ...
  [
    ...
    ["Total hors TVA", summary.totalHt],
    ["Total TVA", summary.totalVat],
    ["Total des recettes", summary.total],
  ...
  synth.mergeCells("A12:D12");
  synth.getCell("A12").value = "PAR MOYEN DE PAIEMENT";
  styleSection(synth.getCell("A12"));
  synth.getRow(12).height = 22;
  addTable(synth, {
    startRow: 13,
  ...
  const catStart = 13 + summary.byMethod.length + 2;
