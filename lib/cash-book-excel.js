...
import { sumLedgerVat } from "@/lib/cash-book/day-groups";
...
export async function buildCashBookWorkbook({ ledger, report }) {
  ...
  const exportedAt = new Date().toLocaleString("fr-BE", {
    day: "2-digit",
    month: "2-digit",
    year: "numeric",
    hour: "2-digit",
  ...
  const subtitle = `Du ${filters.from} au ${filters.to} · Exporté le ${exportedAt}`;
  ...
  [
    ["Total des entrées", totals.entrees],
    ["Total des sorties", totals.sorties],
    ["Total HT", vatTotals.ht],
    ["Total TVA", vatTotals.vat],
  ...
  if (report) {
    ...
    addTable(synth, {
      ...
      rows: [
        ...
        ...(report.ecart ? [["Écart de caisse", report.ecart]] : []),
    ...
    nextRow +=
      4 +
      (report.openingBalance != null ? 1 : 0) +
      (report.ecart ? 1 : 0) +
      (report.expectedCash != null ? 1 : 0) +
      2;
  ...
  addWorkbookHeader(journal, { title: "Journal de caisse", subtitle, columns: 10 });
  addTable(journal, {
    startRow: 4,
    headers: ["Date", "N° pièce", "Référence", "Désignation", "Taux TVA", "HT (€)", "TVA (€)", "Entrées (€)", "Sorties (€)", "Solde (€)"],
    rows: rows.map((row) => [
    ...
    moneyColumns: [6, 7, 8, 9, 10],
    widths: [20, 14, 16, 40, 10, 14, 12, 14, 14, 14],
  });
