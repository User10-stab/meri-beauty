...
export function creditNoteEligibility(invoice) {
  ...
  // A rent invoice now carries its pending transfer Payment
  // (lib/staff-rent-payment.js), but still has no sale to correct.
  if (invoice.source === "STAFF_CONTRACT") return { allowed: false, reason: "Aucun paiement lié (facture de contrat)." };
  ...
  if (credited + 0.01 >= Number(invoice.totalInclVat ?? 0)) return { allowed: false, reason: "Facture entièrement créditée." };
