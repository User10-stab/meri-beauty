...
// ─── Salon branding (header logo / phone / address) ────────────────────────────
// The single "main-salon" row is the source of truth for the brand identity
// shown in every email header. Loaded once (lazily, cached) and refreshed at
// server start; buildEmailShell reads the cached value synchronously so it
// never has to become async (which would force `await` onto ~40 call sites).
// If the cache is still empty (very first email before warmup), the header
// gracefully degrades to the wordmark only — never a plain/default email.

let _salonBranding = null;
let _salonBrandingLoading = false;

export function getSalonBrandingSync() {
  return _salonBranding;
}

export async function refreshSalonBranding() {
  if (_salonBrandingLoading) return _salonBranding;
  _salonBrandingLoading = true;
  try {
    ...
    if (salon) {
      ...
      _salonBranding = {
        name: salon.name || "Meri Beauty",
        logo: salon.logo || null,
        phone: salon.phone || null,
        address: structured || salon.address || null,
        instagram: salon.instagram || null,
        facebook: salon.facebook || null,
      };
    }
  } catch (err) {
    console.error("[email-templates] refreshSalonBranding failed:", err);
  } finally {
    _salonBrandingLoading = false;
  }
  return _salonBranding;
}

/**
 * Builds the full responsive, branded HTML email shell (dark header with
 * logo + salon phone/address, white body, footer). Every template function
 * and the central sendEmail guard route through this, so no email can leave
 * the app looking like plain/default markup.
 *
 * @param {string} title
 * @param {string} bodyHtml
 * @param {string|null} [unsubscribeUrl]
 * @param {{name?:string,logo?:string|null,phone?:string|null,address?:string|null,instagram?:string|null,facebook?:string|null}|null} [branding]
 */
function buildEmailShell(title, bodyHtml, unsubscribeUrl = null, branding = null) {
  const name     = branding?.name    || "Meri Beauty";
  const phone    = branding?.phone   || null;
  const address  = branding?.address || null;
  const instagram = branding?.instagram || null;
  const facebook  = branding?.facebook  || null;

  // Website: derive from env so it stays in sync with the actual deployment URL
  const website = process.env.NEXT_PUBLIC_APP_URL
    ? process.env.NEXT_PUBLIC_APP_URL.replace(/^https?:\/\//, "")
    : null;
  const websiteHref = process.env.NEXT_PUBLIC_APP_URL || null;

  const logoUrl = branding?.logo
    ? (branding.logo.startsWith("http://") || branding.logo.startsWith("https://")
        ? branding.logo
        : getAbsoluteUrl(branding.logo))
    : null;

  const logoBlock = logoUrl
    ? `<img src="${escapeHtml(logoUrl)}" alt="${escapeHtml(name)}" style="display:inline-block;max-height:60px;width:auto;" />`
    : "";

  const nameBlock = `<div style="color:#ffffff;font-size:22px;font-weight:700;letter-spacing:1px;line-height:1.2;${logoUrl ? "margin-top:10px;" : ""}">${escapeHtml(name)}</div>`;

  const contactBits = [];
  if (phone)   contactBits.push(`<span style="white-space:nowrap;">${escapeHtml(phone)}</span>`);
  if (address) contactBits.push(`<span style="white-space:nowrap;">${escapeHtml(address)}</span>`);
  const contactBlock = contactBits.length
    ? `<div style="margin-top:10px;color:#e9e3d7;font-size:13px;line-height:1.7;">${contactBits.join('<span style="opacity:.45;margin:0 8px;">·</span>')}</div>`
    : "";

  // ── Footer columns ─────────────────────────────────────────────────────────
  const hasContact = phone || address || websiteHref;
  const hasSocial  = instagram || facebook;

  const contactColHtml = hasContact ? `
    <td style="padding:0 20px 0 0;vertical-align:top;width:50%;">
      <p style="margin:0 0 10px;font-size:10px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:#999;">Nous contacter</p>
      ${phone    ? `<p style="margin:0 0 5px;font-size:13px;"><a href="tel:${escapeHtml(phone.replace(/\s/g,""))}" style="color:#C8A46A;text-decoration:none;font-weight:600;">${escapeHtml(phone)}</a></p>` : ""}
      ${address  ? `<p style="margin:0 0 5px;font-size:12px;color:#888;line-height:1.5;">${escapeHtml(address)}</p>` : ""}
      ${websiteHref ? `<p style="margin:0;font-size:12px;"><a href="${escapeHtml(websiteHref)}" style="color:#2F3A2E;text-decoration:underline;">${escapeHtml(website)}</a></p>` : ""}
    </td>` : "";

  const socialColHtml = hasSocial ? `
    <td style="padding:0 0 0 ${hasContact ? "20px" : "0"};vertical-align:top;${hasContact ? "border-left:1px solid #e8e5e0;" : ""}width:50%;">
      <p style="margin:0 0 10px;font-size:10px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:#999;">Rejoignez-nous</p>
      ${instagram ? `<p style="margin:0 0 5px;font-size:12px;color:#666;">${escapeHtml(instagram)}</p>` : ""}
      ${facebook  ? `<p style="margin:0 0 5px;font-size:12px;color:#666;">${escapeHtml(facebook)}</p>` : ""}
      <p style="margin:0;font-size:12px;color:#666;">Donnez votre avis</p>
    </td>` : "";

  const footerColumns = (hasContact || hasSocial) ? `
    ...
    <div style="height:1px;background:#e8e5e0;margin-bottom:16px;"></div>` : "";
...
<head>
  ...
  <style>
    @media only screen and (max-width:600px){
      ...
      .mb-header,.mb-body,.mb-footer{padding-left:20px!important;padding-right:20px!important;}
      .mb-footer-cols td{display:block!important;width:100%!important;padding:0 0 16px 0!important;border-left:none!important;}
    }
  </style>
</head>
<body style="margin:0;padding:0;background:#f5f4f0;font-family:Arial,Helvetica,sans-serif;">
  <!-- MERI_BEAUTY_EMAIL_SHELL -->
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f5f4f0;padding:40px 0;">
    <tr>
      <td align="center">
        <table class="mb-container" width="580" cellpadding="0" cellspacing="0" style="width:580px;max-width:580px;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.07);">

          <!-- ── Header ── -->
          <tr>
            <td class="mb-header" style="background:${darkColor};padding:28px 40px 24px;text-align:center;">
              ${logoBlock}
              ${nameBlock}
              ${contactBlock}
            </td>
          ...
          <!-- ── Body ── -->
          <tr>
            <td class="mb-body" style="padding:36px 40px 32px;">
              ${bodyHtml}
            </td>
          </tr>
          ...
          <!-- ── Footer ── -->
          <tr>
            <td class="mb-footer" style="background:#f9f8f5;border-top:1px solid #eeede9;padding:28px 40px 20px;">
              ${footerColumns}
              <p style="margin:0;font-size:11px;color:#bbb;text-align:center;">
                © ${new Date().getFullYear()} ${escapeHtml(name)} – Tous droits réservés.
              </p>
              ${unsubscribeUrl ? `
              <p style="margin:6px 0 0;font-size:11px;color:#ccc;text-align:center;">
                <a href="${escapeHtml(unsubscribeUrl)}" style="color:#bbb;text-decoration:underline;">Se désabonner</a>
              </p>` : ""}
            </td>
          </tr>
...
function htmlWrapper(title, bodyHtml, unsubscribeUrl = null) {
  return buildEmailShell(title, bodyHtml, unsubscribeUrl, getSalonBrandingSync());
}

/**
 * Public wrapper used by the central sendEmail guard to bring ad-hoc HTML
 * (call sites that still hand-build a fragment) into the same branded shell.
 * Callers that already go through a template function should NOT use this —
 * their output already carries the shell (see the MERI_BEAUTY_EMAIL_SHELL
 * marker), and re-wrapping would double-wrap.
 */
export function brandedHtml(title, bodyHtml, unsubscribeUrl = null) {
  return buildEmailShell(title, bodyHtml, unsubscribeUrl, getSalonBrandingSync());
}
...
L'équipe Meri Beauty
  ...
  const html = htmlWrapper(
    ...
    ${pendingInvoiceBlock(pendingInvoiceNote)}
...
export function manualDepositRequiredEmail({
  customerName,
  serviceName,
  staffName,
  date,
  ...
  const amountToPay = Number(depositAmount);
  const total = Number(totalAmount);
  const remaining = Math.max(0, total - amountToPay);
  const isFullPayment = Math.abs(amountToPay - total) < 0.01;
  const subject = isFullPayment
    ? "Votre réservation nécessite un paiement – Meri Beauty"
    : "Votre réservation nécessite le paiement de l'acompte – Meri Beauty";
  const amountLabel = isFullPayment ? "Montant à régler" : "Acompte à régler";
  const text = `
Bonjour ${customerName},

Votre réservation a bien été créée, mais elle n'est pas encore confirmée. ${isFullPayment ? "Un paiement est requis pour la valider." : "Un acompte est requis pour la valider."}

  Service   : ${serviceName}
  Experte   : ${staffName}
  Date      : ${formatDate(date)} à ${time}
  ${amountLabel} : €${amountToPay.toFixed(2)}${!isFullPayment ? ` (sur un total de €${total.toFixed(2)})` : ""}

Veuillez régler ${isFullPayment ? "le montant" : "l'acompte"} en cliquant sur le lien ci-dessous :

${paymentUrl}

Une fois le paiement effectué, votre rendez-vous sera automatiquement confirmé.
...
L'équipe Meri Beauty
  ...
  const html = htmlWrapper(subject, `
    <h2 style="margin:0 0 8px;font-size:22px;color:${darkColor};">${isFullPayment ? "Paiement à régler" : "Acompte à régler"}</h2>
    <p style="margin:0 0 24px;color:#666;font-size:15px;">Bonjour <strong>${escapeHtml(customerName)}</strong>,<br/>
    Votre réservation a bien été ajoutée. Pour la confirmer, merci de régler ${isFullPayment ? "le montant demandé" : "l'acompte demandé"}.</p>
    <table cellpadding="0" cellspacing="0" style="width:100%;background:#f9f8f5;border-radius:8px;padding:4px 20px;margin-bottom:24px;"><tbody>
      ${infoRow("Service", escapeHtml(serviceName))}
      ${infoRow("Experte", escapeHtml(staffName))}
      ${infoRow("Date", escapeHtml(formatDate(date)))}
      ${infoRow("Heure", escapeHtml(time))}
      ${infoRow(isFullPayment ? "Montant" : "Acompte", `<span style="color:${brandColor};font-size:16px;font-weight:700;">€${amountToPay.toFixed(2)}</span>`)}
      ${!isFullPayment ? infoRow("Total", `€${total.toFixed(2)}`) : ""}
    </tbody></table>
    <div style="background:#fff3cd;border-left:4px solid ${brandColor};padding:16px;border-radius:4px;margin-bottom:24px;">
      <p style="margin:0;font-size:14px;color:#856404;line-height:1.6;">
        ...
        Votre créneau est réservé provisoirement. Après le paiement, votre rendez-vous passera automatiquement au statut <strong>CONFIRMÉ</strong>.
      </p>
    </div>
    <a href="${escapeHtml(paymentUrl)}" style="display:inline-block;background:${brandColor};color:#fff;text-decoration:none;padding:14px 32px;border-radius:8px;font-size:16px;font-weight:600;margin-bottom:12px;">${isFullPayment ? `Payer — €${amountToPay.toFixed(2)}` : `Payer l'acompte — €${amountToPay.toFixed(2)}`}</a>
    ${remaining > 0.01 ? `<p style="margin:0 0 24px;font-size:13px;color:#999;">Paiement sécurisé via Stripe. Le solde de €${remaining.toFixed(2)} sera à régler au salon.</p>` : `<p style="margin:0 0 24px;font-size:13px;color:#999;">Paiement sécurisé via Stripe.</p>`}
    ${divider()}
    <p style="margin:0;font-size:14px;color:#888;">À bientôt,<br/><strong style="color:${darkColor};">L'équipe Meri Beauty</strong></p>
  `);
  return { subject, text, html };
...
/**
 * Staff "Relancer le paiement" on an atelier or formation booking whose online
 * payment never went through. Same shape as manualDepositRequiredEmail, but an
 * activity has a session date and a seat count rather than an expert and a
 * time slot, and the place is only held until the link expires.
 */
export function activityPaymentRelanceEmail({
  customerName,
  activityLabel,
  activityTitle,
  sessionDate,
  ...
  const subject = `Finalisez le paiement de votre réservation – ${activityTitle} – Meri Beauty`;
  const amountLabel = isFullPayment ? "Montant à régler" : "Acompte à régler";
  const seats = `${seatsCount} place${seatsCount > 1 ? "s" : ""}`;
...
Le paiement de votre réservation pour ${activityLabel} "${activityTitle}" n'a pas abouti. Votre place vous est de nouveau réservée : il vous suffit de finaliser le paiement.

  Séance   : ${sessionDate}
  Places   : ${seats}
  ${amountLabel} : €${amount.toFixed(2)}${!isFullPayment ? ` (sur un total de €${total.toFixed(2)})` : ""}

Payer en ligne :

${paymentUrl}

Ce lien est valable jusqu'au ${expiry}. Passé ce délai, la place sera libérée.
...
L'équipe Meri Beauty
  ...
  const html = htmlWrapper(subject, `
    <h2 style="margin:0 0 8px;font-size:22px;color:${darkColor};">${isFullPayment ? "Paiement à finaliser" : "Acompte à finaliser"}</h2>
    <p style="margin:0 0 24px;color:#666;font-size:15px;">Bonjour <strong>${escapeHtml(customerName)}</strong>,<br/>
    Le paiement de votre réservation pour ${escapeHtml(activityLabel)} n'a pas abouti. Votre place vous est de nouveau réservée : il vous suffit de finaliser le paiement.</p>
    <table cellpadding="0" cellspacing="0" style="width:100%;background:#f9f8f5;border-radius:8px;padding:4px 20px;margin-bottom:24px;"><tbody>
      ${infoRow(activityLabel === "la formation" ? "Formation" : "Atelier", escapeHtml(activityTitle))}
      ${infoRow("Séance", escapeHtml(sessionDate))}
      ${infoRow("Places", escapeHtml(seats))}
      ${infoRow(isFullPayment ? "Montant" : "Acompte", `<span style="color:${brandColor};font-size:16px;font-weight:700;">€${amount.toFixed(2)}</span>`)}
      ${!isFullPayment ? infoRow("Total", `€${total.toFixed(2)}`) : ""}
    </tbody></table>
    <div style="background:#fff3cd;border-left:4px solid ${brandColor};padding:16px;border-radius:4px;margin-bottom:24px;">
      <p style="margin:0;font-size:14px;color:#856404;line-height:1.6;">
        ⏳ <strong>Place réservée jusqu'au ${escapeHtml(expiry)}</strong><br/>
        Passé ce délai, le lien expire et la place est libérée. Dès le paiement reçu, votre réservation est automatiquement <strong>CONFIRMÉE</strong>.
      </p>
    </div>
    <a href="${escapeHtml(paymentUrl)}" style="display:inline-block;background:${brandColor};color:#fff;text-decoration:none;padding:14px 32px;border-radius:8px;font-size:16px;font-weight:600;margin-bottom:12px;">${isFullPayment ? `Payer — €${amount.toFixed(2)}` : `Payer l'acompte — €${amount.toFixed(2)}`}</a>
    ${!isFullPayment ? `<p style="margin:0 0 24px;font-size:13px;color:#999;">Paiement sécurisé via Stripe. Le solde de €${remaining.toFixed(2)} sera à régler au salon.</p>` : `<p style="margin:0 0 24px;font-size:13px;color:#999;">Paiement sécurisé via Stripe.</p>`}
    ${divider()}
    <p style="margin:0;font-size:14px;color:#888;">À bientôt,<br/><strong style="color:${darkColor};">L'équipe Meri Beauty</strong></p>
  `);
  return { subject, text, html };
...
Ce lien expire dans ${formatDurationLong(expiresInMinutes)} pour des raisons de sécurité.
...
L'équipe Meri Beauty
  ...
  const html = htmlWrapper(
    ...
    <div style="text-align:center;margin:0 0 24px;">
      <a href="${resetUrl}" style="display:inline-block;background:#2F3A2E;color:#ffffff;font-size:15px;font-weight:700;text-decoration:none;padding:14px 32px;border-radius:999px;">Réinitialiser mon mot de passe</a>
    </div>

    <table cellpadding="0" cellspacing="0" style="width:100%;background:#f9f8f5;border-radius:8px;padding:4px 20px;margin-bottom:24px;">
      <tbody>
        ${infoRow("Expire dans", formatDurationLong(expiresInMinutes))}
      </tbody>
    </table>
 ...
 *   variant?: string | null,
 * }} params
 * `variant: "reservation"` tailors the copy to the appointment-reservation
 * flow (the client returns straight to their booking after one click).
 * Every other caller keeps the exact copy as before.
 */
export function emailVerificationEmail({
  ...
  const subject = isReservation
    ? "Validez votre adresse email pour continuer votre réservation – Meri Beauty"
    : "Validez votre adresse email – Meri Beauty";

  const text = isReservation ? `
Bonjour ${customerName},

Merci pour votre réservation sur Meri Beauty. Dernière étape avant de continuer : vérifiez votre adresse email en cliquant sur le bouton ci-dessous :

  ${verificationUrl}

Ce lien expire dans ${formatDurationLong(expiresInMinutes)} pour des raisons de sécurité. Après vérification, vous reviendrez directement à votre réservation, sans rien ressaisir.

Si vous n'avez pas fait de réservation, vous pouvez ignorer cet email.
...
L'équipe Meri Beauty
  ...
  const introLine = isReservation
    ? `Merci pour votre réservation sur Meri Beauty. Dernière étape avant de continuer : vérifiez votre adresse email en un clic. Après vérification, vous reviendrez directement à votre réservation, sans rien ressaisir.`
    : `Merci de vous être inscrit sur Meri Beauty. Pour activer votre compte, veuillez vérifier votre adresse email.`;

  const ignoreLine = isReservation
  ...
  const html = htmlWrapper(
    subject,
    `
    <h2 style="margin:0 0 8px;font-size:22px;color:${darkColor};">Validez votre adresse email</h2>
    <p style="margin:0 0 24px;color:#666;font-size:15px;">Bonjour <strong>${customerName}</strong>,<br/>
    ${introLine}</p>

    <div style="text-align:center;margin:0 0 24px;">
      <a href="${verificationUrl}" style="display:inline-block;background:#2F3A2E;color:#ffffff;font-size:15px;font-weight:700;text-decoration:none;padding:14px 32px;border-radius:999px;">Vérifier mon email</a>
    </div>
...
/**
 * The sentence that replaced the invoice PDF these e-mails used to carry
 * (3 Sep 2026 policy: only tickets go out automatically, on every channel).
 * The invoice is still issued and numbered — Belgian VAT requires it — only
 * its delivery moved to Opérations: Peppol for a Belgian company, e-mail on
 * demand for anyone else (see actions/invoices/send-invoice-peppyrus.js and
 * actions/invoices/send-invoice-email.js). Callers pick the wording with
 * isPeppolMandatoryCustomer (lib/tax-policy.js); this only renders it.
 */
function pendingInvoiceBlock(note) {
  if (!note) return "";
  return `
    <div style="background:#f4f6f4;border-left:4px solid ${darkColor};padding:14px 16px;border-radius:4px;margin-bottom:24px;">
      <p style="margin:0;font-size:13px;color:#555;line-height:1.6;">🧾 ${escapeHtml(note)}</p>
    </div>
  `;
}
 ...
 */
/**
 * `refundPending` is the third state this template was missing.
 *
 * It used to take a single boolean, so "the refund is approved but the money
 * has not moved yet" fell through to the `false` branch and told the
 * customer the exact opposite of the decision just taken —
 * "l'acompte n'est pas remboursable" on a refund an admin had approved
 * seconds earlier. Since the application no longer refunds anything itself
 * (an admin does it by hand in Stripe), that in-between state is now the
 * NORMAL one at cancellation time, not an edge case.
 *
 * `refunded: true` is reserved for money that has actually landed — which in
 * practice is announced by lib/refunds/notify-refund-complete.js, not here.
 *
 * @param {{refunded?: boolean, refundPending?: boolean, refundAmount?: number|null, decisionNote?: string|null}} params
 */
export function workshopCancellationEmail({
  customerName,
  activityTitle,
  sessionDate,
  refunded = false,
  ...
  const formattedAmount =
    refundAmount == null
      ? null
      : new Intl.NumberFormat("fr-BE", { style: "currency", currency: "EUR" }).format(Number(refundAmount));

  const depositNote = refunded
    ? "À titre exceptionnel, l'acompte versé vous a été remboursé."
    : refundPending
      ? `Un remboursement${formattedAmount ? ` de ${formattedAmount}` : ""} a été approuvé à titre exceptionnel et vous sera envoyé sous peu. Vous recevrez une confirmation dès qu'il aura été effectué.`
      : "Conformément à nos conditions, l'acompte déjà versé n'est pas remboursable.";

  // The admin's written reply to an exceptional request. It was being
  // dropped entirely on the approval path while the rejection path did send
  // it — so a customer whose request was refused heard why, and one whose
  // request was granted heard nothing.
  const teamMessage = decisionNote?.trim() ? `Message de l'équipe : ${decisionNote.trim()}` : null;
  const positive = refunded || refundPending;
...
L'équipe Meri Beauty
  ...
  const html = htmlWrapper(
    ...
    <div style="background:${positive ? "#d1e7dd" : "#f8d7da"};border-left:4px solid ${positive ? "#0f5132" : "#842029"};padding:14px 16px;border-radius:4px;margin-bottom:24px;">
      <p style="margin:0;font-size:13px;color:${positive ? "#0f5132" : "#842029"};line-height:1.6;">
        ${depositNote}
      </p>
    </div>
    ...
    ${
      teamMessage
        ? `<div style="background:#f4f4f2;padding:14px 16px;border-radius:4px;margin-bottom:24px;">
      <p style="margin:0;font-size:13px;color:#444;line-height:1.6;">${escapeHtml(teamMessage)}</p>
    </div>`
        : ""
    }
...
/**
 * Sent after either a free admin transfer or a legacy paid session change.
 * The optional fee keeps already-issued Stripe links backwards compatible;
 * new admin transfers use the price/balance fields and never charge a fee.
 */
export function workshopSessionChangeEmail({
  ...
  const directTransfer = totalPrice != null;
  const destinationTitle = newActivityTitle || activityTitle;
  const subject = `Votre réservation pour "${destinationTitle}" a été modifiée – Meri Beauty`;

  const financialText = directTransfer
    ? `\n  Nouveau total : €${Number(totalPrice).toFixed(2)}\n  Déjà payé : €${Number(paidAmount).toFixed(2)}\n  Solde restant : €${Number(balanceDue).toFixed(2)}\n  Frais de modification : €0.00`
    : `\n  Frais de modification réglés : €${Number(changeFeeAmount).toFixed(2)}`;

  const text = `
Bonjour ${customerName},

Votre réservation a bien été déplacée par l'équipe Meri Beauty.

  Activité précédente : ${previousActivityTitle || activityTitle}
  Ancienne séance : ${previousSessionDate}
  Nouvelle activité : ${destinationTitle}
...
L'équipe Meri Beauty
  ...
  const html = htmlWrapper(
    ...
    <table cellpadding="0" cellspacing="0" style="width:100%;background:#f9f8f5;border-radius:8px;padding:4px 20px;margin-bottom:24px;">
      <tbody>
        ${infoRow("Activité précédente", previousActivityTitle || activityTitle)}
        ${infoRow("Ancienne séance", previousSessionDate)}
        ${infoRow("Nouvelle activité", destinationTitle)}
        ${infoRow("Nouvelle séance", newSessionDate)}
        ${
          directTransfer
            ? `${infoRow("Nouveau total", `€${Number(totalPrice).toFixed(2)}`)}
               ${infoRow("Déjà payé", `€${Number(paidAmount).toFixed(2)}`)}
               ${infoRow("Solde restant", `€${Number(balanceDue).toFixed(2)}`)}
               ${infoRow("Frais de modification", "€0.00")}`
...
/**
 * @param {{ customerName: string, reference: string, itemsSummary: string }} params
 */
export function returnRequestReceivedEmail({ customerName, reference, itemsSummary }) {
  const retReceivedSubject = `Demande de retour reçue – Ticket ${reference} – Meri Beauty`;

  const retReceivedText = `
Bonjour ${customerName},

Nous avons bien reçu votre demande de retour pour le ticket ${reference} :
...
L'équipe Meri Beauty
  ...
  const retReceivedHtml = htmlWrapper(
    ...
    <p style="margin:0 0 24px;color:#666;font-size:15px;">Bonjour <strong>${escapeHtml(customerName)}</strong>,<br/>
    Nous avons bien reçu votre demande de retour pour le ticket <strong>${reference}</strong> :</p>
...
/**
 * @param {{ customerName: string, reference: string, instructions: string }} params
 */
export function returnApprovedEmail({ customerName, reference, instructions }) {
  const retApprovedSubject = `Retour approuvé – Ticket ${reference} – Meri Beauty`;

  const retApprovedText = `
Bonjour ${customerName},

Votre demande de retour pour le ticket ${reference} a été approuvée.
...
L'équipe Meri Beauty
  ...
  const retApprovedHtml = htmlWrapper(
    ...
    <p style="margin:0 0 24px;color:#666;font-size:15px;">Bonjour <strong>${customerName}</strong>,<br/>
    Votre demande de retour pour le ticket <strong>${reference}</strong> a été approuvée.</p>
...
/**
 * @param {{ customerName: string, reference: string, refundAmount: number, manualRefund?: boolean, creditNoteAttached?: boolean }} params
 */
export function returnCompletedEmail({
  ...
  refundPending = false,
  manualRefund = false,
  creditNoteAttached = false,
}) {
  const retCompletedSubject = `Retour finalisé – Ticket ${reference} – Meri Beauty`;

  // A credit note only exists when the order was invoiced, which for a
  // particulier it never is. Promising an attachment that is not on the
  // message is the same failure as a screen describing a refund the app does
  // not send — so the sentence is only written when the file is really there.
  const creditNoteLine = creditNoteAttached
    ? "Vous trouverez la note de crédit correspondante en pièce jointe. "
    : "";

  // This app does not move money — an ONLINE refund is only ever queued at
  // this point, for an admin to complete by hand in Stripe. `manualRefund`
  // is the one case where the money has genuinely already moved, because
  // the cash/card hand-over at the counter is confirmed synchronously,
  // before this e-mail is ever sent.
  // The third branch is reached only when the refund was NOT queued —
  // `refundPending` is `!manualRefund && refundQueued`, so falling through
  // both means an online return whose refund never made it into the ledger.
  // completeReturnRequest treats that same condition as an inconsistency and
  // e-mails the salon "⚠️ Remboursement à vérifier", with a comment saying the
  // alert exists rather than "silently telling the customer it's handled".
  // This line was telling the customer exactly that: money would "appear in
  // your account in a few days" when nothing was scheduled to pay it and
  // nobody had been assigned to. It now says what is true — a person has to
  // finish this — and gives the customer somewhere to push if they don't hear
  // back, which is the only thing that makes an unqueued refund recoverable
  // from their side.
  const refundNoteText = refundPending
    ...
    : manualRefund
      ...
      : "Notre équipe organise votre remboursement et vous recontactera. Sans nouvelle de notre part sous quelques jours, répondez à cet e-mail.";

  const retCompletedText = `
Bonjour ${customerName},

Nous avons bien reçu et vérifié votre retour pour le ticket ${reference}.

Montant remboursé : €${Number(refundAmount).toFixed(2)}
${creditNoteLine}${refundNoteText}
...
L'équipe Meri Beauty
  ...
  const retCompletedHtml = htmlWrapper(
    ...
    <p style="margin:0 0 24px;color:#666;font-size:15px;">Bonjour <strong>${customerName}</strong>,<br/>
    Nous avons bien reçu et vérifié votre retour pour le ticket <strong>${reference}</strong>.</p>
 ...
 *
 * @param {{ customerName: string, reference: string, reason: string, decidedAt: Date }} params
 */
export function returnRejectedEmail({ customerName, reference, reason, decidedAt }) {
  const retRejectedSubject = `Retour refusé – Ticket ${reference} – Meri Beauty`;
...
Votre demande de retour pour le ticket ${reference} a été refusée le ${decidedAtLabel}.
...
L'équipe Meri Beauty
  ...
  const retRejectedHtml = htmlWrapper(
    ...
    <p style="margin:0 0 24px;color:#666;font-size:15px;">Bonjour <strong>${escapeHtml(customerName)}</strong>,<br/>
    Votre demande de retour pour le ticket <strong>${reference}</strong> a été refusée le ${decidedAtLabel}.</p>
...
/**
 * Formation equivalent of workshopSessionChangeEmail — simpler, since
 * formations have no seat-change-fee legacy path: this is always a direct,
 * free transfer, never the "fee paid" variant the workshop version also
 * renders.
 */
export function formationSessionChangeEmail({
  customerName,
  previousFormationTitle,
  newFormationTitle,
  previousSessionDate,
  newSessionDate,
  totalPrice,
  ...
  const subject = `Votre réservation pour "${newFormationTitle}" a été modifiée – Meri Beauty`;
...
Votre réservation a bien été déplacée par l'équipe Meri Beauty.
  ...
  Formation précédente : ${previousFormationTitle}
  Ancienne séance : ${previousSessionDate}
  Nouvelle formation : ${newFormationTitle}
  Nouvelle séance  : ${newSessionDate}
...
L'équipe Meri Beauty
  ...
  const html = htmlWrapper(
    ...
    <table cellpadding="0" cellspacing="0" style="width:100%;background:#f9f8f5;border-radius:8px;padding:4px 20px;margin-bottom:24px;">
      <tbody>
        ${infoRow("Formation précédente", previousFormationTitle)}
        ${infoRow("Ancienne séance", previousSessionDate)}
        ${infoRow("Nouvelle formation", newFormationTitle)}
        ${infoRow("Nouvelle séance", newSessionDate)}
        ${infoRow("Nouveau total", `€${Number(totalPrice).toFixed(2)}`)}
        ${infoRow("Déjà payé", `€${Number(paidAmount).toFixed(2)}`)}
        ${infoRow("Solde restant", `€${Number(balanceDue).toFixed(2)}`)}
...
/**
 * Shared body for a free counter seat-count change — see
 * lib/reservations/change-reservation-seats.js. Unlike
 * workshopSessionChangeEmail/formationSessionChangeEmail (which move a
 * booking to a different session, possibly for a fee), this only ever
 * changes the headcount on the *same* session and is always free — there is
 * no legacy fee-path variant to render, so one shared body covers both
 * ateliers/événements and formations; workshopSeatsChangedEmail and
 * formationSeatsChangedEmail below just name it for their own activityLabel.
 */
function reservationSeatsChangedEmail({
  customerName,
  activityTitle,
  sessionDate,
  previousSeats,
  ...
  const subject = `Votre réservation pour "${activityTitle}" a été modifiée – Meri Beauty`;

  const text = `
Bonjour ${customerName},

Le nombre de places de votre réservation pour « ${activityTitle} » a été modifié par l'équipe Meri Beauty.

  Séance : ${sessionDate}
  Places : ${previousSeats} → ${newSeats}
...
L'équipe Meri Beauty
  ...
  const html = htmlWrapper(
    subject,
    `
    <h2 style="margin:0 0 8px;font-size:22px;color:${darkColor};">Nombre de places modifié ✓</h2>
    <p style="margin:0 0 24px;color:#666;font-size:15px;">
      Bonjour <strong>${customerName}</strong>,<br/>
      Le nombre de places de votre réservation pour « ${activityTitle} » a été modifié par l'équipe Meri Beauty.
    </p>
    ...
    <table cellpadding="0" cellspacing="0" style="width:100%;background:#f9f8f5;border-radius:8px;padding:4px 20px;margin-bottom:24px;">
      <tbody>
        ${infoRow("Séance", sessionDate)}
        ${infoRow("Places", `${previousSeats} → ${newSeats}`)}
        ${infoRow("Nouveau total", `€${Number(totalPrice).toFixed(2)}`)}
        ${infoRow("Déjà payé", `€${Number(paidAmount).toFixed(2)}`)}
        ${infoRow("Solde restant", `€${Number(balanceDue).toFixed(2)}`)}
      </tbody>
...
export function workshopSeatsChangedEmail(params) {
  return reservationSeatsChangedEmail(params);
}

export function formationSeatsChangedEmail(params) {
  return reservationSeatsChangedEmail(params);
}
 ...
 */
export function formationCancellationEmail({
  customerName,
  formationTitle,
  sessionDate,
  salonPhone,
  ...
  // The default wording is "l'acompte n'est pas remboursable", which is the
  // standing policy — but it was hard-coded, so an admin *approving* an
  // exceptional refund (medical certificate, bereavement) sent the customer
  // a message stating the exact opposite of the decision just taken, while
  // the money was queued for refund on /dashboard/operations. Mirrors
  // workshopCancellationEmail, which already had these three states.
  const depositNote = refunded
    ? "À titre exceptionnel, l'acompte versé vous a été remboursé."
    : refundPending
      ? `Un remboursement${formattedAmount ? ` de ${formattedAmount}` : ""} a été approuvé à titre exceptionnel et vous sera envoyé sous peu. Vous recevrez une confirmation dès qu'il aura été effectué.`
  ...
  // The admin's written reply to an exceptional request. The rejection path
  // sends it; the approval path dropped it — so a customer who was refused
  // heard why, and one who was granted heard nothing at all.
  const teamMessage = decisionNote?.trim() ? `Message de l'équipe : ${decisionNote.trim()}` : null;
  const positive = refunded || refundPending;
...
${depositNote}
${teamMessage ? `\n${teamMessage}\n` : ""}
Pour toute question, n'hésitez pas à nous contacter directement${contactLine ? ` (${contactLine})` : ""}.
...
L'équipe Meri Beauty
  ...
  const html = htmlWrapper(
    ...
    <p style="margin:0;font-size:14px;color:#666;line-height:1.6;">Pour toute question, n'hésitez pas à nous contacter directement${salonPhone || salonEmail ? ` (${formatContactLineHtml(salonPhone, salonEmail, "#666")})` : ""}.</p>
    ${divider()}
    <p style="margin:20px 0 0;font-size:14px;color:#888;">L'équipe Meri Beauty</p>
    `
  );
...
/**
 * Sent when an invoice is (re-)dispatched to the customer by email.
 *
 * @param {{
 *   customerName: string,
...
export function invoiceEmail({
  ...
  vatAmount,
  totalInclVat,
  sellerName = "Meri Beauty",
}) {
  const subject = `Votre facture ${invoiceNumber} — ${sellerName}`;
  ...
  const linesText = lines
    .map((l) => `  ${l.description.padEnd(40)} x${l.quantity}  €${Number(l.lineTotal).toFixed(2)}`)
    .join("\n");

  const text = `
Bonjour ${customerName},

Une nouvelle facture a été émise par l'équipe ${sellerName}. Voici les détails ci-dessous.

  Numéro de facture : ${invoiceNumber}
  Montant Total     : €${Number(totalInclVat).toFixed(2)}${dueLabel ? `\n  Date d'échéance   : ${dueLabel}` : ""}
${linesText ? `\n  Détail :\n${linesText}\n` : ""}
  Sous-total HT  : €${Number(subtotalExclVat).toFixed(2)}
  TVA (${Number(vatRate).toFixed(0)}%)     : €${Number(vatAmount).toFixed(2)}
  Total TTC      : €${Number(totalInclVat).toFixed(2)}

Vous trouverez la facture détaillée en pièce jointe (format PDF).
...
L'équipe ${sellerName}
  ...
  // ── HTML line-items rows ────────────────────────────────────────────────────
  const lineRows = lines
    .map((l, i) => {
      const isEven = i % 2 === 0;
      return `
      <tr>
        <td style="padding:14px 16px;font-size:14px;color:#2F3A2E;border-bottom:1px solid #f0ede8;line-height:1.5;${isEven ? "background:#fdfcfa;" : "background:#ffffff;"}">${escapeHtml(l.description)}</td>
        <td style="padding:14px 16px;font-size:14px;color:#6b7280;text-align:center;border-bottom:1px solid #f0ede8;white-space:nowrap;${isEven ? "background:#fdfcfa;" : "background:#ffffff;"}">${l.quantity}</td>
        <td style="padding:14px 16px;font-size:14px;color:#6b7280;text-align:right;border-bottom:1px solid #f0ede8;white-space:nowrap;${isEven ? "background:#fdfcfa;" : "background:#ffffff;"}">€${Number(l.unitPrice).toFixed(2)}</td>
        <td style="padding:14px 16px;font-size:14px;font-weight:600;color:#2F3A2E;text-align:right;border-bottom:1px solid #f0ede8;white-space:nowrap;${isEven ? "background:#fdfcfa;" : "background:#ffffff;"}">€${Number(l.lineTotal).toFixed(2)}</td>
      </tr>`;
    })
    .join("");
  ...
  const html = htmlWrapper(
    ...
    `
    <!-- ── Greeting banner ── -->
    <div style="background:linear-gradient(135deg,#fdf8ee 0%,#f9f5ed 100%);border-radius:10px;padding:24px 28px;margin-bottom:28px;">
      <p style="margin:0 0 6px;font-size:16px;color:#2F3A2E;">
        Bonjour <strong>${escapeHtml(customerName)}</strong>,
      </p>
      <p style="margin:0;font-size:14px;color:#6b7280;line-height:1.6;">
        Votre facture a été émise. Vous la trouverez en pièce jointe (PDF).
      </p>
    </div>

    <!-- ── Invoice summary card ── -->
    <table cellpadding="0" cellspacing="0" width="100%"
           style="border:1px solid #e8e5df;border-radius:10px;overflow:hidden;margin-bottom:28px;">
      <tr>
        <td style="background:#2F3A2E;padding:14px 24px;">
          <table cellpadding="0" cellspacing="0" width="100%">
            <tr>
              <td style="font-size:12px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#e9e3d7;">Facture</td>
              <td style="text-align:right;font-size:15px;font-weight:700;color:#ffffff;">${escapeHtml(invoiceNumber)}</td>
            </tr>
          </table>
        </td>
      </tr>
          ...
          <table cellpadding="0" cellspacing="0" width="100%">
            <tr>
              <td style="padding:0 0 12px;">
                <table cellpadding="0" cellspacing="0" width="100%">
                  <tr>
                    <td style="font-size:13px;color:#9ca3af;">Montant Total</td>
                    <td style="text-align:right;font-size:20px;font-weight:700;color:#2F3A2E;">€${Number(totalInclVat).toFixed(2)}</td>
                  </tr>
                </table>
              </td>
            </tr>
            ...
            <tr>
              <td style="padding:0 0 12px;">
                <table cellpadding="0" cellspacing="0" width="100%">
                  <tr>
                    <td style="font-size:13px;color:#9ca3af;">Date d'échéance</td>
                    <td style="text-align:right;font-size:14px;font-weight:600;color:#2F3A2E;">${dueLabel}</td>
                  </tr>
                </table>
              </td>
            </tr>` : ""}
            ...
            <tr>
              <td>
                <table cellpadding="0" cellspacing="0" width="100%">
                  <tr>
                    <td style="font-size:13px;color:#9ca3af;">Date d'émission</td>
                    <td style="text-align:right;font-size:14px;color:#6b7280;">${issuedLabel}</td>
                  </tr>
                </table>
              </td>
            </tr>` : ""}
    ...
    ${lines.length > 0 ? `
    <table cellpadding="0" cellspacing="0" width="100%"
           style="border:1px solid #e8e5df;border-radius:10px;overflow:hidden;margin-bottom:4px;">
      <thead>
        <tr style="background:#2F3A2E;">
          <th style="padding:12px 16px;font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#e9e3d7;text-align:left;">Description</th>
          <th style="padding:12px 16px;font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#e9e3d7;text-align:center;">Qté</th>
          <th style="padding:12px 16px;font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#e9e3d7;text-align:right;">Prix unit.</th>
          <th style="padding:12px 16px;font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#e9e3d7;text-align:right;">Total HT</th>
        </tr>
      </thead>
    ...
    <table cellpadding="0" cellspacing="0" width="100%"
      ...
      <tr>
        <td style="padding:10px 16px 10px 0;font-size:13px;color:#9ca3af;">Sous-total HT</td>
        <td style="padding:10px 0;font-size:13px;color:#4b5563;text-align:right;white-space:nowrap;">€${Number(subtotalExclVat).toFixed(2)}</td>
      </tr>
      <tr>
        <td style="padding:10px 16px 10px 0;font-size:13px;color:#9ca3af;">TVA (${Number(vatRate).toFixed(0)}%)</td>
        <td style="padding:10px 0;font-size:13px;color:#4b5563;text-align:right;white-space:nowrap;">€${Number(vatAmount).toFixed(2)}</td>
      </tr>
      <tr>
        <td colspan="2" style="padding:6px 0 0;">
          <div style="height:1px;background:linear-gradient(90deg,#e8e5df 0%,#C8A46A 50%,#e8e5df 100%);border-radius:1px;"></div>
        </td>
      </tr>
      <tr>
        <td style="padding:14px 16px 0 0;font-size:16px;font-weight:700;color:#2F3A2E;">Total TTC</td>
        <td style="padding:14px 0 0;font-size:20px;font-weight:700;color:#C8A46A;text-align:right;white-space:nowrap;">€${Number(totalInclVat).toFixed(2)}</td>
      </tr>
    </table>

    <!-- ── Help text ── -->
    <p style="margin:28px 0 0;font-size:13px;color:#9ca3af;line-height:1.6;text-align:center;">
      Questions concernant cette facture ?<br/>
      <a href="mailto:${escapeHtml(sellerName.toLowerCase().replace(/\s+/g, ""))}@example.com" style="color:#C8A46A;text-decoration:none;font-weight:600;">Contactez-nous</a>
    </p>
    `
  );
...
// ─── Ticket (till receipt) email ───────────────────────────────────────────────

/**
 * Sent only when a staff member passing canSendTicketEmail() (an admin, or
 * the one hardcoded ticket-email operator — see lib/authorization.js)
 * deliberately e-mails a client their till-style ticket
 * (actions/payments/send-ticket-email.js) — never automatically; see the
 * AUDIT_ACTIONS.TICKET_EMAILED comment in lib/audit-log.js.
 *
 * @param {{
 *   customerName: string,
 *   ticketNumber: string,
...
/**
 * The manual counterpart to ticketEmail, for a client who never got (or
 * lost) the check-in QR from their original confirmation e-mail — sent from
 * actions/payments/send-checkin-email.js, gated the same way.
 *
 * Deliberately its own, minimal template rather than replaying the original
 * reservationConfirmedEmail/workshopReservationConfirmationEmail/
 * formationReservationConfirmationEmail: those carry pricing and payment
 * details that may have changed since, and a staff member resending "your
 * entry ticket" should not also restate a possibly-stale amount.
 *
 * @param {{ customerName: string, activityTitle: string, sessionDate?: string|null, checkInCode: string }} params
 * @returns {{ subject: string, text: string, html: string }}
 */
export function checkInReminderEmail({ customerName, activityTitle, sessionDate = null, checkInCode }) {
  const subject = `Votre billet d'entrée — ${activityTitle} – Meri Beauty`;

  const text = `
Bonjour ${customerName},

Voici votre billet d'entrée pour ${activityTitle}${sessionDate ? ` du ${sessionDate}` : ""}.

  Votre billet d'entrée : ${checkInCode}

Le QR code correspondant est joint à cet e-mail. Présentez-le à l'entrée — ou donnez simplement le code ci-dessus.
...
L'équipe Meri Beauty
  ...
  const html = htmlWrapper(
    ...
    <p style="margin:0 0 5px;font-size:15px;color:#444;">
      Bonjour <strong style="color:${darkColor};">${escapeHtml(customerName)}</strong>,
    </p>
    <p style="margin:0 0 24px;font-size:15px;color:#555;line-height:1.7;">
      Voici votre billet d'entrée pour <strong>${escapeHtml(activityTitle)}</strong>${sessionDate ? ` du ${escapeHtml(sessionDate)}` : ""}.
    </p>
    ...
    <p style="margin:0;font-size:14px;color:#888;">
      À bientôt,<br/>
      <strong style="color:${darkColor};">L'équipe Meri Beauty</strong>
    </p>
    `
  );
...
/**
 * A plain payment confirmation: the client's payment went through and the
 * transaction is complete. That is all it says.
 *
 * Sent instead of ticketEmail when a practitioner other than the salon's own
 * accounts settles a booking — the salon issues no ticket for those
 * (lib/tickets/allocate-ticket-number.js). Deliberately silent about tickets,
 * VAT and who took the payment: the client only needs to know it is done.
 * Sent by lib/payments/send-settlement-email.js.
 *
 * @param {{ customerName: string, description: string, amount: number, paidAt: Date|string }} params
 * @returns {{ subject: string, text: string, html: string }}
 */
export function paymentConfirmedEmail({ customerName, description, amount, paidAt }) {
  const subject = "Confirmation de votre paiement – Meri Beauty";
...
Votre paiement de ${amountLabel} pour ${description} a bien été reçu le ${paidLabel}. Tout est en ordre.
...
L'équipe Meri Beauty
  ...
  const html = htmlWrapper(
    ...
    `
    <p style="margin:0 0 5px;font-size:15px;color:#444;">
      Bonjour <strong style="color:#2F3A2E;">${escapeHtml(customerName)}</strong>,
    </p>
    <p style="margin:0 0 24px;font-size:15px;color:#555;line-height:1.7;">
      Votre paiement de <strong>${amountLabel}</strong> pour <strong>${escapeHtml(description)}</strong> a bien été reçu le ${paidLabel}. Tout est en ordre.
    </p>
    <p style="margin:0;font-size:14px;color:#888;">
      Merci pour votre confiance et à bientôt,<br/>
      <strong style="color:#2F3A2E;">L'équipe Meri Beauty</strong>
    </p>
    `
  );
...
// ─── Review thank-you ──────────────────────────────────────────────────────
// Sent once, right after a customer submits a review on a completed
// rendez-vous / atelier / événement / formation — see actions/review/review-actions.js.

export function reviewThankYouEmail({ customerName, serviceName, rating }) {
  const subject = "Merci pour votre avis – Meri Beauty";
  const stars = "★".repeat(rating) + "☆".repeat(5 - rating);

  const text = `
Bonjour ${customerName},

Merci d'avoir pris le temps de laisser un avis (${stars}) sur ${serviceName}. Votre retour compte énormément pour nous et nous aide à faire progresser le salon.
...
L'équipe Meri Beauty
  ...
  const html = htmlWrapper(
    ...
    <p style="margin:0 0 16px;font-size:15px;color:#555;line-height:1.7;">
      Merci d'avoir pris le temps de laisser un avis sur <strong>${escapeHtml(serviceName)}</strong>. Votre retour compte énormément pour nous et nous aide à faire progresser le salon.
    </p>
    <p style="margin:0 0 24px;font-size:22px;letter-spacing:2px;color:${brandColor};">
      ${stars}
    </p>
    <p style="margin:0;font-size:14px;color:#888;">
      À très bientôt,<br/>
...
export function ticketEmail({
  ...
  const subject = `Votre ticket ${ticketNumber} — ${sellerName}`;
  const issuedLabel = formatDate(issuedAt);

  const linesText = lines
    .map((l) => `  ${l.description.padEnd(40)} x${l.quantity}  €${Number(l.unitPrice).toFixed(2)}`)
    .join("\n");

  const text = `
Bonjour ${customerName},

Voici votre ticket de caisse pour votre passage du ${issuedLabel} chez ${sellerName}.
...
Vous trouverez le ticket détaillé en pièce jointe (format PDF).
...
L'équipe ${sellerName}
  ...
  const lineRows = lines
    .map((l) => `
      <tr>
        <td style="padding:11px 14px;font-size:14px;color:#2F3A2E;border-bottom:1px solid #f0ede8;line-height:1.4;">${escapeHtml(l.description)}</td>
        <td style="padding:11px 14px;font-size:14px;color:#777;text-align:center;border-bottom:1px solid #f0ede8;white-space:nowrap;">${l.quantity}</td>
        <td style="padding:11px 14px;font-size:14px;font-weight:600;color:#2F3A2E;text-align:right;border-bottom:1px solid #f0ede8;white-space:nowrap;">€${Number(l.unitPrice).toFixed(2)}</td>
      </tr>`)
    .join("");
  ...
  const html = htmlWrapper(
    ...
    <p style="margin:0 0 16px;font-size:15px;color:#555;line-height:1.7;">
      Voici votre ticket de caisse pour votre passage du ${issuedLabel} chez <strong>${escapeHtml(sellerName)}</strong>.
    </p>
    <p style="margin:0 0 24px;font-size:14px;color:#666;line-height:1.6;">
      Vous trouverez le ticket détaillé en pièce jointe (format PDF).
    </p>
    <p style="margin:0 0 28px;font-size:14px;color:#888;">
      À bientôt,<br/>
      <strong style="color:#2F3A2E;">L'équipe ${escapeHtml(sellerName)}</strong>
    </p>

    ${divider()}

    <table cellpadding="0" cellspacing="0" width="100%"
           style="background:#fdf8ee;border:1px solid ${brandColor};border-radius:8px;margin-bottom:28px;">
      <tr>
        <td style="padding:16px 20px;line-height:2;">
          <p style="margin:0;font-size:14px;color:#2F3A2E;">
            <strong>Numéro de ticket :</strong>&nbsp;${escapeHtml(ticketNumber)}
          </p>
          <p style="margin:0;font-size:14px;color:#2F3A2E;">
            <strong>Montant Total :</strong>&nbsp;€${Number(totalInclVat).toFixed(2)}
          </p>
        </td>
      </tr>
    </table>
    ...
    ${lines.length > 0 ? `
    <table cellpadding="0" cellspacing="0" width="100%"
           style="border:1px solid #eeede9;border-radius:8px;overflow:hidden;margin-bottom:4px;">
      <thead>
        <tr style="background:#2F3A2E;">
          <th style="padding:10px 14px;font-size:11px;font-weight:700;letter-spacing:.07em;text-transform:uppercase;color:#e9e3d7;text-align:left;">Description</th>
          <th style="padding:10px 14px;font-size:11px;font-weight:700;letter-spacing:.07em;text-transform:uppercase;color:#e9e3d7;text-align:center;">Qté</th>
          <th style="padding:10px 14px;font-size:11px;font-weight:700;letter-spacing:.07em;text-transform:uppercase;color:#e9e3d7;text-align:right;">Total</th>
        </tr>
      </thead>
      <tbody>${lineRows}</tbody>
    </table>
    ...
    <table cellpadding="0" cellspacing="0" style="margin-left:auto;margin-bottom:0;min-width:260px;">
      <tr>
        <td style="padding:6px 16px 6px 0;font-size:13px;color:#888;">Sous-total HT</td>
        <td style="padding:6px 0;font-size:13px;color:#444;text-align:right;white-space:nowrap;">€${Number(subtotalExclVat).toFixed(2)}</td>
      </tr>
      <tr>
        <td style="padding:6px 16px 6px 0;font-size:13px;color:#888;">TVA (${Number(vatRate).toFixed(0)}%)</td>
        <td style="padding:6px 0;font-size:13px;color:#444;text-align:right;white-space:nowrap;">€${Number(vatAmount).toFixed(2)}</td>
      </tr>
      <tr>
        <td colspan="2" style="padding:8px 0 0;">
          <div style="height:2px;background:#eeede9;border-radius:1px;"></div>
        </td>
      </tr>
      <tr>
        <td style="padding:10px 16px 0 0;font-size:15px;font-weight:700;color:#2F3A2E;">Total TTC</td>
        <td style="padding:10px 0 0;font-size:17px;font-weight:700;color:${brandColor};text-align:right;white-space:nowrap;">€${Number(totalInclVat).toFixed(2)}</td>
      </tr>
    </table>
    `
  );
...
/**
 * Sent when a credit note is (re-)dispatched to the customer by email.
 *
 * @param {{
 *   customerName: string,
...
export function creditNoteEmail({
  ...
  const subject = `Votre note de crédit ${creditNoteNumber} — ${sellerName}`;

  const issuedLabel = new Date(issuedAt).toLocaleDateString("fr-FR", {
    day: "2-digit",
    month: "long",
    year: "numeric",
    timeZone: "Europe/Brussels",
...
Une note de crédit a été émise par l'équipe ${sellerName} en lien avec la facture ${invoiceNumber}. Vous trouverez le document détaillé en pièce jointe (format PDF).

  Numéro de note de crédit : ${creditNoteNumber}
  Facture concernée         : ${invoiceNumber}
  Date d'émission           : ${issuedLabel}
  Montant remboursé         : €${Number(totalInclVat).toFixed(2)}
...
L'équipe ${sellerName}
  ...
  const html = htmlWrapper(
    subject,
    `
    <p style="margin:0 0 6px;font-size:15px;color:#444;">Bonjour <strong style="color:#2F3A2E;">${escapeHtml(customerName)}</strong>,</p>
    <p style="margin:0 0 28px;font-size:15px;color:#555;line-height:1.6;">
      Une note de crédit a été émise par l'équipe <strong>${escapeHtml(sellerName)}</strong> en lien avec la facture <strong>${escapeHtml(invoiceNumber)}</strong>. Vous trouverez le document détaillé en pièce jointe.
    </p>

    <!-- credit note meta card -->
    <table cellpadding="0" cellspacing="0" width="100%"
           style="background:#f9f8f5;border-radius:10px;margin-bottom:28px;overflow:hidden;">
      <tr>
        <td style="padding:20px 24px;">
          <table cellpadding="0" cellspacing="0" width="100%">
            <tr>
              <td style="padding:4px 0;">
                <span style="font-size:12px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#999;">Note de crédit</span><br/>
                <span style="font-size:18px;font-weight:700;color:#2F3A2E;">${escapeHtml(creditNoteNumber)}</span>
              </td>
              <td style="padding:4px 0;text-align:right;">
                <span style="font-size:12px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#999;">Montant remboursé</span><br/>
                <span style="font-size:22px;font-weight:700;color:#2a7a4b;">€${Number(totalInclVat).toFixed(2)}</span>
              </td>
            </tr>
            <tr>
                ...
                <table cellpadding="0" cellspacing="0">
                  <tr>
                    <td style="padding-right:32px;">
                      <span style="font-size:12px;color:#999;">Date d'émission</span><br/>
                      <span style="font-size:14px;font-weight:600;color:#2F3A2E;">${issuedLabel}</span>
                    </td>
                    <td>
                      <span style="font-size:12px;color:#999;">Facture concernée</span><br/>
                      <span style="font-size:14px;font-weight:600;color:#2F3A2E;">${escapeHtml(invoiceNumber)}</span>
                    </td>
                  </tr>
                </table>
              </td>
    ...
    <!-- PDF note -->
    <div style="background:#f0ede8;border-radius:8px;padding:14px 18px;margin-bottom:28px;">
      <p style="margin:0;font-size:13px;color:#666;line-height:1.6;">
        📎 <strong>Note de crédit en pièce jointe</strong> — Vous trouverez le document complet au format PDF en pièce jointe.
      </p>
    </div>
    ...
    <p style="margin:20px 0 0;font-size:14px;color:#888;">À bientôt,<br/><strong style="color:#2F3A2E;">L'équipe ${escapeHtml(sellerName)}</strong></p>
    `
  );
...
Une nouvelle réservation a été créée :
  ...
  Client    : ${customerName}
  Date      : ${formatDate(date)} à ${time}
  ${duration ? `Durée     : ${formatDurationShort(duration)}` : ""}
  ...
  const html = htmlWrapper(
    ...
    <table cellpadding="0" cellspacing="0" style="width:100%;background:#f9f8f5;border-radius:8px;padding:4px 20px;margin-bottom:24px;">
      <tbody>
        ...
        ${infoRow("Date", formatDate(date))}
        ${infoRow("Heure", time)}
        ${duration ? infoRow("Durée", formatDurationShort(duration)) : ""}
...
export function staffMultipleReservationsConfirmedEmail({
  ...
  const rows = appointments.map((appointment, index) => `
    ...
    ${appointment.duration ? infoRow("Durée", formatDurationShort(appointment.duration)) : ""}
...
${appointments.map((appointment, index) => `Rendez-vous ${index + 1}
  ...
  Durée   : ${formatDurationShort(appointment.duration) || "—"}
