...
function formatRentalDayMonthYear(date) {
...
function formatRentalMonthYear(date) {
  return new Date(date).toLocaleDateString("fr-FR", {
    month: "long",
    year: "numeric",
    timeZone: "Europe/Brussels",
...
/**
 * Cleans a freeform rental-type label — never lets "undefined"/"null"/""
 * leak onto a printed document.
 */
function cleanRentalTypeLabel(value) {
...
/**
 * Rental period wording for staff contract invoices, from contract dates.
 * With an end date: "du 01/09/2026 au 30/09/2026". Open-ended (monthly
 * rent): "septembre 2026".
 */
export function buildRentalPeriodLabel({ startDate, endDate }) {
  if (!startDate) return null;
  if (endDate) return `du ${formatRentalDayMonthYear(startDate)} au ${formatRentalDayMonthYear(endDate)}`;
  return formatRentalMonthYear(startDate);
}

/**
 * Line-item description for staff contract invoices — always built from
 * real contract data, never from Contract.rentalType (which does not exist
 * on the Contract model; the cabin type lives on RentalRequest).
 */
export function buildRentalDescription({ startDate, endDate, rentalType }) {
  const clean = cleanRentalTypeLabel(rentalType);
  const space = clean ? clean.charAt(0).toLowerCase() + clean.slice(1) : "cabine professionnelle";
  const period = buildRentalPeriodLabel({ startDate, endDate });
  return period ? `Location d'une ${space} — ${period}` : `Location d'une ${space}`;
}

/** Short stable contract reference for display (last 6 id chars). */
export function shortContractRef(contractId) {
  const text = contractId?.toString() ?? "";
  return text ? text.slice(-6).toUpperCase() : null;
}
 ...
 *
 * A counter price adjustment gets its own line too, for the same reason. The
 * unit price is reconstructed by working *backwards* from the amount actually
 * charged — undoing the discount and the adjustment — so the lines always sum
 * to the total. Without the adjustment term, a booking that carried a promo
 * code and then had its price changed at the till printed a first line at a
 * price nobody was ever quoted: the total and the VAT were right, but the
 * document explained them with a number that never existed. `adjustmentAmount`
 * is signed — negative for a gesture, positive for a correction upwards.
 *
 * @param {{ description: string, totalAmount: number, discountAmount?: number, adjustmentAmount?: number, adjustmentReason?: string|null }} input
 */
export function buildServiceInvoiceLines({
  description,
  totalAmount,
  discountAmount = 0,
  adjustmentAmount = 0,
  ...
  const lines = [
    { description, quantity: 1, unitPrice: round2(Number(totalAmount) + discount - adjustment) },
  ];
  if (discount > 0) {
  ...
  if (adjustment !== 0) {
    ...
    lines.push({
      description: reason ? `Ajustement — ${reason}` : "Ajustement au comptoir",
      quantity: 1,
...
/**
 * The number the next invoice would get, read without taking it — for a
 * preview or a "send this invoice" card shown before issuing. Nothing is
 * reserved: another invoice issued in between takes it, and issueInvoice()
 * then gives the following one. `ahead` = invoices due to be issued before
 * this one (older rents), so each draft shows its own successive number.
 */
export async function peekNextInvoiceNumber(client, ahead = 0) {
  const year = Number(
    new Intl.DateTimeFormat("en", { timeZone: "Europe/Brussels", year: "numeric" }).format(new Date())
  );
  const counter = await client.numberingCounter.findUnique({ where: { key: `invoice-${year}` }, select: { lastNumber: true } });
  return `F-${year}-${String(Number(counter?.lastNumber ?? 0) + 1 + ahead).padStart(6, "0")}`;
}
 ...
 *   source: "ORDER" | "APPOINTMENT" | "WORKSHOP" | "FORMATION" | "STAFF_CONTRACT" | "MANUAL",
 *   totalInclVat: number,
 *   customer: {
...
export function assertBuyerLegalDataComplete(customer) {
  ...
  // No separate legal-name check: issueInvoice falls back to the buyer's
  // name when no company name is on file, so a B2B invoice (any buyer with a
  // VAT number — a sole trader included) can never lack one.
...
/**
 * Everything issueInvoice does except numbering and saving: every guard (a
 * refused invoice is refused here, before any number exists) and every
 * figure. The Factures preview renders exactly this, so what is shown before
 * « Accepter » is what accepting will issue — or fails where it would fail.
 *
 * Reads only; never writes and never touches the numbering counter.
 */
export async function prepareInvoice(tx, {
  paymentId = null,
  contractId = null,
  // Set only by supersedeInvoice's caller, when this invoice replaces one
  // voided by a session transfer — see model Invoice's supersedesInvoiceId.
  supersedesInvoiceId = null,
  // Set by the monthly-billing engine. Monthly staff invoices are not
  // anchored to a contractId (the same Contract row recurs every month and
  // Invoice.contractId is unique) — their idempotency is owned by the
  // StaffMonthlyInvoice row created in the same transaction before this
  // call. Passing monthlyInvoiceId tells issueInvoice() that the third
  // anchor path is active and skips the paymentId/contractId XOR guard.
  ...
  taxNote = null,
  dueDate = null,
  // Free-text comment printed on the PDF — only a manual invoice sets it.
  notes = null,
}) {
  ...
  // Idempotency: exactly one of paymentId, contractId, or monthlyInvoiceId
  // must be set. monthlyInvoiceId is only valid for STAFF_CONTRACT source
  // (the StaffMonthlyInvoice row already owns the dedup for that path).
  const hasPayment = Boolean(paymentId);
  const hasContract = Boolean(contractId);
  const hasMonthly = Boolean(monthlyInvoiceId);
  const anchorCount = [hasPayment, hasContract, hasMonthly].filter(Boolean).length;
  if (anchorCount !== 1) {
    throw new Error("INVOICE_REQUIRES_PAYMENT_OR_CONTRACT");
  }
  if (hasMonthly && source !== "STAFF_CONTRACT") {
    throw new Error("INVOICE_REQUIRES_PAYMENT_OR_CONTRACT");
  }

  // An independent practitioner's sale (Payment.payeeStaffId, see
  // lib/payments/resolve-payee.js) is documented under her own VAT number,
  // never the salon's. Callers skip issueInvoice for it; this guard makes
  // sure a missed call site fails loudly instead of emitting a salon invoice.
  if (hasPayment) {
    const owner = await tx.payment.findUnique({ where: { id: paymentId }, select: { payeeStaffId: true } });
    if (owner?.payeeStaffId) throw new Error("INDEPENDENT_SALE_NO_SALON_INVOICE");
  }
  ...
  // An invoice is issued to a VAT-registered buyer, so it is B2B. A sale
  // invoice already requires a VIES-validated number (B2C_INVOICE_NOT_ALLOWED
  // above) and a staff contract is addressed to an independent with her own
  // VAT number. This used to require a BillingProfile company name as well,
  // which filed a staff member's rent invoice as B2C despite its VAT number.
  // The legal name falls back to the buyer's name, so a B2B document can
  // still never lack one.
  const isB2B = Boolean(customer.vatNumber?.toString().trim());
  const legalName = customer.legalName || customer.fullName;
  ...
  return {
    ...
    lines: allocateNetLines(lines, vatRate, totals.totalExclVat),
  };
}

export async function issueInvoice(tx, input) {
  // Guards first: a refused invoice must not consume a number from the
  // gapless legal sequence, transaction rollback or not.
  ...
  return tx.invoice.create({
    data: { number, ...draft.data, lines: { create: draft.lines } },
    include: { lines: true },
  });
}
...
/**
 * Voids an invoice in favour of a replacement — never edits or deletes the
 * original (Belgian VAT law requires the correction to be its own document).
 * Always credits the FULL original amount: this exists for "the sale moved
 * to a different session", never a partial return, so a partial credit here
 * would be the wrong instrument.
 *
 * Nulls the original's paymentId, freeing its unique slot on Payment so the
 * caller can immediately issue a replacement invoice against the same
 * paymentId inside the same transaction (pass the returned old invoice's id
 * as issueInvoice's supersedesInvoiceId to link the two).
 *
 * @param {object} tx - Prisma transaction client
 * @param {{ invoice: { id: string, totalInclVat: number|string }, reason: string }} input
 */
export async function supersedeInvoice(tx, { invoice, reason }) {
  ...
  await tx.invoice.update({
    where: { id: invoice.id },
    data: { paymentId: null, supersededAt: new Date(), supersededReason: reason },
  });
  return { creditNote };
}

/**
 * Which invoice a counter settlement should leave on file, given that the till
 * may have repriced the booking on the way through.
 *
 * Three outcomes, and the interesting one is the middle:
 *
 *  - price unchanged, invoice already on file → keep it. Nothing about the sale
 *    moved, so a credit note here would be paperwork for its own sake. This is
 *    also what keeps the balance-due path off Invoice.paymentId's unique
 *    constraint: it used to call issueInvoice unguarded, so a B2B client who
 *    had paid online (invoice already issued) and then had their price raised
 *    at the counter hit P2002 and rolled the whole settlement back behind a
 *    generic "Erreur lors de la clôture".
 *  - price changed, invoice already on file → credit the original in full and
 *    reissue at the new total, linked by supersedesInvoiceId. The stored
 *    invoice would otherwise stop matching Payment.totalAmount with no
 *    correcting document, which Belgian VAT law does not allow.
 *  - no invoice yet → issue one iff the customer's VAT identity earns one.
 *
 * `issue` is the caller's own issueInvoice call, taking the id to supersede (or
 * null); the four settlement sites differ in the VAT policy fields and line
 * descriptions they pass, so those stay local to each rather than being
 * threaded through here.
 *
 * @param {object} tx - Prisma transaction client
 * @param {{ existingInvoice: object|null, priceChanged: boolean, adjustmentReason: string|null,
 *          shouldIssue: boolean, issue: (supersedesInvoiceId: string|null) => Promise<object> }} input
 * @returns {Promise<{ invoice: object|null, creditNote: object|null }>}
 */
/**
 * @param {boolean} [canIssue] Defaults to true (today's behavior for every
 *   existing caller that doesn't pass it). Pass false for a non-privileged
 *   staff actor (see isTillCashOperator) — this is a hard no-op: nothing
 *   invoice-related is touched at all, not even superseding an existing
 *   invoice, since a voided invoice with no replacement is worse than one
 *   left stale. A non-privileged staff member simply cannot generate,
 *   correct, or replace an Invoice under any circumstance.
 */
export async function resolveSettlementInvoice(
  tx,
  { existingInvoice, priceChanged, adjustmentReason, shouldIssue, issue, canIssue = true },
) {
  if (!canIssue) return { invoice: existingInvoice ?? null, creditNote: null };

  if (existingInvoice && !priceChanged) return { invoice: existingInvoice, creditNote: null };

  if (existingInvoice) {
    const reason = adjustmentReason
      ? `Ajustement de prix au comptoir — ${adjustmentReason}`
      : "Ajustement de prix au comptoir";
    const { creditNote } = await supersedeInvoice(tx, { invoice: existingInvoice, reason });
    try {
      return { invoice: await issue(existingInvoice.id), creditNote };
    } catch (error) {
      // The original is already credited at this point. Letting a bare
      // B2C_INVOICE_NOT_ALLOWED through would abort the transaction with a
      // message about issuing an invoice, when what actually happened is that
      // the client's VAT number stopped validating since the first one — the
      // named error lets the settlement paths say so.
      if (error.message === "B2C_INVOICE_NOT_ALLOWED") throw new Error("INVOICE_REPLACEMENT_VAT_EXPIRED");
      throw error;
    }
  }

  return { invoice: shouldIssue ? await issue(null) : null, creditNote: null };
}
