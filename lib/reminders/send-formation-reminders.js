 ...
 * customer with an upcoming reservation on each call. Mirrors
 * lib/reminders/send-workshop-reminders.js exactly — kept as a separate
 * function since the two modules are deliberately independent (see
...
export async function sendFormationReservationReminders() {
  ...
  for (const reservation of reservations) {
    ...
    // Atomic claim, gated on the marker still being null — a plain update()
    // here let two job runners (the in-process interval in
