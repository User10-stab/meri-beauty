...
export function getReservationPaymentDecision({
  ...
  // Manual reservations: create PENDING + Payment when staff requires an acompte
  // (depositEnabled + BOTH + depositPct>0) or when staff is ONLINE_ONLY (full payment required).
  // Otherwise keep existing behaviour: CONFIRMED without payment.
  if (isManualReservation) {
    if (normalizedAllowedMethods === "ONLINE_ONLY") {
      return {
        ...
        paymentType: "ONLINE",
        shouldCreatePaymentRecord: true,
        appointmentStatusBeforePayment: "PENDING",
        appointmentStatusAfterPayment: "CONFIRMED",
