 ...
 *      AWAITING_PAYMENT, with a PENDING transfer Payment — NO invoice
 *   5. Advance nextInvoiceDate
 * The invoice (source STAFF_CONTRACT) is issued only when an admin accepts
 * the transfer — actions/invoices/staff-rent.js, lib/staff-rent-payment.js.
 */

import { prisma } from "@/lib/prisma";
import { buildRentalDescription } from "@/lib/invoicing";
import { createPendingRent } from "@/lib/staff-rent-payment";
...
/**
 * Builds the customer object that issueInvoice() expects for a rent invoice
 * — used when an admin accepts the rent (actions/invoices/staff-rent.js).
 */
...
async function billStaffMember(staff, contract, billingDate) {
  ...
  // ── 5. Record the rent due — NO invoice yet ───────────────────────────────
  // The rent is paid by transfer and, like every sale on the site, only
  // invoiced once paid: the StaffMonthlyInvoice row is AWAITING_PAYMENT with
  // a PENDING Payment, and the invoice is issued when an admin accepts the
  // transfer (actions/invoices/staff-rent.js). See lib/staff-rent-payment.js.
  let pending;
  try {
  ...
  // not this rent has been paid yet. Amounts, periods and dedup rules are
  // unchanged.
  ...
  console.log(`${tag} ✓ rent due recorded (row=${pending.id}, awaiting payment — no invoice yet), nextInvoiceDate=${nextDate?.toISOString().split("T")[0] ?? "NULL"}`);
  return { staffId: staff.id, status: "AWAITING_PAYMENT", emailed: false };
}
