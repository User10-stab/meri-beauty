...
| Pickup-on-site orders auto-expire and release stock after 7 days uncollected | 🟡 **Amended 2026-09-05 — timing changed, substance kept.** The 7-day auto-expiry is unchanged. The stock release is no longer part of it: the system cannot distinguish "never collected" from "collected at the counter and never recorded", and releasing on that guess put goods already in a customer's bag back on sale. Day 7 now expires the order, raises the *Retraits à vérifier* worklist (badged in the sidebar so it is not forgotten) and **asks the customer directly** whether they already collected — they are the only party who knows, and were previously never asked. Stock is then released either by a staff verdict or automatically after a further 14 days (`PICKUP_VERIFICATION_GRACE_DAYS`, `lib/orders/expire-stale-orders.js`), so the full arc is 7 + 14 = 21 days and nothing is held indefinitely. **Open with Marie: the 14-day figure.** The principle — stock cannot be held forever — is hers and is preserved; only the number is a developer's choice. |
