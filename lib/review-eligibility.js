// Same order of magnitude as the other free-text fields in this app (e.g.
// the cancellation-request reason textarea), enforced both client-side
// (textarea maxLength) and server-side (lib/validations/review.js).
const REVIEW_COMMENT_MAX_LENGTH = 1000;

/**
 * Shared server-side predicates for appointment review eligibility.
 */
export function getAppointmentReviewBlockReason(appointment, userId) {
  if (!userId) return "AUTH_REQUIRED";
  if (!appointment) return "NOT_FOUND";
  if (appointment.userId !== userId) return "FORBIDDEN";
  if (appointment.status !== "COMPLETED") return "NOT_COMPLETED";
  if (appointment.review) return "ALREADY_REVIEWED";
  return null;
}

export function canReviewAppointment(appointment, userId) {
  return getAppointmentReviewBlockReason(appointment, userId) === null;
}

export const REVIEW_ERRORS = {
  AUTH_REQUIRED: "Vous devez être connecté(e) pour laisser un avis.",
  NOT_FOUND: "Rendez-vous introuvable.",
  FORBIDDEN: "Vous ne pouvez pas laisser un avis sur ce rendez-vous.",
  NOT_COMPLETED: "Vous pourrez laisser un avis une fois le rendez-vous terminé.",
  ALREADY_REVIEWED: "Un avis a déjà été laissé pour ce rendez-vous.",
};

/**
 * Same predicate as getAppointmentReviewBlockReason, shaped for
 * WorkshopReservation / FormationReservation — both share customerId /
 * status / review, unlike Appointment which uses userId.
 */
export function getReservationReviewBlockReason(reservation, userId) {
  if (!userId) return "AUTH_REQUIRED";
  if (!reservation) return "NOT_FOUND";
  if (reservation.customerId !== userId) return "FORBIDDEN";
  if (reservation.status !== "COMPLETED") return "NOT_COMPLETED";
  if (reservation.review) return "ALREADY_REVIEWED";
  return null;
}

export const RESERVATION_REVIEW_ERRORS = {
  AUTH_REQUIRED: "Vous devez être connecté(e) pour laisser un avis.",
  NOT_FOUND: "Réservation introuvable.",
  FORBIDDEN: "Vous ne pouvez pas laisser un avis sur cette réservation.",
  NOT_COMPLETED: "Vous pourrez laisser un avis une fois la prestation terminée.",
  ALREADY_REVIEWED: "Un avis a déjà été laissé pour cette réservation.",
};

export { REVIEW_COMMENT_MAX_LENGTH };
