 ...
 * - Due date: custom date d'échéance if provided, otherwise 7 days after contract.startDate
 ...
 * - Creates a StaffMonthlyInvoice row for dedup protection against the monthly cron.
 ...
 *
 * @param {{ contract: { id: string, fixedRent: any, startDate: Date, dueDate?: Date|string|null, staffId?: string }, staff: { id: string, userId: string }, user: { fullName: string, email: string, vatNumber?: string|null, isCompany?: boolean, billingProfile?: any } }} params
...
export async function createAndSendStaffContractInvoice({ contract, user }) {
  ...
  // Due date: contract.dueDate is now a string representing days after startDate (e.g., "7")
  ...
  const initialDueDays = parseDueDays(contract.dueDate);
  if (initialDueDays != null) {
    dueDate = new Date(startDate);
    dueDate.setDate(dueDate.getDate() + initialDueDays);
    dueDateLabel = `le ${dueDate.toLocaleDateString("fr-FR", { timeZone: "Europe/Brussels" })} (${initialDueDays} jour${initialDueDays !== 1 ? "s" : ""} après le début de votre contrat le ${startDate.toLocaleDateString("fr-FR", { timeZone: "Europe/Brussels" })})`;
  } else if (contract.dueDate) {
    const legacyDate = new Date(contract.dueDate);
    if (!isNaN(legacyDate.getTime())) {
      dueDate = legacyDate;
      dueDateLabel = `le ${dueDate.toLocaleDateString("fr-FR", { timeZone: "Europe/Brussels" })}`;
    } else {
      dueDate = new Date(startDate);
      dueDate.setDate(dueDate.getDate() + 7);
      dueDateLabel = `${dueDate.toLocaleDateString("fr-FR", { timeZone: "Europe/Brussels" })} (7 jours après le début de votre contrat le ${startDate.toLocaleDateString("fr-FR", { timeZone: "Europe/Brussels" })})`;
    }
  } else {
    dueDate = new Date(startDate);
    dueDate.setDate(dueDate.getDate() + 7);
  ...
  try {
    ...
    try {
      ...
      // Use persisted dueDate from DB as fallback if caller didn't provide one
      if ((contractDueDate == null || String(contractDueDate).trim() === "") && enriched?.dueDate) {
        contractDueDate = enriched.dueDate;
        const enrichedDays = parseDueDays(contractDueDate);
        if (enrichedDays != null) {
          dueDate = new Date(startDate);
          dueDate.setDate(dueDate.getDate() + enrichedDays);
          dueDateLabel = `le ${dueDate.toLocaleDateString("fr-FR", { timeZone: "Europe/Brussels" })} (${enrichedDays} jour${enrichedDays !== 1 ? "s" : ""} après le début de votre contrat le ${startDate.toLocaleDateString("fr-FR", { timeZone: "Europe/Brussels" })})`;
        } else if (contractDueDate) {
          const legacyDate = new Date(contractDueDate);
          if (!isNaN(legacyDate.getTime())) {
            dueDate = legacyDate;
            dueDateLabel = `le ${dueDate.toLocaleDateString("fr-FR", { timeZone: "Europe/Brussels" })}`;
          }
      ...
      // Capture staffId for nextInvoiceDate initialization
      if (enriched?.staffId && !contract.staffId) {
        contract.staffId = enriched.staffId;
      }
    } catch (err) {
  ...
  // Calculate nextInvoiceDate = startDate + 1 month (month-end aware)
  const nextInvoiceDate = calculateNextInvoiceDate(startDate);

  // Extract billing period from startDate for the StaffMonthlyInvoice dedup row
  const billingYear = startDate.getUTCFullYear();
  const billingMonth = startDate.getUTCMonth() + 1;
  const staffId = contract.staffId ?? null;
  ...
  try {
    invoice = await prisma.$transaction(async (tx) => {
      ...
      // Create StaffMonthlyInvoice row for dedup protection against monthly cron
      let monthlyRow = null;
      if (staffId) {
        try {
          monthlyRow = await tx.staffMonthlyInvoice.create({
            data: {
              staffId,
              billingYear,
              billingMonth,
          ...
          // P2002 = concurrent creation or already exists — non-fatal
          if (err?.code !== "P2002") {
            console.error("[staff-invoice] StaffMonthlyInvoice creation failed (non-blocking)", err);
          }
        }
      }

      const inv = await issueInvoice(tx, {
        ...(monthlyRow ? { monthlyInvoiceId: monthlyRow.id } : { contractId: contract.id }),
      ...
      // Mark StaffMonthlyInvoice as generated if it was created
      if (monthlyRow) {
        await tx.staffMonthlyInvoice.update({
          where: { id: monthlyRow.id },
          data: { invoiceId: inv.id, status: "GENERATED" },
        });
      }

      // Initialize nextInvoiceDate on Contract
      await tx.contract.update({
        where: { id: contract.id },
        data: { nextInvoiceDate },
      });

      // Initialize nextInvoiceDate on Staff
      if (staffId) {
  ...
  try {
    ...
    const result = await sendEmail({
      ...
      html:
        ...
        `<p>Échéance : <strong>${dueDateLabel}</strong>.</p>` +
