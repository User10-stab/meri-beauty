...
import { rentInvoiceInput } from "@/lib/staff-rent-payment";
...
/**
 * The invoice « Accepter » would issue on a pending row of the Factures page,
 * built without issuing it: same inputs as the accept paths (the shared
...
async function rentInput(id) {
  ...
  return { input: rentInvoiceInput({ paymentId: rent.paymentId, amount: Number(rent.amount), lineDescription: rent.lineDescription, customer }) };
}
...
export async function buildPendingInvoicePreview({ kind, id }) {
  ...
  return {
    invoice: {
      ...draft.data,
      // No number exists until acceptance — none is reserved for a preview.
      number: "—",
      issuedAt: new Date(),
