...
import { reviewReservationCancellationRequest } from "@/actions/reservations/cancellation-request";
...
/** Ateliers and formations share one review queue — the decision and its
 *  consequences (cancel + refund the deposit) are identical for both. */
function describe(request) {
  if (request.workshopReservation) {
    return {
      kindLabel: "Atelier",
      title: request.workshopReservation.session?.workshop?.title ?? "Atelier",
      startDate: request.workshopReservation.session?.startDate,
      seats: request.workshopReservation.seatsCount,
      payment: request.workshopReservation.payment,
    };
  }
  return {
    kindLabel: "Formation",
...
export function ReservationCancellationRequestsClient({ initialRequests }) {
  ...
  async function decide(requestId, decision) {
    ...
    const result = await reviewReservationCancellationRequest({
      requestId,
      decision,
  ...
  if (!requests.length) {
    return (
      <div className="rounded-xl border border-stroke bg-white p-6 text-sm text-gray-500 dark:border-dark-3 dark:bg-gray-dark">
    ...
    <div className="space-y-4">
      {requests.map((request) => {
        ...
        return (
          <article
            key={request.id}
            className="rounded-xl border border-stroke bg-white p-5 shadow-1 dark:border-dark-3 dark:bg-gray-dark"
          >
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-dark-6">
                  Client : {request.requestedBy.email}
                  {request.requestedBy.phone ? ` · ${request.requestedBy.phone}` : ""}
                </p>
                {info.payment && (
                  <p className="text-sm text-gray-500 dark:text-dark-6">
                    Déjà payé : {Number(info.payment.paidAmount).toFixed(2)} € ({info.payment.status})
                  </p>
                )}
              </div>
              <span
            ...
            <div className="mt-4 rounded-lg bg-gray-50 p-3 text-sm text-dark dark:bg-dark-2 dark:text-white">
              <span className="font-semibold">Motif du client : </span>
              {request.reason}
            </div>
              ...
              <div className="mt-4 space-y-3">
                ...
                <div className="flex flex-wrap justify-end gap-2">
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-lg border border-red-200 px-3 py-2 text-sm font-semibold text-red-700 disabled:opacity-50"
                  >
                    <X size={15} /> Refuser
                  </button>
                  <button
                  ...
                  >
                    {processingId === request.id ? <Loader2 size={15} className="animate-spin" /> : <Check size={15} />}
                    Accepter et rembourser l&apos;acompte
                  </button>
                </div>
              </div>
            ) : (
