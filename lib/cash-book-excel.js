...
export async function buildCashBookWorkbook({ ledger, report }) {
  ...
  if (report) {
    ...
    synth.mergeCells(`A${nextRow}:D${nextRow}`);
    synth.getCell(`A${nextRow}`).value = `COMPARAISON — PÉRIODE PRÉCÉDENTE (${new Date(report.previousPeriod.from).toLocaleDateString("fr-BE")} au ${new Date(report.previousPeriod.to).toLocaleDateString("fr-BE")})`;
    styleSection(synth.getCell(`A${nextRow}`));
    synth.getRow(nextRow).height = 22;
    nextRow += 1;
    addTable(synth, {
      ...
      rows: [
        ["Ventes espèces", report.previousPeriod.totalSales],
        ["Total entrées", report.previousPeriod.entrees],
        ["Total sorties", report.previousPeriod.sorties],
        ["Solde", report.previousPeriod.finalBalance],
      ],
      moneyColumns: [2],
      widths: [28, 18],
    });
