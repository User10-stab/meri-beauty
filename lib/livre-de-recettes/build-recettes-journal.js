 ...
 * therefore the payments whose frozen owner is the salon
 * (`Payment.payeeStaffId` null — see lib/payments/resolve-payee.js, which
 * keeps Marie Mercier's sales the salon's). Online purchases a customer makes
 * herself are salon revenue too. See lib/authorization/salon-scope.js —
 * never re-derive that rule here.
 *
...
export async function buildRecettesJournal(client, { from, to, fromDate, toDate, method, category }) {
  const found = await client.transaction.findMany({
    where: {
      ...
      isDeleted: false,
      ...(method !== "ALL" ? { method } : {}),
      // The salon's own takings only — never an independent's, and there is
      // no parameter that widens this. In the `where`, not filtered afterwards,
      // so `truncated` and every total describe exactly what was kept.
      // See lib/authorization/salon-scope.js.
    ...
    include: {
      payment: {
        include: {
          invoice: { select: { number: true, vatRate: true, source: true, customerName: true, customerEmail: true } },
  ...
  for (const transaction of transactions) {
    ...
    rows.push({
      ...
      reference:
        ...
        null,
      // Staff rent has no customer relation — the invoice names the staff member.
      customerName: customer?.fullName ?? transaction.payment?.invoice?.customerName ?? null,
      customerEmail: customer?.email ?? transaction.payment?.invoice?.customerEmail ?? null,
      category: catCode,
  ...
  return {
    ...
    summary: {
      ...
      // Net of refunds, like `total` — the three always satisfy HT + TVA = total.
      totalHt: roundMoney(rows.reduce((sum, row) => sum + (row.isRefund ? -1 : 1) * row.amountHt, 0)),
      totalVat: roundMoney(rows.reduce((sum, row) => sum + (row.isRefund ? -1 : 1) * row.amountVat, 0)),
      grossInflow,
      refundTotal,
...
function labelForPayment(payment, { isRefund }) {
  ...
  if (payment.invoice?.source === "STAFF_CONTRACT") return `${prefix}Loyer staff — ${payment.invoice.customerName ?? ""}`.trim();
  return `${prefix}Vente`;
}
