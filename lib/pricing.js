 ...
 *
 * Reads `variant.priceExclVat`, never `variant.price`: serializeCart already
 * added Belgian VAT to `price` so the cart can display a legal consumer price,
 * and taxing that again would compound 21 % on 21 %. The net figure is carried
 * alongside precisely so this can reprice at whatever rate actually applies.
 */
...
export function calculateItemPricing(item, vatRate = TVA_RATE) {
  ...
  // Catalogue prices are net. Apply the rate to the unit before multiplying,
  // so the cart preview follows the same rounding as order creation.
