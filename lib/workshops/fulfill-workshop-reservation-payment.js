...
import { resolvePayeeForWorkshopSession, resolvePayeeFromCheckout, payeePaymentData } from "@/lib/payments/resolve-payee";
 ...
 */
export async function confirmWorkshopReservationPayment(session, { stripeAccountId = null } = {}) {
  ...
  if (reservation.status === "CONFIRMED") {
    ...
    console.error(
      "[confirmWorkshopReservationPayment] second payment on an already-confirmed reservation, flagging for manual refund:",
      session.id,
      reservation.id
    );
  ...
  ({ invoice } = await prisma.$transaction(async (tx) => {
    ...
    const payment = await tx.payment.create({
      data: {
        ...
        ...payeePaymentData(
          await resolvePayeeFromCheckout(tx, session, () => resolvePayeeForWorkshopSession(tx, { sessionId: reservation.sessionId })),
          { stripeAccountId }
        ),
    ...
    if (isFullPayment && !independentSale) {
      await allocatePaymentTicketNumber(tx, payment.id, "WORKSHOP", reservation.session.workshop.type);
    }
    ...
    if (err.message === "HOLD_EXPIRED_OVERBOOKED" || err.message === "INVALID_SESSION_CAPACITY") {
      ...
      sendEmail({
        ...
        text:
          ...
          `Votre réservation pour l'atelier "${reservation.session.workshop.title}" n'a pas pu être confirmée : la dernière place a été réglée par une autre personne juste avant vous. ` +
        ...
        html:
          ...
          `<p>Votre réservation pour l'atelier "${reservation.session.workshop.title}" n'a pas pu être confirmée : la dernière place a été réglée par une autre personne juste avant vous. ` +
