...
export default function ReservationFormationSuccesPage() {
  return (
    <Suspense fallback={null}>
      <ReservationFormationSuccesContent />
    </Suspense>
  );
}
...
function ReservationFormationSuccesContent() {
  ...
  useEffect(() => {
    ...
    // payment asynchronously and can lag a few seconds behind the Stripe
    // redirect, so poll briefly for PENDING_DEPOSIT -> CONFIRMED.
    let cancelled = false;
    let attempts = 0;
    ...
    async function fetchReservation() {
      try {
        const res = await fetch(`/api/formation-reservations/${reservationId}`);
