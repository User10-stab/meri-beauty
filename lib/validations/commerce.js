...
export const closeShippedOrderSchema = z.object({
  ...
  collectedAt: z.coerce
    .date({ required_error: "La date de retrait au point relais est obligatoire.", invalid_type_error: "Date invalide." })
...
export const cancelOrderSchema = z.object({
  ...
  manualRefundConfirmed: z.coerce.boolean().optional().default(false),
...
export const requestReturnSchema = z.object({
  ...
  reasonCategory: z.enum(RETURN_REASON_CATEGORIES, {
    required_error: "Merci d'indiquer le motif du retour.",
    invalid_type_error: "Motif de retour invalide.",
  }),
  reason: z
    .string({ required_error: "Merci de préciser le motif du retour." })
    .trim()
    ...
    .array(
      z.object({
        ...
        condition: z.enum(RETURN_ITEM_CONDITIONS, {
          required_error: "Indiquez l'état de chaque article retourné avant de finaliser.",
        }),
      })
    )
...
export const rejectReturnRequestSchema = returnActionSchema.extend({
  staffNote: z
    .string({ required_error: "Indiquez le motif du refus — il sera envoyé au client." })
    .trim()
