 ...
 * @param {"CUSTOMER_SELF_CANCELLATION"|"CUSTOMER_REQUEST_APPROVED"|"SALON_CANCELLATION"|"NO_SHOW_EXCEPTION"|"SHOP_RETURN"} input.trigger
 ...
 * @param {boolean} [input.customerIsBusiness]
...
export async function queueManualRefund(
  ...
  if (inFlight) {
    // This function is called inside the cancellation/return transaction.
    // Throwing rolls back any credit note or fulfilment changes made by that
    // attempt, so callers cannot report success with a second note and no
    // corresponding refund legs.
    const error = new Error("REFUND_ALREADY_PENDING");
    error.operationId = inFlight.id;
    throw error;
  }

  const planned = allocateRefund(transactions, Number(amount));
  const plannedAmount = planned.reduce((sum, leg) => sum + Number(leg.amount), 0);
  // Never turn a decided amount into a smaller work item. That means the
  // ledger cannot justify the refund, so callers must roll back and send it
  // to reconciliation rather than leaving part of the debt untracked.
  if (Math.abs(plannedAmount - Number(amount)) > REFUND_EPSILON) {
    throw new Error("REFUND_ALLOCATION_INCOMPLETE");
  }
  if (planned.length === 0) return null;

  // Whose sale this is (see lib/payments/resolve-payee.js). An independent
  // practitioner's refund is hers to make: off the salon's cash book, no
  // salon credit note expected, and it lands on her worklist, not the salon's.
  const owner = await tx.payment.findUnique({
    where: { id: paymentId },
    select: { payeeStaffId: true, payeeStaff: { select: { userId: true } } },
  });
  const independentSale = Boolean(owner?.payeeStaffId);
  const salonCashRefund = (leg) => leg.method === "CASH" && !independentSale;

  const openCashSession = planned.some(salonCashRefund)
  ...
  for (const leg of planned) {
    legData.push({
      ...
      pieceNumber: salonCashRefund(leg) ? await allocatePieceNumber(tx, pieceSeriesFor(source, activityType)) : null,
      cashSessionId: salonCashRefund(leg) ? openCashSession?.id ?? null : null,
    });
  }

  // The salon never documented her sale, so there is no credit note to
  // demand for it — her B2B paperwork is hers.
  if (!creditNoteId && customerIsBusiness && !independentSale) {
    throw new Error("B2B_REFUND_REQUIRES_CREDIT_NOTE");
  }
  ...
  // She has money to give back that she may not know about yet (a customer
  // cancelled online, say). Written in this transaction, so the notice
  // exists exactly when the debt does.
  if (independentSale && owner.payeeStaff?.userId) {
    const total = Number(operation.totalAmount).toFixed(2).replace(".", ",");
    await tx.notification.create({
      data: {
        userId: owner.payeeStaff.userId,
        type: "GENERAL",
        title: "Remboursement à effectuer",
        message: `${total} € sont à rembourser à votre client. Retrouvez le détail dans Mes opérations.`,
        status: "PENDING",
        actionUrl: "/dashboard/mes-operations",
      },
    });
  }
