 ...
 *
 * @param {string} [actorId] When given, every row (both CASH transactions
 *   and drawer movements) is scoped to what THIS user personally recorded —
 *   see Transaction.recordedById / CashMovement.recordedById. The real
 *   dashboard action always passes the caller's own id (nobody, including
 *   the owner, sees another operator's rows any more); tests may omit it to
 *   exercise the ledger math against a mixed fixture. Once scoped, the
 *   running "solde" is this operator's own cumulative total, not the
 *   physical drawer balance — see CaisseClient.jsx's labeling.
 */
export async function buildCashBookLedger(client, { fromDate, toDate, actorId }) {
