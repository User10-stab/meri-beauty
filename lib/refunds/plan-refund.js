 ...
 *
 * Imported directly by scripts/audit-refund-states.mjs, which must run
 * through `npm run audit:refund-states` — this is a `.js` in a package with
 * no "type", so Node 20 reads it as CommonJS and the bare `node` invocation
 * cannot see these named exports. See that script's header.
 *
...
export function refundedByMethod(transactions = []) {
  const totals = { CASH: 0, CARD: 0, ONLINE: 0, TRANSFER: 0 };
...
export function classifyRefundReprise({ transactions = [], invoice = null } = {}) {
  ...
  // Money already went back (a Dashboard refund or a Stripe-confirmed refund) but the
