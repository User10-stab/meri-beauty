 ...
 * PENDING to CONFIRMED status). The customer must complete payment to finalize.
 * 
 * @param {{
 *   customerName: string,
 *   serviceName: string,
...
export function reservationAcceptedWithPaymentLinkEmail({
  ...
  const subject = "Votre réservation a été acceptée – Procédez au paiement";
  ...
  } else if (allowedPaymentMethods === 'CASH_ONLY') {
...
⚠️ **Attention : Cette réservation n'est pas encore confirmée**

Pour finaliser votre réservation, vous devez procéder au paiement. Cliquez sur le lien ci-dessous pour accéder à la page de paiement :

${paymentUrl}

Sur la page de paiement, vous pourrez choisir entre :
${paymentOptionsText}

Une fois le paiement complété, vous recevrez une confirmation définitive de votre réservation.
  ...
  const html = htmlWrapper(
    ...
    <div style="background:#fff3cd;border-left:4px solid #856404;padding:16px;border-radius:4px;margin-bottom:24px;">
      <p style="margin:0 0 8px;font-size:14px;color:#856404;line-height:1.6;">
        ⚠️ <strong>Cette réservation n'est pas encore confirmée</strong><br/>
        Vous devez compléter le paiement pour finaliser votre réservation.
      </p>
    </div>
    ...
    <div style="background:#d1ecf1;border-left:4px solid #0c5460;padding:16px;border-radius:4px;margin-bottom:24px;">
      <p style="margin:0 0 12px;font-size:14px;color:#0c5460;line-height:1.6;">
        💳 <strong>Paiement requis pour confirmer la réservation</strong><br/>
        Cliquez sur le bouton ci-dessous pour accéder à la page de paiement.
      </p>
      <p style="margin:0;font-size:13px;color:#0c5460;">
        Vous pourrez choisir entre :<br/>
        • <strong>Paiement en ligne (100%)</strong> : réservation confirmée immédiatement<br/>
        • <strong>Paiement sur place (acompte de 10% en ligne)</strong> : payez un acompte, puis réglez le solde au salon
      </p>
    </div>
    ...
    <p style="margin:0 0 24px;font-size:13px;color:#999;">
      Le bouton vous redirigera vers notre page de paiement sécurisée pour choisir votre mode de paiement.
    </p>

    ${divider()}

    <p style="margin:0;font-size:14px;color:#666;line-height:1.6;">
      Après paiement, vous recevrez une confirmation définitive de votre réservation.<br/>
...
export function reservationConfirmedEmail({
  ...
  const subject = "Votre rendez-vous est réservé — Meri Beauty";
  ...
  const html = htmlWrapper(
    ...
    <table cellpadding="0" cellspacing="0" style="width:100%;background:#f9f8f5;border-radius:8px;padding:4px 20px;margin-bottom:24px;">
      <tbody>
        ...
        ${infoRow("Date", formatDate(date))}
        ${infoRow("Heure", time)}
        ${infoRow("Total", `<span style="color:${brandColor};font-size:16px;">€${Number(totalAmount).toFixed(2)}</span>`)}
      </tbody>
    </table>
 ...
 */
export function returnCompletedEmail({ customerName, orderNumber, refundAmount, refundFailed = false }) {
