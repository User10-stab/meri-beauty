 ...
 */
function generateAllTimeSlots(workStartMinutes, workEndMinutes, slotInterval, serviceDuration, freeIntervals, reservationStarts = null) {
  const allSlots = [];
  ...
  while (slotStart < workEndMinutes) {
    ...
    // Available only if it matches a canonical reservation window. When reservationStarts
    // is not provided (legacy callers), fall back to free-interval containment.
    const available = reservationStarts
      ? reservationStarts.has(slotStart)
      : freeIntervals.some((free) => slotStart >= free.start && serviceEnd <= free.end);
...
export function buildAvailabilityForDate({ staffService, selectedDate, salon, existingAppointments }) {
  ...
  // A slot is marked `available` only if it exactly matches a canonical reservation window.
  // This guarantees the UI never shows a slot as clickable that the backend will reject
  // via hasReservationWindow / validateAppointmentSlot (the old code used free-interval
  // containment, which enabled e.g. 09:30 even though only 09:00/10:00 are actually bookable).
  const SLOT_INTERVAL_MINUTES = 30; // Fixed display interval
  const reservationStarts = new Set(reservationWindows.map((w) => w.startMinutes));
  const allSlots = generateAllTimeSlots(workStartMinutes, workEndMinutes, SLOT_INTERVAL_MINUTES, duration, freeIntervals, reservationStarts);
