...
|---|---|---|---|
| **L1** | **RGPD / legal pages missing** (CGV, Politique de confidentialité, Mentions légales) | ⏳ | Drafted using Marie's real business data (statut personne physique, BCE/TVA BE0751.854.027, adresse Rue Bonaventure 113 1090 Jette, téléphone, email) — `app/(public)/cgv`, `mentions-legales`, `politique-de-confidentialite`, wired into `Footer.jsx` and `app/sitemap.js`. Grounded in the site's real behavior (50/50 acompte non remboursable, 14-day droit de rétractation, TVA 21%, Stripe/Resend/Neon/Mondial Relay as sous-traitants, cookies actually in use). **Not launch-ready yet**: still has one bracketed placeholder (Marie's full legal name, required for a `personne physique`) and the final hosting provider name — and the whole draft needs Marie's/a legal read before publishing. |
