...
import { peekNextInvoiceNumber, prepareInvoice } from "@/lib/invoicing";
import { buildStaffCustomer, olderUnissuedRents, rentInvoiceInput } from "@/lib/staff-rent-payment";
 ...
 * *InvoiceInput helpers) and same guards and figures (prepareInvoice). It
 * shows the number the invoice would get, but takes none and writes nothing. A row that would not get an
...
async function rentInput(id) {
  ...
  // Older rents take their numbers first: this one comes after them.
  const ahead = (await olderUnissuedRents(prisma, rent)).length;
  return { ahead, input: rentInvoiceInput({ paymentId: rent.paymentId, amount: Number(rent.amount), lineDescription: rent.lineDescription, customer, dueDate: rent.dueDate }) };
}
...
export async function buildPendingInvoicePreview({ kind, id }) {
  ...
  // Shown so the preview reads like the real invoice (user's call,
  // 2026-09-22) — read, not reserved: it is attributed only at issue.
  const number = await peekNextInvoiceNumber(prisma, planned.ahead ?? 0);
