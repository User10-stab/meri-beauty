...
export const staffServiceAssignmentSchema = z.object({
  ...
  margin: z.coerce.number({ invalid_type_error: "La marge est invalide." }).nonnegative("La marge ne peut pas être négative.").optional().nullable(),
...
export const staffServiceUpdateSchema = z.object({
  ...
  price: z.coerce.number({ invalid_type_error: "Le prix est invalide." }).nonnegative("Le prix ne peut pas être négatif.").optional(),
  duration: z.coerce.number({ invalid_type_error: "La durée est invalide." }).int("La durée doit être un nombre entier.").nonnegative("La durée ne peut pas être négative.").optional(),
  margin: z.coerce.number({ invalid_type_error: "La marge est invalide." }).nonnegative("La marge ne peut pas être négative.").optional().nullable(),
  photo: z.string().trim().max(500, "L’URL de la photo est trop longue.").optional().nullable(),
  isActive: z.boolean().optional(),
});
