 ...
 *   When they are not a till cash operator (see canUseSalonTill), the
...
export async function settleReservation({
  ...
  // WORKSHOP covers both ateliers and événements, told apart only by
  // Activity.type — see lib/activities/activity-kind.js.
  const activityType = kind === "WORKSHOP" ? reservation.session?.workshop?.type : null;
  ...
  // An independent practitioner's sale (Payment.payeeStaffId) is off-till
  // whoever collects it — even the admin or Marie: it never enters the
  // salon's cash book, and the salon issues no ticket or invoice for it.
  // The salon's sale enters the till when the collector may run it
  // (canUseSalonTill: Marie, the admins, a CAISSE staff member).
  const offTill = Boolean(payment.payeeStaffId) || !(await canUseSalonTill(actor));
  ...
  // lib/payments/awaited-transfer.js. No attestation, no till session and no
  // invoice here; only the amount expected is written down.
  const awaitsTransfer = collectsAtTill && isAwaitedTransfer(method);
  // Off-till (a non-operator, or an independent animator's formation): refuse
  // rather than fall through to the collection branch, which would mark the
  // balance paid although nothing was received.
  ...
  // « Carte QR »: the client paid on their own phone. The attestation is not
  // a tick-box but Stripe itself, asked here before a centime is recorded —
  // for this reservation and this exact amount (lib/counter/qr-checkout.js).
  const paidByQr = collectsAtTill && isCounterQr(method);
  let qrPayment = null;
  if (paidByQr) {
    qrPayment = await verifyCounterQrPayment(qrSessionId, {
      surface: kind === "WORKSHOP" ? COUNTER_QR_SURFACES.WORKSHOP : COUNTER_QR_SURFACES.FORMATION,
  ...
  try {
    result = await prisma.$transaction(async (tx) => {
      ...
      if (!hasBalanceDue) {
        ...
        await allocatePaymentTicketNumber(tx, payment.id, kind, activityType, new Date(), offTill);
      ...
      // The client leaves without paying: the booking is closed, the balance
      // stays due and the transfer is expected. No Transaction, no ticket and
      // no invoice until it is accepted.
      if (awaitsTransfer) {
        ...
        await tx.auditLog.create({
          data: {
            ...
            entityType: kind === "WORKSHOP" ? "WorkshopReservation" : "FormationReservation",
            entityId: reservationId,
            after: { status: "COMPLETED", balanceCollected: 0, paymentMethod: AWAITED_TRANSFER_METHOD, awaitedTransferAmount: balance },
      ...
      // A QR charge lives on Stripe, never in the drawer: ONLINE, no cash
      // session, no piece number, and the payment intent as its reference.
      ...
      const collection = await tx.transaction.create({
        data: {
          ...
          amount: balance,
          // A card terminal collection records as CARD; a QR charge as
          // ONLINE; a cash handover — whether it joins the till or is taken
          // off-till — records as CASH.
      ...
      await allocatePaymentTicketNumber(tx, updatedPayment.id, kind, activityType, new Date(), offTill);
      ...
      // checkbox exists anywhere in the app for this exact reason. A
      // non-privileged staff member (offTill) can never cause an Invoice to
      // be created — see canUseSalonTill — resolveSettlementInvoice is a
      // hard no-op for them.
      const { invoice, creditNote } = await resolveSettlementInvoice(tx, {
        existingInvoice: payment.invoice,
        priceChanged: priceAdjustment.changed,
  ...
  if (result.awaited) {
    return {
      success: true,
      message: `Réservation clôturée — virement de €${result.balance.toFixed(2)} attendu. Acceptez-le dans « Ventes en attente de paiement » à sa réception.`,
    };
  }
 ...
 */
export async function markReservationNoShow({ kind, reservationId, actorId, actor }) {
  const config = RESERVATION_KINDS[kind];
  if (!config) return { success: false, message: "Type de réservation inconnu." };
  if (!reservationId) return { success: false, message: "Identifiant de réservation manquant." };
  ...
  const actorUsesTill = await canUseSalonTill(actor);
  ...
  try {
    const claimed = await prisma.$transaction(async (tx) => {
      ...
      if (payment && Number(payment.paidAmount) > 0.01 && !payment.invoice) {
        // A non-privileged staff member (offTill) can never cause an
        // Invoice to be created — see canUseSalonTill — the no-show still
        // gets recorded and the deposit kept, it simply never gets an
        // invoice.
        const offTillActor = !actorUsesTill || Boolean(payment.payeeStaffId);
        if (hasInvoiceableVatIdentity(reservation.customer) && !offTillActor) {
        ...
        await allocatePaymentTicketNumber(tx, payment.id, kind, activityType, new Date(), offTillActor);
      }
