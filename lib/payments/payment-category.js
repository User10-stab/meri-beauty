 ...
 * @returns {"ORDER"|"APPOINTMENT"|"FORMATION"|"WORKSHOP"|"EVENT"|"STAFF_RENT"|null}
 */
export function categoryForPayment(payment) {
  if (!payment) return null;
  // Staff rent: its source is the rent contract (lib/staff-rent-payment.js);
  // a rent invoice issued before that link existed is recognised by its source.
  if (payment.staffContractId || payment.invoice?.source === "STAFF_CONTRACT") return "STAFF_RENT";
...
/**
 * The customer behind a polymorphic Payment, by walking the same source
 * relation as categoryForPayment — needed wherever a VAT rate has to be
 * estimated for a not-yet-invoiced payment (resolveGoodsVatPolicy /
 * resolveServiceVatPolicy in lib/tax-policy.js both branch on whether the
 * customer is a VIES-validated foreign-EU company).
 *
 * @param {object|null} payment - a Payment with its source relations included,
 *   each carrying its own `user`/`customer` select.
 */
export function customerForPayment(payment) {
  if (!payment) return null;
  return (
    payment.order?.user ??
