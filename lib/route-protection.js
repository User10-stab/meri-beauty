...
import { isAdminRole, canAccessDashboard, hasDashboardPermission, isTillCashOperator, canUseSalonTill } from "@/lib/authorization";
...
/** Require one granular STAFF capability. OWNER/ADMIN always pass. */
/**
 * Pages that belong to the salon itself — the till, the Livre de caisse,
 * boutique orders. Not a grantable permission: only the admin and Marie
 * Mercier (isTillCashOperator, despite her STAFF role) get through; every
 * other practitioner is independent and is redirected.
 *
 * @returns {Promise<{ user: Object, session: Object }>}
 */
export async function requireTillCashOperator() {
  const { user, session } = await requireDashboard();
  if (!isTillCashOperator(user)) redirect("/dashboard");
  return { user, session };
}

/**
 * The till screen (« Caisse »): the salon's accounts plus any staff member
 * granted CAISSE — see canUseSalonTill. The Livre de caisse and Commandes
 * stay on requireTillCashOperator.
 *
 * @returns {Promise<{ user: Object, session: Object }>}
 */
export async function requireSalonTill() {
  const { user, session } = await requireDashboard();
  if (!(await canUseSalonTill(user))) redirect("/dashboard");
  return { user, session };
}

export async function requireDashboardPermission(permission) {
  const { user, session } = await requireDashboard();

  if (!(await hasDashboardPermission(user, permission))) {
    redirect("/dashboard");
  }
