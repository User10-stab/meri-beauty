...
export async function saveCheckoutVatNumber(client, user, rawVatNumber) {
  ...
  if (!viesResult.success) {
    // VIES outage fallback — OFF by default (env flag), flipped on only while
    // the EU registry or a member-state node is unreachable.
    //
    // Narrow on purpose: a Belgian number whose mod-97 checksum already passed
    // isValidVatFormat() above. A BE B2B sale is DOMESTIC 21% regardless of
    // B2B status (resolveGoodsVatPolicy), so provisional acceptance loses no
    // VAT — only the printed company identity is not registry-confirmed yet.
    // Foreign-EU numbers still hard-fail here: their 0 % reverse charge must
    // never rest on an unverified number. A VIES that is reachable and says
    // "not registered" (viesResult.valid === false, below) also still fails.
    const outageFallback =
      process.env.VAT_VIES_OUTAGE_FALLBACK === "true" &&
      viesResult.reason === "VIES_UNAVAILABLE" &&
      getVatCountryCode(vatNumber) === "BE";
    ...
    const provisional = await client.user.update({
      where: { id: user.id },
      data: {
        isCompany: true,
        vatNumber,
        vatValidatedAt: new Date(),
        // No registry answer — leave the name blank rather than inventing one.
        // The invoice labels B2C-with-VAT-number until a legal name is supplied
        // (BillingProfile.companyLegalName) or VIES re-verifies on a later sale.
        vatValidationName: null,
        vatValidationAddress: null,
      },
    });
    return { success: true, user: provisional };
  }
  if (!viesResult.valid) {
    return {
      success: false,
